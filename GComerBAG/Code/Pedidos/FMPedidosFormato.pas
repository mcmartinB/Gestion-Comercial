...
begin
  case ActiveControl.Tag of
    ...
    kSuministro: DespliegaRejilla(btnSuministro,[cliente_pdc.Text]);
    ...
    begin
      calibre_pdd.Tag:= kCalibre;
      DespliegaRejilla(btnCalibre,[empresa_pdc.Text, DSDetalle.DataSet.FieldByName('producto_pdd').AsString ] );
      calibre_pdd.Tag:= kCalibre;
    end;
    kCategoria:
    begin
      categoria_pdd.Tag:= kCategoria;
      DespliegaRejilla(btnCategoria,[empresa_pdc.Text, DSDetalle.DataSet.FieldByName('producto_pdd').AsString ] );
      categoria_pdd.Tag:= kCategoria;
    end;
    kColor:
    begin
      color_pdd.Tag:= kColor;
      DespliegaRejilla(btnColor,[empresa_pdc.Text, DSDetalle.DataSet.FieldByName('producto_pdd').AsString ] );
      color_pdd.Tag:= kColor;
    end;
...
begin
  ...
  begin
    ...
    stEnvase.Caption:= desEnvaseProducto( empresa_pdc.Text, envase_pdd.Text, producto_pdd.Text );
...
begin
  rImpuestosActual:= ImpuestosCliente( empresa_pdc.Text, centro_pdc.Text, cliente_pdc.Text, dir_suministro_pdc.Text, StrToDateTimeDef( fecha_pdc.Text, Now ) );
...
begin
  ...
  STCliente.Caption := desCliente(cliente_pdc.Text);
...
begin
  ...
  if SeleccionaFormatoCliente( self,formato_cliente_pdd, empresa_pdc.Text, producto_pdd.Text,
                               cliente_pdc.Text, dir_suministro_pdc.Text, fecha_pdc.Text, sAux ) then
    formato_cliente_pdd.Text:= sAux;
end;

procedure TfrmMPedidosFormato.producto_pddChange(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      DSDetalle.DataSet.FieldByName('producto_pdd').AsString:= 'PLA';
    end
    else
    begin
    ...
    begin
      case TipoIvaProductoBase( empresa_pdc.Text, producto_base_pdd.Text ) of
...
begin
  ...
  begin
    stEnvase.Caption:= desEnvaseProducto( empresa_pdc.Text, envase_pdd.Text, producto_pdd.Text );
    if producto_pdd.Text = '' then
    begin
      porc_iva_pdd.Text:= '';
    end
    else
    begin
      case TipoIvaEnvase( empresa_pdc.Text, envase_pdd.Text, producto_pdd.Text ) of
        0: porc_iva_pdd.Text:= FloatToStr( rImpuestosActual.rSuper );
        1: porc_iva_pdd.Text:= FloatToStr( rImpuestosActual.rReducido );
        2: porc_iva_pdd.Text:= FloatToStr( rImpuestosActual.rGeneral );
      end;
