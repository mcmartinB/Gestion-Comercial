...
begin
  ...
  QListado.SQL.Add('        where envase_e = envase_pdd ');
  QListado.SQL.Add('        and producto_e = producto_pdd ) peso_neto_e, ');
  QListado.SQL.Add('        ( select nvl(unidades_e,0) ');
  QListado.SQL.Add('        from frf_envases ');
  QListado.SQL.Add('        where envase_e = envase_pdd ');
  QListado.SQL.Add('        and producto_e = producto_pdd ) unidades_e ');
...
begin
  ...
  QUnidad.SQL.Add(' and producto_ce = :producto');
  ...
  QTotalDetalle.SQL.Add('             when unidad_pdd = ''U'' then Round( unidades_pdd / ');
  QTotalDetalle.SQL.Add('                                                 ( select max(nvl(case when unidades_e = 0 then 1 else unidades_e end,1)) ');
  QTotalDetalle.SQL.Add('                                                   from frf_envases ');
  QTotalDetalle.SQL.Add('                                                   where envase_e = envase_pdd ');
  QTotalDetalle.SQL.Add('                                                   and producto_e = producto_pdd ) ) ');
  QTotalDetalle.SQL.Add('             when unidad_pdd = ''K'' then Round( unidades_pdd /  ');
  QTotalDetalle.SQL.Add('                                                 ( select max(nvl(case when peso_neto_e = 0 then 1 else peso_neto_e end ,1)) ');
  QTotalDetalle.SQL.Add('                                                   from frf_envases ');
  QTotalDetalle.SQL.Add('                                                   where envase_e = envase_pdd ');
  QTotalDetalle.SQL.Add('                                                   and producto_e = producto_pdd ) ) ');
