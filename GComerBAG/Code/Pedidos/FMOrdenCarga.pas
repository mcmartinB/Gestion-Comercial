...
uses CVariables, CGestionPrincipal, UDMBaseDatos, UDMAuxDB, CReportes,
  CAuxiliarDB, Principal, LBancos, DError, DPreview, bSQLUtils;

{$R *.DFM}

procedure TfrmOrdenCarga.AbrirTablas;
begin
     // Abrir tablas/Querys
...
procedure TfrmOrdenCarga.CerrarTablas;
begin
     // Cerrar tabla
...
procedure TfrmOrdenCarga.FormCreate(Sender: TObject);
begin
     //Variables globales
  M := Self;
  MD := nil;
 ...
 {+}Order := ' ORDER BY empresa_b, banco_b, descripcion_b ';
     //Abrir tablas/Querys
  try
    AbrirTablas;
...
//     empresa_t.Tag:=kEmpresa;
  ...
  {+}FocoLocalizar := empresa_b;

end;

{+ CUIDADIN }

procedure TfrmOrdenCarga.FormActivate(Sender: TObject);
begin
...
procedure TfrmOrdenCarga.FormDeactivate(Sender: TObject);
begin
 {*}//Por si acaso el nuevo form no necesita rejilla
  gRF := nil;
end;

procedure TfrmOrdenCarga.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  //Liberamos recursos
  Lista.Free;
...
procedure TfrmOrdenCarga.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmOrdenCarga.Filtro;
var Flag: Boolean;
  i: Integer;
begin
  where := ''; Flag := false;
...
procedure TfrmOrdenCarga.AnyadirRegistro;
var Flag: Boolean;
  i: Integer;
begin
  Flag := false;
...
procedure TfrmOrdenCarga.ValidarEntradaMaestro;
var i: Integer;
begin
  for i := 0 to Lista.Count - 1 do
  begin
...
procedure TfrmOrdenCarga.Previsualizar;
var
  enclave: TBookMark;
begin
  ...
  QRLBancos.PrinterSettings.PrinterIndex := giPrintListados;
...
procedure TfrmOrdenCarga.AntesDeModificar;
var i: Integer;
begin
  for i := 0 to Lista.Count - 1 do
  begin
...
procedure TfrmOrdenCarga.AntesDeVisualizar;
var i: Integer;
begin
    //Resaturar estado
  for i := 0 to Lista.Count - 1 do
...
procedure TfrmOrdenCarga.RequiredTime(Sender: TObject;
  var isTime: Boolean);
begin
  isTime := false;
...
procedure TfrmOrdenCarga.FormCanResize(Sender: TObject; var NewWidth,
  NewHeight: Integer; var Resize: Boolean);
begin
...
procedure TfrmOrdenCarga.ARejillaFlotanteExecute(Sender: TObject);
begin
  if empresa_b.Focused then
    DespliegaRejilla(BGridButton1);
end;

procedure TfrmOrdenCarga.DSMaestroDataChange(Sender: TObject; Field: TField);
begin
  empresa_des.Caption := desEmpresa(empresa_b.Text);
end;
