...
type
  TQRGastosTransitos = class(TQuickRep)
    TitleBand1: TQRBand;
...
procedure TQRGastosTransitos.QRSysData1Print(sender: TObject; var Value: string);
begin
  if Tag > 0 then
    Value := Value + ' de ' + IntToStr(Tag);
end;

procedure TQRGastosTransitos.ChildBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TQRGastosTransitos.qreempresa_svPrint(sender: TObject; var Value: String);
begin
  Value:= value + ' ' + desEmpresa( value );
end;

procedure TQRGastosTransitos.qrestatus_svPrint(sender: TObject; var Value: String);
begin
  if Value = '0' then
  begin
...
constructor TQRGastosTransitos.Create(AOwner: TComponent);
begin
  inherited;
  with QSalidas do
  begin
...
destructor TQRGastosTransitos.Destroy;
begin
  QSalidas.Close;
  Inherited;
end;

procedure TQRGastosTransitos.QServiciosAfterOpen(DataSet: TDataSet);
begin
  QSalidas.Open;
  QGastos.Open;
end;

procedure TQRGastosTransitos.QServiciosBeforeClose(DataSet: TDataSet);
begin
  QSalidas.Close;
  QGastos.Close;
end;

procedure TQRGastosTransitos.bndCabSalidasBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:= not QSalidas.IsEmpty;
end;

procedure TQRGastosTransitos.qrecliente2Print(sender: TObject;
  var Value: String);
var
  sAux: string;
begin
  ...
  if sAux = '' then
  begin
    Value:= desCliente( DataSet.FieldByName('empresa_sv').AsString,Value );
  end
  else
  begin
    Value:= sAux;
...
procedure TQRGastosTransitos.bndCabGastosBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:= not QGastos.IsEmpty;
end;

procedure TQRGastosTransitos.qreTipo1Print(sender: TObject;
  var Value: String);
begin
  Value:= desTipoGastos( Value );
end;

procedure TQRGastosTransitos.qreProducto1Print(sender: TObject;
  var Value: String);
begin
