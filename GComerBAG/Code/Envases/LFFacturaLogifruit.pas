...
type
  ...
  private
     ...
     bPrimeraVez: boolean;
     dIni, dFin: TDateTime;
     bEntradas,bSalidas, bTransitos: Boolean;

    function  CamposVacios: boolean;
    procedure Imprimir;
    function  GetEntradas( const AFlag: Boolean ): Boolean;
    procedure PutEntradasSQL( const ACentro, AEnvaseComer: Boolean );
    function  GetVentas( const AFlag: Boolean ): Boolean;
    procedure PutVentaCajasSQL( const ACliente, ACentro, AEnvaseComer, AExcluir: Boolean );
    procedure PutVentaPaletsSQL( const ACliente, ACentro, AEnvaseComer, AExcluir: Boolean );
    function  GetTransitos( const AFlag: Boolean ): Boolean;
    procedure PutTransitosSQL( const ACentro, AEnvaseComer: Boolean );
    //procedure PutTransitosCajasSQL( const ACentro, AEnvaseComer: Boolean );
    //procedure PutTransitosPaletsSQL( const ACentro, AEnvaseComer: Boolean );
    function  CargarDatosListado: boolean;
    function  AnyadirLineasDeDatos: Boolean;
...
begin
  ...
  kmtListado.FieldDefs.Add('referencia', ftString, 15, False);
  ...
  kmtResumen.FieldDefs.Add('envase', ftString, 5, False);
...
begin
  ...
  bTransitos:= chkTransitos.Checked;
  if not( bEntradas or bSalidas or bTransitos ) then
  begin
    bEntradas:= chkEntradas.Enabled;
    bSalidas:= True;
    bTransitos:= True;
  end;
...
procedure TFLFacturaLogifruit.PutEntradasSQL( const ACentro, AEnvaseComer: Boolean );
begin
  ...
  begin
    ...
    SQL.Add('       referencia_ece referencia, cod_producto_ece envase, cantidad_ece cantidad ');
...
procedure TFLFacturaLogifruit.PutVentaCajasSQL( const ACliente, ACentro, AEnvaseComer, AExcluir: Boolean );
begin
  with qryMovimientos do
  begin
    SQL.Clear;
    ...
    SQL.Add('       cast( n_albaran_sl as char(15) ) referencia, env_comer_producto_e envase, cajas_sl cantidad   ');
...
procedure TFLFacturaLogifruit.PutVentaPaletsSQL( const ACliente, ACentro, AEnvaseComer, AExcluir: Boolean );
begin
  with qryMovimientos do
  begin
    SQL.Clear;
    ...
    SQL.Add('       cliente_sal_sc cliente, dir_sum_sc almacen,  fecha_sl fecha, ');
    SQL.Add('       cast( n_albaran_sl as char(15) ) referencia, env_comer_producto_tp envase, n_palets_sl cantidad   ');
...
procedure TFLFacturaLogifruit.PutTransitosSQL( const ACentro, AEnvaseComer: Boolean );
begin
  (*BAG*)
  (*Solo salidas, paga el centro que envia*)
  with qryMovimientos do
  begin
    SQL.Clear;

    SQL.Add('select empresa_ect empresa, cod_operador_ect operador,');
    SQL.Add('       centro_ect centro, 3 tipo, planta_destino_ect cliente,');
    SQL.Add('       centro_destino_ect almacen, fecha_ect fecha, nota_ect referencia,');
    SQL.Add('       cod_producto_ect envase, cantidad_ect cantidad ');
...
{
procedure TFLFacturaLogifruit.PutTransitosCajasSQL( const ACentro, AEnvaseComer: Boolean );
begin
  (*BAG*)
  (*Solo salidas, paga el centro que envia*)
  with qryMovimientos do
  begin
    ...
    SQL.Add('select planta_origen_tc empresa, env_comer_operador_e operador,');
    SQL.Add('       centro_tl centro, 3 tipo, planta_destino_tc cliente,');
    SQL.Add('       centro_destino_tc almacen, fecha_tc fecha, cast( referencia_tl as char(10) ) referencia,');
    SQL.Add('       env_comer_producto_e envase, cajas_tl cantidad ');
...
procedure TFLFacturaLogifruit.PutTransitosPaletsSQL( const ACentro, AEnvaseComer: Boolean );
begin
  (*BAG*)
  (*Solo salidas, paga el centro que envia*)
  with qryMovimientos do
  begin
    ...
    SQL.Add('select planta_origen_tc empresa, env_comer_operador_tp operador,');
    SQL.Add('       centro_tc centro, 3 tipo, planta_destino_tc cliente,');
    SQL.Add('       centro_destino_tc almacen, fecha_tc fecha, cast( referencia_tl as char(10) ) referencia,');
    SQL.Add('       env_comer_producto_tp envase, palets_tl cantidad ');
...
function TFLFacturaLogifruit.AnyadirLineasDeDatos: Boolean;
begin
  with qryMovimientos do
  begin
    Open;
...
function TFLFacturaLogifruit.GetEntradas( const AFlag: Boolean ): Boolean;
begin
  if AFlag then
  begin
    with qryMovimientos do
    begin
      ...
      PutEntradasSQL( Trim(edtCentro.Text) <> '', Trim(edtEnvComer.Text) <> '' );
...
function TFLFacturaLogifruit.GetVentas( const AFlag: Boolean ): Boolean;
begin
  if AFlag then
  begin
    with qryMovimientos do
    begin
      ...
      PutVentaCajasSQL( Trim(edtCliente.Text) <> '', Trim(edtCentro.Text) <> '', Trim(edtEnvComer.Text) <> '', chkExcluir.Checked );
      ParamByName('empresa').AsString:= edtEmpresa.Text;
      ParamByName('operador').AsString:= edtOperador.Text;
      ParamByName('fechaini').AsDateTime:= dIni;
      ParamByName('fechafin').AsDateTime:= dFin;
      ...
      PutVentaPaletsSQL( Trim(edtCliente.Text) <> '', Trim(edtCentro.Text) <> '', Trim(edtEnvComer.Text) <> '', chkExcluir.Checked );
      ParamByName('empresa').AsString:= edtEmpresa.Text;
      ParamByName('operador').AsString:= edtOperador.Text;
      ParamByName('fechaini').AsDateTime:= dIni;
      ParamByName('fechafin').AsDateTime:= dFin;
      ...
      result:= AnyadirLineasDeDatos or result;
    end;
  end
  else
  begin
...
function TFLFacturaLogifruit.GetTransitos( const AFlag: Boolean ): Boolean;
begin
  if AFlag then
  begin
    with qryMovimientos do
    begin
      ...
      PutTransitosSQL( Trim(edtCentro.Text) <> '', Trim(edtEnvComer.Text) <> '' );
      ParamByName('empresa').AsString:= edtEmpresa.Text;
      ParamByName('operador').AsString:= edtOperador.Text;
      ParamByName('fechaini').AsDateTime:= dIni;
      ParamByName('fechafin').AsDateTime:= dFin;
...
{
      PutTransitosCajasSQL( Trim(edtCentro.Text) <> '', Trim(edtEnvComer.Text) <> '' );
      ParamByName('empresa').AsString:= edtEmpresa.Text;
      ParamByName('operador').AsString:= edtOperador.Text;
      ParamByName('fechaini').AsDateTime:= dIni;
      ParamByName('fechafin').AsDateTime:= dFin;
      ...
      PutTransitosPaletsSQL( Trim(edtCentro.Text) <> '', Trim(edtEnvComer.Text) <> '' );
      ParamByName('empresa').AsString:= edtEmpresa.Text;
      ParamByName('operador').AsString:= edtOperador.Text;
      ParamByName('fechaini').AsDateTime:= dIni;
      ParamByName('fechafin').AsDateTime:= dFin;
...
begin
  ...
  *)
  ...
  result:= GetEntradas( bEntradas );
  result:= GetVentas( bSalidas ) or result;
  result:= GetTransitos( bTransitos ) or result;
