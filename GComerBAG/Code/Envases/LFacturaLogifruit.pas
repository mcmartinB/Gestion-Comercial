...
begin
  ...
  if DataSet.FieldbyName('tipo').AsString = '3' then
  begin
    Value:= Value + ' '  + desCentro( DataSet.FieldbyName('cliente').AsString, Value );
  end;
end;
...
begin
  ...
  if DataSet.FieldbyName('tipo').AsString = '3' then
    Value:= 'MOVIMIENTOS ENTRE CENTROS';
end;
...
begin
  ...
  if DataSet.FieldbyName('tipo').AsString = '3' then
    Value:= 'TRÁNSITOS ' + DataSet.FieldbyName('empresa').AsString + '/' + DataSet.FieldbyName('centro').AsString;
end;
