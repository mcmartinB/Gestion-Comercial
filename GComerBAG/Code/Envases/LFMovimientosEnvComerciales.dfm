object FLMovimientosEnvComerciales: TFLMovimientosEnvComerciales
  Left = 352
  Top = 129
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'MOVIMIENTOS ENVASE COMERCIAL/RETORNABLE'
  ClientHeight = 452
  ClientWidth = 491
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 491
    Height = 452
    Align = alClient
    BevelInner = bvLowered
    TabOrder = 0
    object SpeedButton1: TSpeedButton
      Left = 257
      Top = 389
      Width = 89
      Height = 25
      Action = BAceptar
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
    end
    object SpeedButton2: TSpeedButton
      Left = 351
      Top = 389
      Width = 89
      Height = 25
      Action = BCancelar
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
    end
    object Label1: TLabel
      Left = 44
      Top = 37
      Width = 100
      Height = 19
      AutoSize = False
      Caption = ' Empresa'
      Color = cl3DLight
      ParentColor = False
      Layout = tlCenter
    end
    object btnEmpresa: TBGridButton
      Left = 188
      Top = 36
      Width = 13
      Height = 21
      Action = ADesplegarRejilla
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Control = edtEmpresa
      Grid = RejillaFlotante
      GridAlignment = taDownRight
      GridWidth = 280
      GridHeigth = 100
    end
    object lblDesde: TLabel
      Left = 44
      Top = 62
      Width = 100
      Height = 19
      AutoSize = False
      Caption = ' Fecha Desde'
      Color = cl3DLight
      ParentColor = False
      Layout = tlCenter
    end
    object btnFechaDesde: TBCalendarButton
      Left = 228
      Top = 61
      Width = 17
      Height = 21
      Action = ADesplegarRejilla
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Control = edtFechaDesde
      Calendar = CalendarioFlotante
      CalendarAlignment = taDownLeft
      CalendarWidth = 197
      CalendarHeigth = 145
    end
    object lbl1: TLabel
      Left = 279
      Top = 62
      Width = 55
      Height = 19
      AutoSize = False
      Caption = ' Hasta'
      Color = cl3DLight
      ParentColor = False
      Layout = tlCenter
    end
    object btnFechaHasta: TBCalendarButton
      Left = 416
      Top = 61
      Width = 17
      Height = 21
      Action = ADesplegarRejilla
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Control = edtFechaHasta
      Calendar = CalendarioFlotante
      CalendarAlignment = taDownLeft
      CalendarWidth = 197
      CalendarHeigth = 145
    end
    object Label2: TLabel
      Left = 44
      Top = 134
      Width = 100
      Height = 19
      AutoSize = False
      Caption = ' Envases Comer.'
      Color = cl3DLight
      ParentColor = False
      Layout = tlCenter
    end
    object lblCentro: TLabel
      Left = 44
      Top = 87
      Width = 100
      Height = 19
      AutoSize = False
      Caption = ' Centro Salida'
      Color = cl3DLight
      ParentColor = False
      Layout = tlCenter
    end
    object btnCentro: TBGridButton
      Left = 188
      Top = 86
      Width = 13
      Height = 21
      Action = ADesplegarRejilla
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Control = edtCentro
      Grid = RejillaFlotante
      GridAlignment = taDownRight
      GridWidth = 280
      GridHeigth = 100
    end
    object btnEnvComer: TBGridButton
      Left = 203
      Top = 133
      Width = 13
      Height = 21
      Action = ADesplegarRejilla
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Control = edtEnvComer
      Grid = RejillaFlotante
      GridAlignment = taDownRight
      GridWidth = 280
      GridHeigth = 100
    end
    object lblOperador: TLabel
      Left = 44
      Top = 110
      Width = 100
      Height = 19
      AutoSize = False
      Caption = ' Operador Envases'
      Color = cl3DLight
      ParentColor = False
      Layout = tlCenter
    end
    object btnOperador: TBGridButton
      Left = 188
      Top = 109
      Width = 13
      Height = 21
      Action = ADesplegarRejilla
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Control = edtOperador
      Grid = RejillaFlotante
      GridAlignment = taDownRight
      GridWidth = 280
      GridHeigth = 100
    end
    object lblVer: TLabel
      Left = 44
      Top = 180
      Width = 100
      Height = 19
      AutoSize = False
      Caption = ' Marcar para ver'
      Color = cl3DLight
      ParentColor = False
      Layout = tlCenter
    end
    object lbl2: TLabel
      Left = 263
      Top = 183
      Width = 175
      Height = 13
      Caption = '(Todos si no seleccionamos alguno)'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsItalic, fsUnderline]
      ParentFont = False
    end
    object lbl3: TLabel
      Left = 44
      Top = 157
      Width = 100
      Height = 19
      AutoSize = False
      Caption = ' Totalizar Stock'
      Color = cl3DLight
      ParentColor = False
      Layout = tlCenter
    end
    object edtEmpresa: TBEdit
      Left = 147
      Top = 36
      Width = 40
      Height = 21
      ColorEdit = clMoneyGreen
      Zeros = True
      MaxLength = 3
      TabOrder = 0
      OnChange = PonNombre
    end
    object txtEmpresa: TStaticText
      Left = 202
      Top = 38
      Width = 236
      Height = 17
      AutoSize = False
      BorderStyle = sbsSunken
      TabOrder = 1
    end
    object edtFechaDesde: TBEdit
      Left = 146
      Top = 61
      Width = 81
      Height = 21
      ColorEdit = clMoneyGreen
      InputType = itDate
      MaxLength = 10
      TabOrder = 2
    end
    object edtFechaHasta: TBEdit
      Left = 333
      Top = 61
      Width = 81
      Height = 21
      ColorEdit = clMoneyGreen
      InputType = itDate
      MaxLength = 10
      TabOrder = 3
    end
    object edtEnvComer: TBEdit
      Left = 147
      Top = 133
      Width = 55
      Height = 21
      ColorEdit = clMoneyGreen
      MaxLength = 5
      TabOrder = 8
      OnChange = PonNombre
    end
    object edtCentro: TBEdit
      Left = 147
      Top = 86
      Width = 25
      Height = 21
      ColorEdit = clMoneyGreen
      MaxLength = 1
      TabOrder = 4
      OnChange = PonNombre
    end
    object txtCentro: TStaticText
      Left = 202
      Top = 88
      Width = 236
      Height = 17
      AutoSize = False
      BorderStyle = sbsSunken
      TabOrder = 5
    end
    object txtEnvComer: TStaticText
      Left = 218
      Top = 135
      Width = 220
      Height = 17
      AutoSize = False
      BorderStyle = sbsSunken
      TabOrder = 9
    end
    object edtOperador: TBEdit
      Left = 147
      Top = 109
      Width = 40
      Height = 21
      ColorEdit = clMoneyGreen
      MaxLength = 3
      TabOrder = 6
      OnChange = PonNombre
    end
    object txtOperador: TStaticText
      Left = 202
      Top = 111
      Width = 236
      Height = 17
      AutoSize = False
      BorderStyle = sbsSunken
      TabOrder = 7
    end
    object chkIN: TCheckBox
      Left = 147
      Top = 181
      Width = 95
      Height = 17
      Caption = 'IN: Inventario'
      TabOrder = 11
    end
    object chkAJ: TCheckBox
      Left = 147
      Top = 203
      Width = 200
      Height = 17
      Caption = 'AJ: Ajustes'
      TabOrder = 12
    end
    object chkEC: TCheckBox
      Left = 147
      Top = 225
      Width = 200
      Height = 17
      Caption = 'EC: Entradas cajas'
      TabOrder = 13
    end
    object chkETF: TCheckBox
      Left = 147
      Top = 270
      Width = 200
      Height = 17
      Caption = 'ETF: Entradas por tr'#225'nsitos de fruta'
      TabOrder = 15
    end
    object chkSTC: TCheckBox
      Left = 147
      Top = 293
      Width = 200
      Height = 17
      Caption = 'STC: Salidas por tr'#225'nsitos de caja'
      TabOrder = 16
    end
    object chkSTF: TCheckBox
      Left = 147
      Top = 315
      Width = 200
      Height = 17
      Caption = 'STF: Salidas por tr'#225'nsitos de fruta'
      TabOrder = 17
    end
    object chkSVF: TCheckBox
      Left = 147
      Top = 338
      Width = 200
      Height = 17
      Caption = 'SVF: Salidas por ventas de fruta'
      TabOrder = 18
    end
    object chkETC: TCheckBox
      Left = 147
      Top = 248
      Width = 200
      Height = 17
      Caption = 'ETC: Entradas por tr'#225'nsitos de cajas'
      TabOrder = 14
    end
    object chkDesglosar: TCheckBox
      Left = 168
      Top = 358
      Width = 200
      Height = 17
      Caption = 'Separar albaranes de venta y tr'#225'nsitos'
      TabOrder = 19
    end
    object chkDiario: TCheckBox
      Left = 147
      Top = 158
      Width = 95
      Height = 17
      Caption = 'Diario'
      TabOrder = 10
    end
  end
  object RejillaFlotante: TBGrid
    Left = 444
    Top = 103
    Width = 137
    Height = 135
    Color = clInfoBk
    FixedColor = clInfoText
    Options = [dgTabs, dgRowSelect, dgCancelOnExit]
    ReadOnly = True
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Visible = False
  end
  object CalendarioFlotante: TBCalendario
    Left = 467
    Top = 36
    Width = 197
    Height = 153
    Date = 36717.643820682870000000
    ShowToday = False
    TabOrder = 1
    Visible = False
    WeekNumbers = True
  end
  object ListaAcciones: TActionList
    Images = DMBaseDatos.ImgBotones
    Left = 72
    Top = 12
    object BAceptar: TAction
      Caption = 'Aceptar'
      ShortCut = 112
      OnExecute = BBAceptarClick
    end
    object BCancelar: TAction
      Caption = 'Cancelar'
      ShortCut = 27
      OnExecute = BBCancelarClick
    end
    object ADesplegarRejilla: TAction
      ImageIndex = 0
      ShortCut = 113
      OnExecute = ADesplegarRejillaExecute
    end
  end
end
