...
type
  TFMTransitosEnvComerciales = class(TMaestro)
    ...
    lblNota: TLabel;
    nota_ect: TBDEdit;
    lblPlantaDestino: TLabel;
    btnPlantaDestino: TBGridButton;
    planta_destino_ect: TBDEdit;
    txtPlantaDestino: TStaticText;
  ...
  private
    ...
    sPlantaOrigen, sCentroOrigen, sPlantaDestino, sCentroDestino, sOperador, sFecha: string;
...
begin
 ...
 {+}Order := ' ORDER BY empresa_ect, fecha_ect, centro_ect, cod_operador_ect, cod_producto_ect ';
  ...
  planta_destino_ect.Tag := kEmpresa;
...
begin
  ...
  if planta_destino_ect.Text <> '' then
  begin
    if flag then
      where := where + ' and ';
    where := where + ' planta_destino_ect LIKE ' + SQLFilter(planta_destino_ect.Text);
    flag := True;
  end;
...
begin
  ...
  if planta_destino_ect.Text <> '' then
  begin
    where := where + ' and planta_destino_ect = ' + QuotedStr(planta_destino_ect.Text);
  end;
  if centro_destino_ect.Text <> '' then
  begin
...
begin
  ...
  if txtPlantaDestino.Caption = '' then
  begin
    planta_destino_ect.Focused;
    raise Exception.Create('Falta el código de la planta destino o es incorrecto.');
  end;
  ...
  sPlantaOrigen:= empresa_ect.Text;
  sCentroOrigen:= centro_ect.Text;
  sPlantaDestino:= planta_destino_ect.Text;
  sCentroDestino:= centro_destino_ect.Text;
  sOperador:= cod_operador_ect.Text;
  sFecha:= fecha_ect.Text;
...
begin
  ...
  DMBaseDatos.QListado.SQL.Add( ' order by empresa_ect, centro_ect, planta_destino_ect, centro_destino_ect, cod_operador_ect, cod_producto_ect, fecha_ect ');
  try
...
begin
  ...
  case ActiveControl.Tag of
    kEmpresa:
    begin
      if empresa_ect.Focused then
         DespliegaRejilla(btnEmpresa)
      else
      if planta_destino_ect.Focused then
        DespliegaRejilla(btnPlantaDestino);
    end;
    kCentro:
    begin
      ...
      if centro_destino_ect.Focused then
        DespliegaRejilla(btnDestino, [planta_destino_ect.Text]);
    end;
...
procedure TFMTransitosEnvComerciales.AntesDeInsertar;
begin
  empresa_ect.Text:= sPlantaOrigen;
  centro_ect.Text:= sCentroOrigen;
  planta_destino_ect.Text:= sPlantaDestino;
  centro_destino_ect.Text:= sCentroDestino;
  cod_operador_ect.Text:= sOperador;
  fecha_ect.Text:= sFecha;
end;
  ...
  case TComponent(Sender).Tag of
    kEmpresa:
      begin
        ...
        txtPlantaDestino.Caption := desEmpresa(planta_destino_ect.Text);
        txtDestino.Caption := desCentro(planta_destino_ect.Text, centro_destino_ect.Text);
      end;
    kCentro:
