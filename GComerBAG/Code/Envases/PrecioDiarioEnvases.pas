...
uses
  ...
  dxSkinBlue, dxSkinBlueprint, dxSkinFoggy, dxSkinMoneyTwins, cxControls,
  cxContainer, cxEdit, cxTextEdit, cxDBEdit, cxButtons, SimpleSearch;
...
type
  TFPrecioDiarioEnvases = class(TMaestro)
    ...
    procedure SimpleSearch1AntesEjecutar(Sender: TObject);
...
begin
  ...
  begin
    raise Exception.Create('Unidad de facturacion incorrecta, solo puede se U,K, o C.');
  end;
...
begin
  ...
  if Trim(Where) <> '' then
  begin
    QRLPrecioDiarioEnvases.QListado.SQL.Add( Where );
    QRLPrecioDiarioEnvases.QListado.SQl.Add(' and empresa_epd = ' + QuotedStr( empresa_epd.Text ) );
  end
  else
  begin
    QRLPrecioDiarioEnvases.QListado.SQl.Add(' where empresa_epd =' + QuotedStr( empresa_epd.Text ) );
  end;
  QRLPrecioDiarioEnvases.QListado.SQl.Add(' and envase_e= envase_epd ');
...
procedure TFPrecioDiarioEnvases.SimpleSearch1AntesEjecutar(Sender: TObject);
begin
    ssEnvase.SQLAdi := '';
...
begin
  ...
  begin
    ...
    begin
      sCentro:= '1';
      //sCentro:= gsDefCentro;
      //if InputQuery('PRECIOS DIARIOS POR ENVASES', 'Introduce centro', sCentro ) then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if GetPrecioDiarioEnvases.Preguntar( APlanta + ' PRECIOS DIARIOS POR ARTICULOS (' + DateToStr( AFecha ) + ')',
                                          'Precio ' + FieldByName('descripcion_e').AsString + ' (' +
//                                          FieldByName('empresa_e').AsString + '/' +
