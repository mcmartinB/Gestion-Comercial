...
type
  ...
  public
    procedure PreparaListado(
      ...
      const AAgrupar, AVisualizar: integer; const AVerAlbaran: boolean );
  end;

  function VerListadoSalidasPorFactura( const AOwner: TComponent;
    const AEmpresa, ACentro, ACliente, AProducto, AFechaIni, AFechaFin: string;
    const AAgrupar, AVisualizar: integer; const AVerAlbaran: boolean ): Boolean;
...
begin
  ...
  try
    QLSalidasPorFactura.PreparaListado( AEmpresa, ACentro, ACliente, AProducto, AFechaIni, AFechaFin, AAgrupar, AVisualizar, AVerAlbaran );
...
begin
  ...
  try
    result:= DLSalidasPorFactura.DatosQuerySalidasPorFactura( AEmpresa, ACentro, ACliente, AProducto, AFechaIni, AFechaFin, AAgrupar, AVisualizar, AVerAlbaran );
    if result then
    begin
      VerListadoSalidasPorFacturaEx( AOwner, AEmpresa, ACentro, ACliente, AProducto, AFechaIni, AFechaFin, AAgrupar, AVisualizar, AVerAlbaran );
    end;
  finally
...
begin
  ...
  if ACliente <> '' then
    lblCliente.Caption:= DesCliente( ACliente )
  else
  ...
  qrlbl1.Enabled:= AVerAlbaran;
  qrlbl2.Enabled:= AVerAlbaran;
  qrdbtxt1.Enabled:= AVerAlbaran;
  qrdbtxt2.Enabled:= AVerAlbaran;
...
procedure TQLSalidasPorFactura.qrdbtxt2Print(sender: TObject;
  var Value: String);
begin
  if Value <> '' then
    Value:= FormatDateTime('dd/mm/yy', DLSalidasPorFactura.QSalidasPorFactura.FieldByName('fecha_albaran').AsDateTime );
end;
