...
begin
  ...
  begin
    ...
    if AVerAlbaran then
      SQL.Add('        n_albaran_sc albaran, fecha_sc fecha_albaran, ')
    else
      SQL.Add('        n_factura_sc albaran, fecha_factura_sc fecha_albaran, ');
    ...
    *)
    ...
    SQL.Add(' group by fecha_factura_sc, n_factura_sc, cliente_fac_sc, producto_sl, albaran, fecha_albaran');
    ...
    if AVerAlbaran then
      SQL.Add('        producto_fal producto, n_albaran_fal albaran, fecha_albaran_fal fecha_albaran, ')
    else
      SQL.Add('        producto_fal producto, factura_fal albaran, fecha_fal fecha_albaran, ');
    ...
    SQL.Add('   and producto_e = producto_fal ');

    SQL.Add(' group by fecha_fal, factura_fal, cliente_fac_f, producto_fal, n_albaran_fal, fecha_albaran_fal  ');
...
function TDLSalidasPorFactura.DatosQuerySalidasPorFactura(
  ...
  const AAgrupar, AVisualizar: integer; const AVerAlbaran: Boolean ): boolean;
begin
  PreparaQuerySalidasPorFactura( ACentro, ACliente, AProducto, AFechaIni, AFechaFin, AAgrupar, AVisualizar, AVerAlbaran );
  with QSalidasPorFactura do
  begin
