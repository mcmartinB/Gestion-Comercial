object FLPaletsConfeccionados: TFLPaletsConfeccionados
  Left = 561
  Top = 151
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = '    Listado Palets Confeccionados.'
  ClientHeight = 263
  ClientWidth = 431
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyUp = FormKeyUp
  DesignSize = (
    431
    263)
  PixelsPerInch = 96
  TextHeight = 13
  object lblEmpresa: TnbLabel
    Left = 32
    Top = 32
    Width = 114
    Height = 21
    Caption = 'Empresa'
    About = 'NB 0.1/20020725'
  end
  object nbLabel2: TnbLabel
    Left = 32
    Top = 56
    Width = 114
    Height = 21
    Caption = 'Centro'
    About = 'NB 0.1/20020725'
  end
  object stEmpresa: TLabel
    Left = 203
    Top = 36
    Width = 133
    Height = 13
    Caption = '(Falta c'#243'digo de la empresa)'
  end
  object stCentro: TLabel
    Left = 203
    Top = 60
    Width = 114
    Height = 13
    Caption = '(Falta c'#243'digo del centro)'
  end
  object Label1: TLabel
    Left = 32
    Top = 183
    Width = 277
    Height = 13
    Caption = 'Este listado muestra en tiempo real los palets que han sido '
  end
  object Label2: TLabel
    Left = 32
    Top = 195
    Width = 150
    Height = 13
    Caption = 'confeccionados el dia indicado.'
  end
  object nbLabel1: TnbLabel
    Left = 32
    Top = 104
    Width = 114
    Height = 21
    Caption = 'Fecha Confec. del'
    About = 'NB 0.1/20020725'
  end
  object nbLabel3: TnbLabel
    Left = 32
    Top = 80
    Width = 114
    Height = 21
    Caption = 'Cliente'
    About = 'NB 0.1/20020725'
  end
  object stCliente: TLabel
    Left = 203
    Top = 84
    Width = 130
    Height = 13
    Caption = '(Vacio = Todos los clientes)'
  end
  object nbLabel4: TnbLabel
    Left = 240
    Top = 104
    Width = 20
    Height = 21
    Caption = 'al'
    About = 'NB 0.1/20020725'
  end
  object btnCerrar: TBitBtn
    Left = 315
    Top = 214
    Width = 90
    Height = 25
    Anchors = [akRight, akBottom]
    Caption = 'Cerrar (Esc)'
    TabOrder = 0
    TabStop = False
    OnClick = btnCerrarClick
    Kind = bkCancel
  end
  object btnImprimir: TBitBtn
    Left = 218
    Top = 214
    Width = 90
    Height = 25
    Anchors = [akRight, akBottom]
    Caption = 'Imprimir (F1)'
    ModalResult = 1
    TabOrder = 1
    TabStop = False
    OnClick = btnImprimirClick
    Glyph.Data = {
      
      0000}
    NumGlyphs = 2
  end
  object empresa: TnbDBSQLCombo
    Left = 147
    Top = 32
    Width = 54
    Height = 21
    About = 'NB 0.1/20020725'
    CharCase = ecUpperCase
    OnChange = empresaChange
    TabOrder = 2
    SQL.Strings = (
      'select empresa_e, nombre_e '
      'from frf_empresas'
      'order by empresa_e')
    DatabaseName = 'BDProyecto'
    FillAuto = True
    OnlyNumbers = False
    NumChars = 3
  end
  object centro: TnbDBSQLCombo
    Left = 147
    Top = 56
    Width = 41
    Height = 21
    About = 'NB 0.1/20020725'
    CharCase = ecUpperCase
    OnChange = centroChange
    TabOrder = 3
    DatabaseName = 'BDProyecto'
    OnGetSQL = centroGetSQL
    NumChars = 1
  end
  object fechaDesde: TnbDBCalendarCombo
    Left = 147
    Top = 104
    Width = 90
    Height = 21
    About = 'NB 0.1/20020725'
    CharCase = ecUpperCase
    Text = '09/11/2007'
    TabOrder = 5
  end
  object cliente: TnbDBSQLCombo
    Left = 147
    Top = 80
    Width = 54
    Height = 21
    About = 'NB 0.1/20020725'
    CharCase = ecUpperCase
    OnChange = clienteChange
    TabOrder = 4
    DatabaseName = 'BDProyecto'
    OnGetSQL = clienteGetSQL
    FillAuto = True
    OnlyNumbers = False
    NumChars = 3
  end
  object cbxClientesNumericos: TCheckBox
    Left = 32
    Top = 127
    Width = 161
    Height = 17
    Caption = 'Ignorar Clientes Num'#233'ricos'
    Checked = True
    State = cbChecked
    TabOrder = 6
  end
  object fechaHasta: TnbDBCalendarCombo
    Left = 266
    Top = 104
    Width = 90
    Height = 21
    About = 'NB 0.1/20020725'
    CharCase = ecUpperCase
    Text = '09/11/2007'
    TabOrder = 7
  end
  object rbtResumen: TRadioButton
    Left = 136
    Top = 126
    Width = 113
    Height = 17
    Caption = 'Resumen'
    Checked = True
    TabOrder = 8
    TabStop = True
    OnClick = rbtResumenClick
  end
  object rbtDetalle: TRadioButton
    Left = 136
    Top = 143
    Width = 113
    Height = 17
    Caption = 'Detalle'
    TabOrder = 9
    OnClick = rbtResumenClick
  end
  object cbxAgruparProducto: TCheckBox
    Left = 176
    Top = 163
    Width = 161
    Height = 17
    Caption = 'Agrupar por producto'
    Enabled = False
    TabOrder = 10
  end
end
