...
type
  ...
  public
    ...
    procedure BorrarGastosEntrega( const ACompra: string );
  end;
...
begin
  ...
  begin
    SQL.Clear;
    SQL.Add(' select tipo_gc, importe_gc, ref_fac_gc, fecha_fac_gc, nota_gc, pagado_gc, ');
    SQL.Add('        empresa_gc, centro_gc, numero_gc, producto_gc ');
  ...
  kbmGastos.FieldDefs.Add('producto_ge', ftString, 3, False);
  kbmGastos.FieldDefs.Add('importe_ge', ftFloat, 0, False);
  kbmGastos.FieldDefs.Add('ref_fac_ge', ftString, 15, False);
...
begin
  ...
  rKilosTotal:= QSumEntregas.FieldByname('kilos_el').AsFloat;
  ...
  begin
    ...
    begin
      sCodigo:= QSelectEntregas.FieldByName('codigo_el').AsString;
      sProducto:= QSelectEntregas.FieldByName('producto_el').AsString;
      rKilosAux:= QSelectEntregas.FieldByName('kilos_el').AsFloat;
  ...
  begin
    ...
    begin
      AMsg:= 'ERROR: Compra sin entregas. ' + sCompra + ' Producto: ' + QSelectLineas.FieldByName('producto_gc').AsString;
    end;
  end;
end;
...
begin
  ...
  kbmGastos.FieldByName('producto_ge').AsString:= QSelectLineas.FieldByName('producto_gc').AsString;
  ...
  if QSelectLineas.FieldByName('fecha_fac_gc').AsString <> '' then
...
procedure TDGastosCompra.BorrarGastosEntrega( const ACompra: string );
begin
  sCompra:= ACompra;
