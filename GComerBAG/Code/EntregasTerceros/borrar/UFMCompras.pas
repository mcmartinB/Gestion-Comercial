...
type
  TFMCompras = class(TMaestro)
    ...
    procedure QComprasBeforeDelete(DataSet: TDataSet);
...
uses UDMAuxDB, CVariables, CGestionPrincipal, UDMBaseDatos, CReportes,
  ...
  UFMGastosCompras, CDGastosCompra, UFMEntregasCompras,
  CompraFichaDL;
...
begin
 ...
 {+}Order := ' ORDER BY empresa_c, fecha_c desc, numero_c desc';
  ...
  begin
    SQL.Clear;
    SQL.Add(' select codigo_ec codigo, producto_ec Prod, fecha_carga_ec carga, albaran_ec albaran ');
  ...
  begin
    SQL.Clear;
    SQL.Add(' select producto_gc Prod, tipo_gc Tipo, ref_fac_gc factura, fecha_fac_gc fecha, importe_gc Importe ');
...
procedure TFMCompras.QComprasBeforeDelete(DataSet: TDataSet);
var
  sCompra: string;
begin
  DGastosCompra:= TDGastosCompra.Create( self );
  try
    sCompra:= '(' + DataSet.FieldByName('empresa_c').AsString + ', ' +
                    DataSet.FieldByName('centro_c').AsString + ', ' +
                    DataSet.FieldByName('numero_c').AsString  + ')';
    DGastosCompra.BorrarGastosEntrega( sCompra );
  finally
    FreeAndNil( DGastosCompra );
  end;
end;
