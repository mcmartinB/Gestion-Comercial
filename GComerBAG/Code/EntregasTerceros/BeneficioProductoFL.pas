...
type
  TFLBeneficioProducto = class(TForm)
    btnCerrar: TBitBtn;
    btnImprimir: TBitBtn;
    lblEmpresa: TnbLabel;
    stEmpresa: TLabel;
...
uses CVariables, Principal, CGestionPrincipal, UDMAuxDB, bTimeUtils,
     DPreview, UDMBaseDatos, UDMConfig, BeneficioProductoQL;

procedure TFLBeneficioProducto.ConfigurarFormulario;
begin
  (*CODIGO VARIABLE*)
...
procedure TFLBeneficioProducto.FormCreate(Sender: TObject);
begin
  (*CODIGO FIJO*)
  FormType:=tfOther;
  BHFormulario;
...
procedure TFLBeneficioProducto.FormClose(Sender: TObject;
...
procedure TFLBeneficioProducto.btnCerrarClick(Sender: TObject);
begin
  Close;
end;

procedure TFLBeneficioProducto.ValidarCampos;
var
...
procedure TFLBeneficioProducto.btnImprimirClick(Sender: TObject);
begin
  ...
  try
    ValidarCampos;
    if not BeneficioProductoQL.ListadoBeneficioProducto( self, Trim(eEmpresa.Text),
      Trim(eProducto.Text), Trim(eProveedor.Text), Trim(eEntrega.Text), Trim(eFechaDesde.Text),
      Trim(eFechaHasta.Text), Trim(eAprovecha.Text),lblMsg2 ) then
...
function TFLBeneficioProducto.eProductoGetSQL: String;
begin
  result:= 'select producto_p, descripcion_p from frf_productos order by producto_p';
end;

function TFLBeneficioProducto.eProveedorGetSQL: String;
begin
...
function TFLBeneficioProducto.eCentroGetSQL: String;
begin
...
procedure TFLBeneficioProducto.FormKeyUp(Sender: TObject;
...
procedure TFLBeneficioProducto.eEmpresaChange(Sender: TObject);
begin
...
procedure TFLBeneficioProducto.eProductoChange(Sender: TObject);
begin
...
procedure TFLBeneficioProducto.eProveedorChange(Sender: TObject);
begin
