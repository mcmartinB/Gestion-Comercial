...
type
  TQLMargenSemanal = class(TQuickRep)
    TitleBand1: TQRBand;
    lblTitulo: TQRLabel;
    ...
    procedure bndTotalesBeforePrint(Sender: TQRCustomBand;
      var PrintBand: Boolean);
    procedure qrecoste_materialPrint(sender: TObject; var Value: String);
  private
    sEmpresa: string;
  public
...
uses
  CReportes, MargenBeneficiosDL, DPreview, UDMAuxDB, bMath, Printers;

var
  QLMargenSemanal: TQLMargenSemanal;
...
procedure VerMargenSemanal( const AOwner: TComponent;
                            ...
                            const AExpandido: boolean  );
begin
  QLMargenSemanal:= TQLMargenSemanal.Create( AOwner );
  try
    QLMargenSemanal.PreparaListado( AEmpresa, ACliente, AProducto, AFechaIni, AFechaFin, AExpandido );
    QLMargenSemanal.bPaletsEntrada:= APaletsEntrada;
    QLMargenSemanal.bPaletsSalida:= APaletsSalida;
    QLMargenSemanal.bVariasSemanas:= ( AFechaFin - AFechaIni ) > 6;
    QLMargenSemanal.bndDetalle.ForceNewPage:= APaletsEntrada or APaletsSalida;
    Previsualiza( QLMargenSemanal );
  finally
    FreeAndNil( QLMargenSemanal );
  end;
end;

procedure TQLMargenSemanal.PreparaListado( const AEmpresa, ACliente, AProducto: string;
                                           const AFechaIni, AFechaFin: TDateTime;
                                           const AExpandido: boolean );
begin
  sEmpresa:= AEmpresa;
...
procedure TQLMargenSemanal.qreProductoPrint(sender: TObject;
  var Value: String);
begin
  Value:= Value + ' ' + desProducto( sEmpresa, Value );
end;

procedure TQLMargenSemanal.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  if Exporting then
  begin
...
procedure TQLMargenSemanal.qrgCabProductoBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:= bVariasSemanas and not Exporting;
end;

procedure TQLMargenSemanal.bndTotalesBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:= not Exporting;
end;

procedure TQLMargenSemanal.qrecoste_materialPrint(sender: TObject;
  var Value: String);
begin
  if Exporting then
