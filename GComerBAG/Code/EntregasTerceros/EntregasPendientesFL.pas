...
begin
  ...
  eSemana.Enabled:= not DMConfig.EsValencia;
...
begin
  ...
  begin
    ...
    iAnyoSem:= StrToInt( eSemana.Text );
  end
  else
  begin
    iAnyoSem:= -1;
...
begin
  ValidarCampos;
  if not DMConfig.EsValencia then
  begin
    if not EntregasPendientesMasetQL.VerListadoEntregasPendientesMaset( self, empresa.Text, centro.Text, edtProducto.Text,
                                                                        eDesde.Text, eHasta.Text, iAnyoSem, cbbEstadoEntrega.ItemIndex,
                                                                        cbbEstadoFecha.ItemIndex ) then
  ...
  begin
    if not EntregasPendientesQL.VerListadoEntregasPendientes( self, empresa.Text, centro.Text, edtProducto.Text,
                                                              eDesde.Text, eHasta.Text, cbbEstadoEntrega.ItemIndex,
                                                              cbbEstadoFecha.ItemIndex ) then
      ShowMEssage('No hay entregas pendientes para los parametros usados.');
  end;
end;
