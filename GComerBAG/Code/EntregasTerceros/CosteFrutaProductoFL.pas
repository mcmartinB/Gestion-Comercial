...
type
  TFLCosteFrutaProducto = class(TForm)
    btnCerrar: TBitBtn;
    btnImprimir: TBitBtn;
    lblEmpresa: TnbLabel;
    lblCentro: TnbLabel;
    ...
    lbl7: TLabel;
    bvl1: TBevel;
    rbVariedad: TRadioButton;
    rbCalibre: TRadioButton;
    rbProveedor: TRadioButton;
    rbSinAgrupar: TRadioButton;
...
uses CVariables, Principal, CGestionPrincipal, UDMAuxDB,
     DPreview, UDMBaseDatos, UDMConfig, CosteFrutaProductoQL;


procedure TFLCosteFrutaProducto.ConfigurarFormulario;
begin
  (*CODIGO VARIABLE*)
  ...
  begin
    lblCentro.Caption:= 'Centro';
    lblFechaDesde.Caption:= 'Fecha Desde';
  end
  else
  *)
  begin
    lblCentro.Caption:= 'Centro Origen';
    lblFechaDesde.Caption:= 'Fecha Origen Desde';
  end;
end;

procedure TFLCosteFrutaProducto.FormCreate(Sender: TObject);
begin
  (*CODIGO FIJO*)
  FormType:=tfOther;
  BHFormulario;
...
procedure TFLCosteFrutaProducto.FormClose(Sender: TObject;
...
procedure TFLCosteFrutaProducto.btnCerrarClick(Sender: TObject);
begin
  Close;
end;

procedure TFLCosteFrutaProducto.ValidarCampos;
var
...
procedure TFLCosteFrutaProducto.btnImprimirClick(Sender: TObject);
begin
  ...
  try
    ValidarCampos;
    if not CosteFrutaProductoQL.VerListadoStock( self, Trim(eEmpresa.Text), Trim(eCentro.Text),
      Trim(eProducto.Text), Trim(eProveedor.Text), Trim(eEntrega.Text), Trim(eFechaDesde.Text),
      Trim(eFechaHasta.Text), rbVariedad.Checked or rbCalibre.Checked, rbCalibre.Checked,
      rbProveedor.Checked or rbEntrega.Checked, rbEntrega.Checked, cbbDetalle.ItemIndex = 0, lblMsg2 ) then
...
function TFLCosteFrutaProducto.eProductoGetSQL: String;
begin
  result:= 'select producto_p, descripcion_p from frf_productos order by producto_p';
end;

function TFLCosteFrutaProducto.eProveedorGetSQL: String;
begin
...
function TFLCosteFrutaProducto.eCentroGetSQL: String;
begin
  if eEmpresa.Text <> '' then
...
procedure TFLCosteFrutaProducto.FormKeyUp(Sender: TObject;
...
procedure TFLCosteFrutaProducto.eEmpresaChange(Sender: TObject);
begin
...
procedure TFLCosteFrutaProducto.eCentroChange(Sender: TObject);
begin
  if Trim(eCentro.Text) = '' then
  begin
...
procedure TFLCosteFrutaProducto.eProductoChange(Sender: TObject);
begin
...
procedure TFLCosteFrutaProducto.eProveedorChange(Sender: TObject);
begin
...
procedure TFLCosteFrutaProducto.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    // Tecla ENTER - Cambio entre los Controles de Datos en modo edición
    //               y entre los Campos de Búsqueda en la localización
  if rbVariedad.Focused or rbCalibre.Focused or rbProveedor.Focused then
  case key of
    vk_Return, vk_down:
      begin
        Key := 0;
