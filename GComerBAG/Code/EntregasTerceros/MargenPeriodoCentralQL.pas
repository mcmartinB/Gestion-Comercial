...
type
  TQLMargenPeriodoCentral = class(TQuickRep)
    TitleBand1: TQRBand;
    lblTitulo: TQRLabel;
...
uses
  CReportes, MargenBeneficiosCentralDL, DPreview, UDMAuxDB, bMath;

var
  QLMargenPeriodoCentral: TQLMargenPeriodoCentral;
...
procedure VerMargenPeriodo( const AOwner: TComponent;
                            ...
                            const APrecioUnidad: boolean  );
begin
  QLMargenPeriodoCentral:= TQLMargenPeriodoCentral.Create( AOwner );
  try
    QLMargenPeriodoCentral.PreparaListado( AEmpresa, ACliente, AProducto, AFechaIni, AFechaFin, APrecioUnidad );
    QLMargenPeriodoCentral.bPaletsEntrada:= APaletsEntrada;
    QLMargenPeriodoCentral.bPaletsSalida:= APaletsSalida;
    QLMargenPeriodoCentral.bndDetalle.ForceNewPage:= APaletsEntrada or APaletsSalida;
    Previsualiza( QLMargenPeriodoCentral );
  finally
    FreeAndNil( QLMargenPeriodoCentral );
  end;
end;

procedure TQLMargenPeriodoCentral.PreparaListado( const AEmpresa, ACliente, AProducto: string;
                                           const AFechaIni, AFechaFin: TDateTime;
                                           const APrecioUnidad: boolean );
begin
  sEmpresa:= AEmpresa;
...
procedure TQLMargenPeriodoCentral.qreProductoPrint(sender: TObject;
  var Value: String);
begin
  Value:= Value + ' ' + desProducto( sEmpresa, Value );
end;

procedure TQLMargenPeriodoCentral.bndDetalleBeforePrint(
...
begin
  ...
  begin
    bnddPaletsConfeccionados.DataSet.Filter:= '';
    bnddPaletsProveedores.DataSet.Filter:= '';
    bnddDetProveedor.DataSet.Filter:= '';
  end;
end;

procedure TQLMargenPeriodoCentral.qregrupoPrint(sender: TObject;
  var Value: String);
begin
  if Value = '0' then
  begin
...
procedure TQLMargenPeriodoCentral.qregrupo_inPrint(sender: TObject;
  var Value: String);
begin
  if Value = '0' then
  begin
...
procedure TQLMargenPeriodoCentral.qreenvasePrint(sender: TObject;
  var Value: String);
begin
    Value:= ' ' + Value + ' ' + desEnvasePBase ( sEmpresa, Value, bnddPaletsConfeccionados.DataSet.FieldByName('producto_base_out').AsString );
end;

procedure TQLMargenPeriodoCentral.qrevariedadPrint(sender: TObject;
  var Value: String);
begin
...
procedure TQLMargenPeriodoCentral.qreproveedorPrint(sender: TObject;
  var Value: String);
begin
    Value:= Value + ' ' + desProveedor ( sEmpresa, Value );
end;

procedure TQLMargenPeriodoCentral.bndCabPaletsConfeccionadosBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  PrintBand:= bPaletsSalida;
end;

procedure TQLMargenPeriodoCentral.bndcChildBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:= not( bPaletsEntrada or bPaletsSalida );
end;

procedure TQLMargenPeriodoCentral.qrl19Print(sender: TObject; var Value: String);
begin
  Value:= '';
end;

procedure TQLMargenPeriodoCentral.bnddPaletsProveedoresBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  PrintBand:= bPaletsEntrada;
end;

procedure TQLMargenPeriodoCentral.qreresumen_proveedorPrint(sender: TObject;
  var Value: String);
begin
  Value:= Value + ' ' + desProveedor( sEmpresa, value );
end;

procedure TQLMargenPeriodoCentral.qreresumen_productoPrint(sender: TObject;
  var Value: String);
begin
  Value:= Value + ' ' + desProducto( sEmpresa, value );
end;

procedure TQLMargenPeriodoCentral.qrl5Print(sender: TObject;
  var Value: String);
begin
