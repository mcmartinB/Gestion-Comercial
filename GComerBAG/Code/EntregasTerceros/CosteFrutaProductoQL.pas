...
type
  ...
  public
    bAgruparVariedad, bAgruparCalibre, bAgruparProveedor, bAgruparEntrega, bResumen: Boolean;
    procedure PreparaListado( const AEmpresa, ACentro, AProducto, AProveedor, AEntrega, AFechaDesde, AFechaHasta: string );
  end;

  function VerListadoStock( const AOwner: TComponent;
                            const AEmpresa, ACentro, AProducto, AProveedor, AEntrega,
                                  AFechaDesde, AFechaHasta: string;
                            const AAgruparVariedad, AAgruparCalibre, AAgruparProveedor, AAgruparEntrega, AVerDetalle: Boolean;
...
procedure VerListadoStockEx( const AOwner: TComponent;
                             const AEmpresa, ACentro, AProducto, AProveedor, AEntrega, AFechaDesde, AFechaHasta: string;
                             const AAgruparVariedad, AAgruparCalibre, AAgruparProveedor, AAgruparEntrega, AVerDetalle: Boolean  );
begin
  QLCosteFrutaProducto:= TQLCosteFrutaProducto.Create( AOwner );
  try
    QLCosteFrutaProducto.PreparaListado( AEmpresa, ACentro, AProducto, AProveedor, AEntrega, AFechaDesde, AFechaHasta );
    QLCosteFrutaProducto.bAgruparVariedad:= AAgruparVariedad;
    QLCosteFrutaProducto.bAgruparCalibre:= AAgruparCalibre;
    QLCosteFrutaProducto.bAgruparProveedor:= AAgruparProveedor;
    QLCosteFrutaProducto.bAgruparEntrega:= AAgruparEntrega;
    if AAgruparProveedor then
      QLCosteFrutaProducto.QRGroupCabVariedad.Expression:= ' [descripcion_p]+ [proveedor]'
    else
      QLCosteFrutaProducto.QRGroupCabVariedad.Expression:= ' [descripcion_p]+ [descripcion_breve_pp]';
    if AAgruparEntrega then
      QLCosteFrutaProducto.QRGroup2.Expression:= '  [descripcion_p]+ [descripcion_breve_pp]+ [entrega]'
    else
      QLCosteFrutaProducto.QRGroup2.Expression:= '  [descripcion_p]+ [descripcion_breve_pp]+ [calibre]';
...
begin
  ...
  try
    result:= DLCosteFrutaProducto.DatosQueryFrutaProveedor( AEmpresa, ACentro, AProducto, AProveedor, AEntrega,
                                                            AFechaDesde, AFechaHasta, AAgruparVariedad, AAgruparCalibre, AAgruparProveedor, AMensaje );
    if result then
    begin
      VerListadoStockEx( AOwner, AEmpresa, ACentro, AProducto, AProveedor, AEntrega, AFechaDesde, AFechaHasta,
                         AAgruparVariedad, AAgruparCalibre, AAgruparProveedor, AAgruparEntrega, AVerDetalle );
    end;
  finally
...
procedure TQLCosteFrutaProducto.PreparaListado( const AEmpresa, ACentro, AProducto, AProveedor, AEntrega, AFechaDesde, AFechaHasta: string );
begin
...
procedure TQLCosteFrutaProducto.QRGroupPieCalibreBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:= bAgruparCalibre or bAgruparEntrega;
...
procedure TQLCosteFrutaProducto.QRGroupPieVariedadBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  PrintBand:= bAgruparVariedad or bAgruparProveedor;
...
procedure TQLCosteFrutaProducto.QRDBText3Print(sender: TObject;
  var Value: String);
begin
  if bAgruparEntrega then
    Value := 'TOTAL ENTREGA ' + DataSet.fieldByName('entrega').AsString
  else
...
procedure TQLCosteFrutaProducto.qrgProveedorBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  PrintBand:= False;
end;
