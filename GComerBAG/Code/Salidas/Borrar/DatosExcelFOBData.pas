unit DatosExcelFOBData;

interface

uses
  SysUtils, Classes, DB, DBClient, Provider, DBTables, kbmMemTable;

type
  TDMDatosExcelFOBData = class(TDataModule)
    Query: TQuery;
...
*)
    ...
    procedure AltaLineaListadoFOB( const ASemana: string );
  ...
  public
    ...
    function  ObtenerDatos(const AEmpresa, ACentroOrigen, ACentroDestino, AFechaDesde, AFEchaHasta,
                           ...
                           const ADescuento, AGasto, ANo036, ACosteEnvase, ACosteSecciones, AAlb6Digitos: boolean;
                           const AAlbFacturado: INteger): boolean;
...
procedure TDMDatosExcelFOBData.PrepareQGastos( const ANo036: boolean );
begin
...
procedure TDMDatosExcelFOBData.DataModuleCreate(Sender: TObject);
begin
  ...
  begin
    SQL.Clear;
    SQL.Add('select first 1 anyo_ec, mes_ec, coste_ec, secciones_ec ');
...
procedure TDMDatosExcelFOBData.DataModuleDestroy(Sender: TObject);
begin
  ClientDataSet.Close;
  mtListado.Close;
...
function TDMDatosExcelFOBData.ObtenerDatos(const AEmpresa, ACentroOrigen, ACentroDestino,
...
begin
  ...
  begin
    ...
    Query.SQL.Add('                   where empresa_tc = :empresa ');
    Query.SQL.Add('                   and referencia_tc in ( select sl.ref_transitos_sl from frf_salidas_l sl ');
    Query.SQL.Add('                                          where sl.empresa_sl = :empresa and sl.centro_salida_sl = centro_salida_sc ');
...
procedure TDMDatosExcelFOBData.RellenaDatosFaltantes( const AEmpresa: string; const ACategoria: string;
                                 const AGasto, ANo036, ACosteEnvase, ASecciones: boolean );
var
...
procedure TDMDatosExcelFOBData.GetKilosAlbaran(var AKilosTotal, AKilosTransito, AKilosProdTotal, AKilosProdTransito: Real;
      const AEmpresa, ACentro: string; const AAlbaran: Integer; const AFecha: TDateTime; const AProducto: string);
begin
  with QKilos do
  begin
...
procedure TDMDatosExcelFOBData.GetGastosAlbaran(
...
procedure TDMDatosExcelFOBData.GetCostesEnvasado( const AEmpresa, ACentro, AEnvase: string;
...
procedure TDMDatosExcelFOBData.ListadoFOB;
var
  sSemana: string;
begin
  mtListado.Close;
  mtListado.FieldDefs.Clear;
  mtListado.FieldDefs.Add('centro_origen', ftString, 1, False);
  mtListado.FieldDefs.Add('centro_salida', ftString, 1, False);
  mtListado.FieldDefs.Add('producto', ftString, 3, False);
  mtListado.FieldDefs.Add('categoria', ftString, 2, False);
  mtListado.FieldDefs.Add('cliente', ftString, 3, False);
  mtListado.FieldDefs.Add('envase', ftString, 3, False);
  ...
  mtListado.FieldDefs.Add('coste_seccion', ftFloat, 0, False);

  mtListado.IndexFieldNames:= 'centro_origen;centro_salida;producto;categoria;cliente;envase;semana';
  mtListado.CreateTable;
  mtListado.Open;
  ...
  begin
    ...
    begin
      ...
      if mtListado.Locate('centro_origen;centro_salida;producto;categoria;cliente;envase;semana',
                     VarArrayOf([FieldByName('centro_origen').AsString, FieldByName('centro').AsString,
                                 FieldByName('producto').AsString, FieldByName('categoria').AsString,
                                 FieldByName('cliente').AsString, FieldByName('envase').AsString, sSemana]), []) then

        ModLineaListadoFOB( sSemana )
      else
        AltaLineaListadoFOB( sSemana );
      Next;
    end;

  end;
  mtListado.SortOn('centro_origen;centro_salida;producto;cliente;envase;semana',[]);
end;

procedure TDMDatosExcelFOBData.AltaLineaListadoFOB( const ASemana: string );
begin
  with mtListado do
  begin
    Insert;
    FieldByName('centro_origen').AsString:= ClientDataSet.FieldByName('centro_origen').AsString;
    FieldByName('centro_salida').AsString:= ClientDataSet.FieldByName('centro').AsString;
    FieldByName('producto').AsString:= ClientDataSet.FieldByName('producto').AsString;
    FieldByName('categoria').AsString:= ClientDataSet.FieldByName('categoria').AsString;
    FieldByName('cliente').AsString:= ClientDataSet.FieldByName('cliente').AsString;
    FieldByName('envase').AsString:= ClientDataSet.FieldByName('envase').AsString;
    ...
    FieldByName('coste_seccion').AsFloat:= ClientDataSet.FieldByName('coste_seccion').AsFloat;

    Post;
  end;
end;

procedure TDMDatosExcelFOBData.ModLineaListadoFOB( const ASemana: string );
begin
  with mtListado do
  begin
    Edit;
