...
type
  ...
  public
    ...
    function GetPorcentajeIva( const AEmpresa, ACliente, AImpuesto: string; const AFecha: TDateTime; var ASuper, AReducido, AGeneral: Real ): boolean;
    ...
    function InsertarFacturaManual(
           ...
           const AUnidad: integer; const AUnidades, APrecio, ANetoAlb, APorcDescuentoAlb, ADescuentoAlb, AImporteAlb: Real;
    ...
    procedure InsertarDetalleManual(
           ...
           const ACentroSalida: string; const AAlbaran: Integer; const AFechaAlb: TDateTime;
           const AProducto, ACentroOrigen, ACategoria, AEnvase: string;
           const AUnidad: integer; const AUnidades, APrecio, ANeto, APorcDescuento, ADescuento, AImporte: Real;
...
begin
  ...
  begin
    ...
    begin
      ...
      if  ( AEmpresa <> 'F23' ) and ( AEmpresa <> 'F22' ) then
      begin
...
function TDMDFacturaManual.GetPorcentajeIva( const AEmpresa, ACliente, AImpuesto: string; const AFecha: TDateTime; var ASuper, AReducido, AGeneral: real ): boolean;
var
  rRecargoSuper, rRecargoReducido, rRecargoGeneral: real;
begin
  rRecargoSuper:= 0;
  rRecargoReducido:= 0;
  rRecargoGeneral:= 0;
  ...
  begin
    ...
    if result then
    begin
      ASuper:= FieldByName('super_il').AsFloat;
      rRecargoSuper:= FieldByName('recargo_super_il').AsFloat;
      AReducido:= FieldByName('reducido_il').AsFloat;
      rRecargoReducido:= FieldByName('recargo_reducido_il').AsFloat;
      AGeneral:= FieldByName('general_il').AsFloat;
  ...
  begin
    ...
    try
      ...
      if not IsEmpty then
      begin
        ASuper := ASuper + rRecargoSuper;
        AReducido := AReducido + rRecargoReducido;
        AGeneral := AGeneral + rRecargoGeneral;
      end;
    finally
      Close;
    end;
  ...
  begin
    ...
    try
      ...
      InsertarDetalleManual(
           ...
           ACentroOrigen, ACategoria, AEnvase, AUnidad, AUnidades, APrecio, ANetoAlb, APorcDescuentoAlb, ADescuentoAlb, AImporte, ACosechero );
...
begin
  ...
  begin
    ...
    FieldByName('neto_fal').AsFloat:= ANeto;
    FieldByName('porc_descuento_fal').AsFloat:= APorcDescuento;
    FieldByName('descuento_fal').AsFloat:= ADescuento;
