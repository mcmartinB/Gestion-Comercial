...
begin
  if ( Exporting and ( Pos( 'XLS', UpperCase( ExportFilter.ClassName ) ) > 0 ) ) then
    ...
    Value:= Value + ' '  + desEnvaseP( sEmpresa, Value, DataSet.FieldByName('producto_fal').AsString );
end;
...
begin
  if ( Exporting and ( Pos( 'XLS', UpperCase( ExportFilter.ClassName ) ) > 0 ) ) then
    Value:= desEnvaseP( sEmpresa, Value, DataSet.FieldByName('producto_fal').AsString )
  else
    Value:= '';
end;
