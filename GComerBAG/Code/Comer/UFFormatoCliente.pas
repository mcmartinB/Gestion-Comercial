...
type
  ...
  function SeleccionaFormatoCliente( const AForm: TForm; const AControl: TControl;
    const AEmpresa, AProducto, ACliente, ASuministro, AFecha: string; var AResultado: string ): Boolean;
...
function QueryFormatoCliente( const AEmpresa, AProducto, ACliente, ASumnistro, AFecha:string ): String;
var
  slAux: TStringList;
begin
  ...
  if Trim(AFecha) <> '' then
  begin
    slAux.Add(' and ( fecha_baja_f is null or fecha_baja_f > :fecha ) ');
    slAux.Add(' and fecha_alta_f <= :fecha ');
  end
  else
  begin
...
begin
  ...
  begin
    ...
    SQL.Clear;
    (*TODO*)
    SQL.Add( QueryFormatoCliente ( AEmpresa, AProducto, ACliente, ASuministro, AFecha ) );

    if Trim(AProducto) <> '' then
    begin
      ParamByName('producto').AsString:= Trim(AProducto);
    end;
    if Trim(AEmpresa) <> '' then
    begin
      ParamByName('empresa').AsString:= Trim(AEmpresa);
    end;
    if Trim(ACliente) <> '' then
    begin
      ParamByName('cliente').AsString:= Trim(ACliente);
    end;
    if Trim(ASuministro) <> '' then
    begin
      ParamByName('dirsum').AsString:= Trim(ASuministro);
    end;
    if Trim(AFecha) <> '' then
    begin
      ParamByName('fecha').AsString:= Trim(AFecha);
    end;
