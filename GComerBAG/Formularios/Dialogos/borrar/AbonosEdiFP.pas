...
uses UDMAuxDB, CAuxiliarDB, DError, CgestionPrincipal, CVariables,
  UDMBaseDatos, bMath, UDMFacturacionEDI, bTextUtils, UDMConfig;

type
  RCabFacturaEdi = record
    codEmpresa, eanEmpresa: string;
    fecha, fechaAlbaran: TDateTime;
    factura, nodo: Integer;
    n_albaran, centro, pedido, moneda: string;
    bruto, porc_descuento, netoAux, descuento, porc_iva, iva, total: real;
    cod_cliente, comprador, receptor, cliente, pagador: string;
    nombre, domicilo, ciudad, cod_postal, nif: string;
  end;

  RLinFacturaEdi = record
    producto, dun14, nombre, unidad: string;
    unidades, precio, bruto, neto, baseIva, iva, total: real;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      result.netoAux:= qAux.FieldByName('importe_neto_f').AsFloat;
    ...
    qAux.SQL.Clear;
    qAux.SQL.Add(' select  sum(neto_fal) bruto, ');
    qAux.SQL.Add('         porc_descuento_fal porc_descuento, ');
    qAux.SQL.Add('         sum(descuento_fal) descuento, ');
    qAux.SQL.Add('         sum(importe_fal) importe_neto_f ');
    qAux.SQL.Add(' from frf_fac_abonos_l ');
    qAux.SQL.Add(' where empresa_fal = :empresa and fecha_fal= :fecha and factura_fal = :abono ');
    qAux.SQL.Add(' group by porc_descuento ');
    qAux.ParamByName('empresa').AsString:= AEmpresa;
    qAux.ParamByName('abono').AsInteger:= AAbono;
    qAux.ParamByName('fecha').AsDateTime:= AFecha;
    qAux.open;

    result.bruto:= qAux.FieldByName('bruto').AsFloat;
    result.porc_descuento:= qAux.FieldByName('porc_descuento').AsFloat;
    result.descuento:= qAux.FieldByName('descuento').AsFloat;
    rAux:= qAux.FieldByName('importe_neto_f').AsFloat;
    qAux.Close;

    if result.netoAux <>  rAux then
    begin
      raise Exception.Create('El sumatorio del neto de lass lineas del abono/factura manual (' + FloatToStr( rAux  ) + ') ' +
                             'no es el mismo que el importe grabado en la cabecera (' + FloatToStr( result.netoAux ) + ') ');
    end;
...
begin
  ...
  try
    ...
    begin
      ...
      result.fechaAlbaran:= qAux.FieldByName('fecha_sc').AsDateTime;
...
begin
  ...
  try
    ...
    qAux.SQL.Add(' vendedor_fel, empresa_fel, factura_fel, fecha_factura_fel ,num_linea_fel ,producto_fel , ');
    ...
    qAux.SQL.Add(' porc_descuen_fel ,descuentos_fel ,neto_fel) ');
    qAux.SQL.Add(' VALUES( ');
    qAux.SQL.Add(' :vendedor_fel, :empresa_fel, :factura_fel,:fecha_factura_fel ,:num_linea_fel ,:producto_fel , ');
    qAux.SQL.Add(' :descripcion_fel ,:cantidad_fel ,:medida_fel ,:unidad_medida_fel , ');
    qAux.SQL.Add(' :precio_bruto_fel ,:precio_neto_fel ,:porc_cargo_fel ,:cargos_fel , ');
    ...
    qAux.ParamByName('neto_fel').AsFloat := Abs( ALinAbono.bruto );
...
begin
  ...
  try
    ...
    qAux.ParamByName('base_iec').AsFloat := Abs( ACabAbono.NetoAux );
...
begin
  ...
  try
    ...
    qAux.SQL.Add('        unidades_fal, precio_fal, neto_fal, importe_fal ');
    qAux.SQL.Add(' from frf_fac_abonos_l ');
    qAux.SQL.Add(' where empresa_fal = :empresa ');
    qAux.SQL.Add(' and factura_fal = :factura ');
    qAux.SQL.Add(' and fecha_fal = :fecha ');
    ...
    begin
      (*PRODBASE*)
      rLinFacturaEdiVar.producto:= GetCodEdiComprador(ACabAbono.codEmpresa, ACabAbono.cliente,
        qAux.FieldByName('envase_fal').AsString, qAux.FieldByName('producto_fal').AsString,
        rLinFacturaEdiVar.dun14, rLinFacturaEdiVar.nombre );
(*
...
*)
      ...
      rLinFacturaEdiVar.bruto:= qAux.FieldByName('neto_fal').AsFloat;
      ...
      bruto:= bruto + rLinFacturaEdiVar.bruto;
      iva:= iva + rLinFacturaEdiVar.iva;
      total:= total + rLinFacturaEdiVar.total;

      if i = cont then
      begin
        rLinFacturaEdiVar.bruto:= rLinFacturaEdiVar.bruto + ( ACabAbono.bruto - bruto );
        rLinFacturaEdiVar.neto:= rLinFacturaEdiVar.neto + ( ACabAbono.netoAux - neto );
...
begin
  ...
  try
    ...
    qAux.SQL.Add('porc_cargos_fec, cargos_fec,  tipo_desc_fec, porc_descuen_fec,  descuentos_fec, ');
    qAux.SQL.Add('impuestos_fec,  total_factura_fec,  moneda_fec,  vencimiento1_fec, ');
    qAux.SQL.Add('importe_vto1_fec, fecha_albaran_fec )');
    qAux.SQL.Add('VALUES ');
    qAux.SQL.Add('(:empresa_fec, :factura_fec,  :vendedor_fec,  :cobrador_fec,  :comprador_fec, ');
    qAux.SQL.Add(':receptor_fec,  :cliente_fec,  :pagador_fec,  :pedido_fec, ');
    qAux.SQL.Add(':fecha_factura_fec,  :nodo_fec,  :rsocial_fec,  :domicilio_fec, ');
    ...
    qAux.ParamByName('fecha_albaran_fec').AsDateTime := ACabAbono.fechaAlbaran;

    qAux.ParamByName('bruto_fec').AsFloat := Abs( ACabAbono.bruto );
    qAux.ParamByName('porc_cargos_fec').AsFloat := 0.0;
    qAux.ParamByName('cargos_fec').AsFloat := 0.0;

    qAux.ParamByName('tipo_desc_fec').AsString:= 'TD';
    if ACabAbono.porc_descuento <> 0 then
    begin
      qAux.ParamByName('porc_descuen_fec').AsFloat := ACabAbono.porc_descuento;
      qAux.ParamByName('descuentos_fec').AsFloat := Abs( ACabAbono.descuento );
      //qAux.ParamByName('tipo_desc_fec').AsString := 'TD';
    end
    else
    begin
...
begin
  ...
  begin
    ...
    if rCabFacturaEdiVar.netoAux <> NetoAbonoLin( AEmpresa, AAbono, AFecha ) then
    begin
