...
uses CVariables, UDMAuxDB, bTimeUtils, CReportes, Principal, CAuxiliarDB, LFacturasCliente,
  DPreview, LFacturasCliDes, LFacturasCliSem, UDMBaseDatos, UDMCambioMoneda;
...
begin
  ...
  begin
    ...
    if EEmpresa.Text = 'BAG' then
      ...
      SQL.Add(' AND cod_empresa_fac_fc = ' + QuotedStr(EEmpresa.Text) );
    if ESerie.Text <> '' then
    ...
    SQL.Add('SELECT SUM(KILOS_SL) KILOS, CLIENTE_FAC_SC CLIENTE_S, ' +
      ...
      ' WHERE 1=1    ');
    if EEmpresa.Text = 'SAT' then
      SQL.Add('       and (EMPRESA_FAC_SC = ''050'' or EMPRESA_FAC_SC = ''080'' ) ')
    else
    if EEmpresa.Text = 'BAG' then
      SQL.Add('       and EMPRESA_FAC_SC[1,1] = ''F'' ')
    else
      SQL.Add(' AND EMPRESA_FAC_SC = ' + QuotedStr(EEmpresa.Text) );
    if ESerie.Text <> '' then
