...
type
  ...
  public
    ...
    dEmpresa, centro, dProducto, factura: string;
    codProducto, envase, lblEnvase: string;
...
begin
  ...
  dEmpresa := EEmpresa.Text + '  ' + STEmpresa.Caption;
  dProducto := EProducto.Text + '  ' + STProducto.Caption;
  ...
  try
    ...
    if envase <> '' then
    begin
      QRLVentasPorCliente.lblEnvase.Caption := desEnvaseP(EEmpresa.Text, envase, codProducto);
...
begin
  ...
  begin
    ...
    begin
      ...
      SQL.Add(' from frf_salidas_l s, frf_envases, frf_clientes ');
      SQL.Add(' where empresa_sl = :empresa ');
      SQL.Add(' AND producto_sl = ' + QuotedStr( codProducto  ));
...
begin
  ...
  begin
    ...
    SQL.Add('SELECT empresa_sc, cliente_sal_sc,moneda_sc,fecha_sc,n_albaran_sc,' +
      ...
      '      ROUND( SUM((case when producto_sl = ' + QuotedStr( codProducto ) + ' then importe_neto_sl else 0 end * ' +
    ...
    SQL.Add('       sum(case when producto_sl = ' + QuotedStr( codProducto ) + ' then kilos_sl else 0 end) kilos_producto, ');
    SQL.Add('       sum(case when ref_transitos_sl is not null then kilos_sl else 0 end) kilos_ttotal, ');
    SQL.Add('       sum(case when ( ref_transitos_sl is not null ) and ( producto_sl = ' + QuotedStr( codProducto ) + ' ) then kilos_sl else 0 end) kilos_tproducto, ');


    SQL.Add('      SUM( case when producto_sl = ' + QuotedStr( codProducto ) + ' then cajas_sl else 0 end )cajas, ' +
      '      SUM( case when producto_sl = ' + QuotedStr( codProducto ) + ' then cajas_sl else 0 end * unidades_caja_sl ) Unidades, centro_salida_sc ' +
    ...
    SQL.Add(' GROUP BY empresa_sc, cliente_sal_sc, centro_salida_sc, n_albaran_sc, fecha_sc,' +
      ' moneda_sc');
    SQL.Add(' HAVING ( sum(case when producto_sl = ' + QuotedStr( codProducto ) + ' then kilos_sl else 0 end) > 0 )');
    SQL.Add('        AND ( sum(case when producto_sl = ' + QuotedStr( codProducto ) + ' then importe_neto_sl else 0 end) > 0 )');
    ...
    SQL.Add('SELECT s.*, ' +
      ...
      ' and   (producto_g is null or  producto_g  = ' + QuotedStr( codProducto ) + ') ' +
