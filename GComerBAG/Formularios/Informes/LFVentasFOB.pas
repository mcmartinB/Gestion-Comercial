...
type
  ...
  private
    ...
    procedure RellenarTemporal(cliente: string; kil: real; ventas, gasto, total: real);
    procedure Imprimir;
    procedure miBorrarTemporal;
    ...
    procedure RellenarTemporalTotal(cliente: string; albaran: integer; fecha: TDate; kil: real; ventas, gasto, total: real);
...
begin
  ...
  TTemporal.IndexFieldNames := 'cliente_tvf';
  ...
  //si el listado es por albaranes se necesitan estos dos campos;
  if RGTipoListado.ItemIndex = 1 then
  begin
    campos := ' albaran_tvf INTEGER, ' +
      ' fecha_tvf DATE, ';
  end
  else
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        RellenarTemporal(cli, kilos, netos, gastos, importe);
        kilos := 0;
        netos := 0;
        gastos := 0;
        importe := 0;
...
procedure TFLVentasFOB.RellenarTemporal(cliente: string; kil: real; ventas, gasto, total: real);
begin
  ...
  try
    TTemporal.InsertRecord([cliente, kil, ventas, gasto, total]);
  except
    on E: EDBEngineError do
    begin
      ShowError('Error al insertar registro en la tabla temporal');
...
procedure TFLVentasFOB.miBorrarTemporal;
begin
  ...
  try
    ...
    ShowError('No se ha podido borrar la tabla temporal, pongase en contacto con el departamento de informática')
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      Result := Result + ( gastog_transitos * kilos_producto ) / kilos_transito;
    end;
  ...
  begin
    ...
    begin
      ...
      importe := (netos - gastos);
            ...
            //se rellena la tabla temporal
      RellenarTemporalTotal(cli, albaran, fecha, kilos, netos, gastos, importe);
      QLVentasFOB.Next;
    end;
  end;
end;

procedure TFLVentasFOB.RellenarTemporalTotal(cliente: string; albaran: integer; fecha: TDate; kil: real; ventas, gasto, total: real);
begin
  if not TTemporal.Active then
  begin
    try
...
begin
        //Llamamos al QReport
  TTemporal.IndexFieldNames := 'cliente_tvf;albaran_tvf;fecha_tvf';
