  ...
  case TComponent(Sender).Tag of
    ...
    begin
      ...
      if edtEnvase.Text = '' then
        ...
        stEnvase.Caption := desEnvaseP(Eempresa.Text, edtEnvase.Text, EProducto.Text);
    end;
...
procedure TFLVentas.edtEnvasePropertiesChange(Sender: TObject);
begin
  if edtEnvase.Text = '' then
    stEnvase.Caption:= 'TODOS LOS ARTICULO'
  else
    stEnvase.Caption := desEnvase(EEmpresa.Text, edtEnvase.Text);
end;

procedure TFLVentas.BorrarTemporal;
begin
  TTemporal.Close;
  TTemporal.TableName := nom_tabla;
  if TTemporal.Exists then
...
begin
  ...
  begin
    ...
    begin
      Clear;
      Add('SELECT n_albaran_sl, fecha_sl, envase_sl, producto_sl, categoria_sl, ');
      ...
      Add(' GROUP BY cliente_sal_sc, n_albaran_sl, fecha_sl, envase_sl, producto_sl, ');
      Add('          categoria_sl, n_factura_sc, fecha_factura_sc,moneda_sc');
