...
type
  TFLSelecFacturas = class(TForm)
    ...
    procedure informativoEnter(Sender: TObject);
    procedure informativoExit(Sender: TObject);
...
uses bDialogs, UDMBaseDatos, CVariables, Principal, CAuxiliarDB, LFacturas, LFacturasWEB,
  UDMFacturacion, CFacturacion, UDMCambioMoneda,  AdvertenciaFD,
  UDMAuxDB, bSQLUtils, bNumericUtils, bMath, UDMConfig, DConfigMail, FacturasNacionalRI;
  //UDMFacturacionWEB;
...
begin
  ...
  begin
    ...
    begin
      ...
      if iAux <> iYear then
      begin
        Result:= VerAdvertencia( Self, 'No se deben facturar albaranes del año ' + IntToStr( iAux) + ' con fecha del ' + IntToStr( iYear) + '.',
...
begin
  ...
  usuario := gsCodigo;
  UDMFacturacion.Definitiva := informativo.ItemIndex = 1;
...
(*  if clienteDesde.text = 'WEB' then
  begin
    UDMFacturacionWEB.FacturacionWEB( empresa.Text,  edtCentro.Text, ClienteDesde.Text,
      AlbaranDesde.Text, AlbaranHasta.Text, fechaFacturar.Text, fechaFactura.Text,
      Pedido.Text, informativo.ItemIndex = 0, printOriginal.Checked, printEmpresa.Checked, chkRiesgo.Checked );
  end
  else
*)
  begin
...
function CanFacturarFecha(ADate: string): Boolean;
var
...
begin
  ...
  if not CanFacturarFecha(fechaFactura.Text) then
  begin
    Limpiar;
    AcaboFacturar(usuario);
    Timer.Enabled := true;
  ...
  PreparaFacturacion(clienteDesde.Text = 'WEB', fechaFactura.text);
  ...
  if ( clienteDesde.Text = 'WEB' ) or ( clienteDesde.Text = 'CON' ) then
    PrevisualizarWeb
  else
    Previsualizar;
...
procedure TFLSelecFacturas.informativoEnter(Sender: TObject);
begin
  informativo.Font.Color := clNavy;
end;

procedure TFLSelecFacturas.informativoExit(Sender: TObject);
begin
  informativo.Font.Color := clBlack;
end;
...
begin
  ...
  *)
  ...
  if informativo.ItemIndex = 0 then
    bDefinitiva := false
  else
    bDefinitiva := true;
...
begin
  ...
  begin
    ...
    //ParamByName('clienteDesde').AsString := clienteDesde.Text;
    //ParamByName('ClienteHasta').AsString := clienteHasta.Text;
    ParamByName('ini').AsString := albaranDesde.Text;
    ParamByName('fin').AsString := albaranHasta.Text;
    ParamByName('fecha').AsDate := StrToDate(fechaFacturar.Text);
...
var
  slCliente, slPedido, slAlbaran: string;
  error: boolean;
      ...
      case FacturarPor(FieldByName('cod_empresa_tf').AsString, FieldByName('cod_cliente_fac_tf').AsString) of
        0:
          begin
            ...
            error := false;
            slcliente := FieldByName('cod_cliente_fac_tf').AsString;
            ...
            repeat
              ...
              begin
                if (moneda <> FieldByName('moneda_tf').AsString) and
                  (slcliente = FieldByName('cod_cliente_fac_tf').AsString) then
                begin
                  Error := true;
              ...
              Next;
            until (slcliente <> FieldByName('cod_cliente_fac_tf').AsString) or
        ...
        1:
          begin
            ...
            slcliente := FieldByName('cod_cliente_fac_tf').AsString;
            slpedido := FieldByName('pedido_tf').AsString;
            slalbaran := FieldByName('albaran_tf').AsString;
            repeat
              edit;
              FieldByName('factura_tf').AsInteger := contador;
              FieldByName('cod_emp_factura_tf').AsString := AEmpresa;
              ...
              Next;
            until (slcliente <> FieldByName('cod_cliente_fac_tf').AsString) or
              (slpedido <> FieldByName('pedido_tf').AsString) or
              (slalbaran <> FieldByName('albaran_tf').AsString) or EOF;
          end;
        2:
          begin
            ...
            repeat
              ...
              Next;
            until (slcliente <> FieldByName('cod_cliente_fac_tf').AsString) or
              (slpedido <> FieldByName('pedido_tf').AsString) or EOF;
          end;
      end;
      contador := contador + 1;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SQL.Add(' WHERE cod_serie_fs = :empresa ');
        SQL.Add(' and anyo_fs = :anyo ');
        ParamByName('empresa').AsString:= sSerie;
        ParamByName('anyo').AsInteger:= iYear;
        ExecSQL;
      end;
    end;
...
begin
  ...
  begin
    empresa.Text := '078';
    clienteDesde.Text := 'MER';
    //clienteHasta.Text := 'MER';
    albaranDesde.Text := '1';
...
begin
  ...
  begin
    ...
    DMAuxDB.QAux.SQL.Add(' where empresa_c ' + SQLEqualS(empresa));
...
begin
  ...
  if PaisCliente( Empresa.Text, clienteDesde.TExt ) = 'ES' then
  begin
    QRLFacturasNacional := TRIFacturasNacional.Create(Application);
    QRLFacturasNacional.LabelFecha.Caption := fechaFactura.Text;
    QRLFacturasNacional.definitivo := bDefinitiva;
    QRLFacturasNacional.sTextoInformativa:= cbbTextoInformativa.Text;

    QRLFacturasNacional.Configurar(Empresa.Text, clienteDesde.TExt);
    ...
    begin
      QRLFacturasNacional.Tag:= 0;
      Preview(QRLFacturasNacional, 1, False, False);
    end;
  end
  else
  begin
    ...
    QRLFacturas.sTextoInformativa:= cbbTextoInformativa.Text;

    QRLFacturas.Configurar(Empresa.Text, clienteDesde.TExt);
...
procedure TFLSelecFacturas.PrevisualizarWEB;
begin
     //Crear el listado
  try
    DMFacturacion.QCabeceraFactura.ParamByName('usuario').AsString := gsCodigo;
    ...
    (*
    DMFacturacion.DireccionFactura( DMFacturacion.QCabeceraFactura.FieldByName('cod_empresa_tfc').AsString,
                                    DMFacturacion.QCabeceraFactura.FieldByName('cod_client_sal_tfc').AsString,
                                    DMFacturacion.QLineaFactura.FieldByName('cod_dir_sum_tf').AsString,
  ...
  QRLFacturasWEB.LabelFecha.Caption := fechaFactura.Text;
  QRLFacturasWEB.definitivo := bDefinitiva;
  QRLFacturasWEB.sTextoInformativa:= cbbTextoInformativa.Text;
  ...
  begin
    QRLFacturasWEB.Tag:= 0;
    Preview(QRLFacturasWEB, 1, False, False);
  end;
  ...
  DMFacturacion.QDirFactura.Close;
  DMFacturacion.QLineaGastos.Close;
  DMFacturacion.QLineaFactura.Close;
  DMFacturacion.QCabeceraFactura.Close;
end;
...
procedure TFLSelecFacturas.edtCentroChange(Sender: TObject);
begin
    if (gRF <> nil) then
      if esVisible( gRF ) then
        Exit;
  ...
  if edtCentro.Text = '' then
    stCentro.Caption := 'VACIO TODOS LOS CENTROS'
     ...
     stCentro.Caption := desCentro( empresa.Text, edtCentro.Text );
end;

procedure TFLSelecFacturas.informativoClick(Sender: TObject);
begin
  chkAlbaranesValidados.Enabled:= informativo.ItemIndex = 0;
