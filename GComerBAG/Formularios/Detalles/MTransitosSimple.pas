...
begin
  ...
  begin
    raise Exception.Create('El formato de la hora de carga del tránsito no es correcto (hh:mm).');
  end;
...
begin
  if DInfTransitosPreguntar.Preguntar('¿DESEA IMPRIMIR EL CARTA DE PORTE DEL TRÁNSITO?') then
  begin
    ImprimirCartaPorte;
  end;
  if DInfTransitosPreguntar.Preguntar('¿DESEA IMPRIMIR EL CMR DEL TRÁNSITO?') then
  begin
...
(*
  if MessageDlg( 'Pone los palets asignados al transito en estado de stock, preparados para usar. ' + #13 + #10 +
              'Usar esta opción cuando se reciba fisicamente el tránsito. ' + #13 + #10 +
              '¿Desea activar el tránsito seleccionado? ', mtConfirmation, mbOKCancel, 0 ) = mrOk then
  begin
    ...
    if PDTransitoActivar.Proceso ( planta_destino_tc.Text, centro_destino_tc.Text, n_orden_tc.Text,
      ...
      ShowMessage('Tránsito sin palets asociados o ya activados.');
