...
uses
  ...
  StdCtrls, ExtCtrls, Buttons, CMaestroDetalle, ActnList, Grids, DBGrids, BGrid,
  BDEdit, BSpeedButton, Menus, ComCtrls, DPreview, BCalendario, BEdit,
  BCalendarButton, BGridButton, CVariables, DError, QRPrntr, DBTables, DBCtrls,
  ...
  dxSkinMoneyTwins, cxTextEdit, cxDBEdit, cxButtons, SimpleSearch, DBClient, dxSkinBlack, dxSkinCaramel, dxSkinCoffee,
  dxSkinDarkRoom, dxSkinDarkSide, dxSkinDevExpressDarkStyle, dxSkinDevExpressStyle, dxSkinGlassOceans,
  dxSkinHighContrast, dxSkiniMaginary, dxSkinLilian, dxSkinLiquidSky, dxSkinLondonLiquidSky, dxSkinMcSkin,
  dxSkinMetropolis, dxSkinMetropolisDark, dxSkinOffice2007Black, dxSkinOffice2007Blue, dxSkinOffice2007Green,
...
type
  ...
  TFMSalidas = class(TMaestroDetalle)
    ...
    procedure QSalidasLCalcFields(DataSet: TDataSet);
    procedure QSalidasLBeforeDelete(DataSet: TDataSet);
  ...
  private
    ...
    function GetNumeroDeAlbaran(const empresa, centro: string; const AIncrementa: boolean; const albaran: integer = -1): Integer;
    ...
    procedure Rejilla(boton: TBGridButton; VAlta: Boolean = FALSE);
...
uses
  Variants, bDialogs, LFacturaProforma, UFTransportistas, bTimeUtils,
  CGestionPrincipal, UDMBaseDatos, StrUtils, CAuxiliarDB, UDMAuxDB, Principal,
  DFacturaProforma, MGastosSalida, CFactura, bSQLUtils, DInfSalidasSelect,
  DInfSalidasPreguntar, DAsignarAlbFac, UFClientes, UFSuministros, CartaPorteDL,
  DConfigMail, UDMConfig, bMath, bTextUtils, UDMCmr, GetFechaDiarioEnvases,
  UDQAlbaranSalida, UDQAlbaranSalidaWEB, SignatureForm, AdvertenciaFD,
  AlbaranEntreCentrosMercadonaDM, AlbaranSalidaEnvasesDM, CSistema,
  DesadvAhorramasDM, DesadvCorteInglesDM, UDMMaster, SeleccionarComercialFD,
  UQRAlbaranAlemaniaNoVar, UQRAlbaranInglesVal, UQRAlbaranExportacionIngles, VerOrdenFD,
  ArticuloDesgloseSalMod;
...
begin
  CloseAuxQuerys;
  bnCloseQuerys([QSalidasC, TSalidasL]);
end;
...
begin
  ...
  begin
    ...
    begin
      SQL.Clear;
      SQl.Add(' select cod_serie_fac_fc serie_fal, n_factura_fc factura_fal,  fecha_factura_fc fecha_fal, ');
      SQl.Add('        case when not( automatica_fc = 0 and anulacion_fc = 0 ) then ''*'' ');
      SQl.Add('               else cod_producto_fd end  producto_fal, ');
      SQl.Add('        n_remesa_rf remesa, fecha_vencimiento_rc fecha_remesa, sum( importe_neto_fd ) importe ');
      ...
      SQl.Add(' group by serie_fal, factura_fal, fecha_fal, producto_fal, n_remesa_rf, fecha_vencimiento_rc ');
      SQl.Add(' order by serie_fal, fecha_fal, factura_fal desc, producto_fal ');
    end;
  end
  else
  begin
...
procedure TFMSalidas.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
begin
  ...
  else if facturable_sc.State = cbChecked then
  begin
    if Flag then
      where := where + ' and ';
    where := where + ' facturable_sc <> 0 ';
...
//  begin
    ...
    begin
      dir_sum_sc.setfocus;
      raise Exception.Create('Dirección de suministro incorrecta o de baja');
    end;
//  end;
  //Cliente facturacion
  ...
  else if (Trim(STOperador_transporte_sc.Caption) = '') then
  begin
    operador_transporte_sc.Text := transporte_sc.Text;
  end
  else if (Trim(STtransporte_sc.Caption) = '') then
  begin
...
begin
  ...
  try
    ...
    begin
      RellenaDatos(empresa_sc.Text, centro_salida_sc.Text, fecha_sc.Text, n_albaran_sc.Text, cliente_sal_sc.Text, dir_sum_sc.Text, AFacDespacho);
      ShowModal;
...
begin
  ...
  begin
    ...
    begin
      result := UDQAlbaranSalidaWEB.PreAlbaran(Self, empresa_sc.text, centro_salida_sc.Text, StrToInt(n_albaran_sc.Text), StrToDateTime(fecha_sc.Text), APedirFirma, AOriginal, self.rGGN);
    end
    else if empresa_sc.Text = '506' then
    begin
      result := AlbaranEntreCentrosMercadonaDM.PreAlbaran(Self, empresa_sc.text, centro_salida_sc.Text, StrToInt(n_albaran_sc.Text), StrToDateTime(fecha_sc.Text), APedirFirma, AOriginal,  self.rGGN);
       (*
    ...
    begin
      result := UDQAlbaranSalida.PreAlbaran(Self, empresa_sc.text, centro_salida_sc.Text, StrToInt(n_albaran_sc.Text), StrToDateTime(fecha_sc.Text), APedirFirma, AOriginal,  self.rGGN);
    end
  end
  else if AIdioma = 1 then
  begin
    result := UQRAlbaranAlemaniaNoVar.PreAlbaranAleman(empresa_sc.Text, centro_salida_sc.Text, cliente_sal_sc.Text, StrToInt(n_albaran_sc.text), StrToDate(fecha_sc.text), APedirFirma, AOriginal,  self.rGGN);
  end
  else if AIdioma = 2 then
  begin
      //Valorado
//    result := UQRAlbaranInglesVal.PreAlbaranIngles(empresa_sc.Text, centro_salida_sc.Text, cliente_sal_sc.Text, StrToInt(n_albaran_sc.text), StrToDate(fecha_sc.text), APedirFirma, AOriginal);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            AddMsg(AMsg, 'ERROR: No coincide los kilos para el artículo ' + FieldByname('envase_sl').AsString + ' Albaran: ' + FieldByname('kilos_sl').AsString + ' Esperado: ' + FieldByname('peso').AsString);
          end;
        end;
        //Unidades
        ...
        begin
          ...
          begin
            AddMsg(AMsg, 'ERROR: No coincide las unidades por caja para el artículo ' + FieldByname('envase_sl').AsString + ' Albaran: ' + FieldByname('unidades_caja_sl').AsString + ' Esperado: ' + FieldByname('unidades_e').AsString);
          end;
        end;
        Next;
      end;
...
begin
  ...
  begin
    iResult := iResult - 10;
    UDMCmr.ExecVentaCMR(empresa_sc.Text, centro_salida_sc.Text, cliente_sal_sc.Text, dir_sum_sc.Text, transporte_sc.Text, nota_sc.Text, StrToDate(fecha_sc.Text), StrToInt(n_albaran_sc.Text), QSalidasC.FieldByName('higiene_sc').AsInteger = 1);
  end;
...
begin
  ...
  begin
    DConfigMail.sAsunto := 'Envio Albarán [' + cliente_sal_sc.Text + '-' + n_albaran_sc.Text + ' || ' + desProductoAlta(producto_sl.Text) + ' || ' + trim(vehiculo_sc.Text) + ' || ' + desSuministroEx(empresa_sc.Text, cliente_sal_sc.Text, dir_sum_sc.Text);
...
begin
  ...
  if ActiveControl <> nil then
    case ActiveControl.Tag of
      ...
      kTipoSalida:
        DespliegaRejilla(btnTipoSalida);
      kProducto:
        Rejilla(BGBproducto_sl, TRUE);
      kMarca:
        DespliegaRejilla(BGBmarca_sl);
//    kEnvase: DespliegaRejilla(BGBenvase_sl, [empresa_sc.Text, producto_sl.Text, cliente_sal_sc.Text]);
      kColor:
        DespliegaRejilla(BGBcolor_sl, [empresa_sc.Text, producto_sl.Text]);
      kCategoria:
        DespliegaRejilla(BGBcategoria_sl, [empresa_sc.Text, producto_sl.Text]);
      kCalibre:
        DespliegaRejilla(BGBcalibre_sl, [empresa_sc.Text, producto_sl.Text]);
      ktipoPalet:
        DespliegaRejilla(BGBTipo_palets_sl);
      kCalendar:
...
begin
  ...
  begin
    ...
    Open;
    if IsEmpty then
      raise Exception.Create('ERROR: No se puede crear la linea de albarán. Falta indicar el % de desglose por el producto');
...
begin
  ...
  else if estoyFacturada then
  begin
//    ShowMessage('Albarán asociado a una factura, recuerde repetirla si modifica el albarán');
    ShowMessage('Albarán asociado a una factura, solo se permitira modificar un número limitado de campos.');   // Pedido por Irene Segur 28/11/2018
    ModificarContabilizada;
  end
  else
  begin
...
begin
  ...
  if oldEnvase <> envase_sl.Text then
    ...
    begin
      ...
      SQl.Add(' and producto_e = ' + QuotedStr(producto_sl.Text));
      SQl.Add(' and fecha_baja_e is  null ');
      Open;
      if IsEmpty then
      begin
        Close;
        raise Exception.Create('No se pueden usar los artículos que han sido dados de baja.');
      end;
      Close;
    end;
  ...
  if (empresa_sc.Text = 'F42') and (producto_sl.Text = 'PLA') then
  begin
    if anyo_semana_entrega_sl.Text = '' then
    begin
      PonFocoDetalle(entrega_sl);
      if MessageDlg('Falta o es incorrecta la entrega de la que proviene la fruta.' + #13 + #10 + '¿Seguro que desea continuar?', mtWarning, [mbNo, mbYes], 0) = mrNo then
        Abort;
    end;
  end;
  ...
  begin
    ...
    if MessageDlg('El artículo seleccionado es de unidades variable y la unidades por cajas son 0, ¿es correcto?.', mtWarning, [mbYes, mbNo], 0) = mrNO then
    begin
  ...
  begin
    raise Exception.Create('Falta el artículo o es incorrecto.');
  end;
...
begin
  ...
  begin
    ...
    TSalidasL.FieldByName('empresa_sl').AsString := empresa_sc.Text;
          //centro
    if Trim(centro_salida_sc.Text) = '' then
      raise Exception.Create('Faltan datos de la cabecera.');
    TSalidasL.FieldByName('centro_salida_sl').AsString := centro_salida_sc.Text;
          //numero de albaran
    if Trim(n_albaran_sc.Text) = '' then
      raise Exception.Create('Faltan datos de la cabecera.');
    TSalidasL.FieldByName('n_albaran_sl').AsString := n_albaran_sc.Text;
          //fecha
    if Trim(fecha_sc.Text) = '' then
      raise Exception.Create('Faltan datos de la cabecera.');
    TSalidasL.FieldByName('fecha_sl').AsDateTime := StrToDate(fecha_sc.Text);
...
begin
    ...
    case value of
      kEmpresa:
        begin
          ...
          DMBaseDatos.QDespegables.Sql.Add(' SELECT DISTINCT empresa_e, nombre_e ' + ' FROM frf_empresas ' + ' ORDER BY nombre_e ');
        end;
      kCentro:
        begin
                //Depende de empresa
      ...
      kMoneda:
        begin
          ...
          RejillaFlotante.ColumnFind := 1;
          DMBaseDatos.QDespegables.Sql.Clear;
          DMBaseDatos.QDespegables.Sql.Add(' SELECT DISTINCT moneda_m, descripcion_m ' + ' FROM frf_monedas ' + ' ORDER BY descripcion_m ');
        end;
      kEnvase:
        begin
          RejillaFlotante.ColumnResult := 0;
          RejillaFlotante.ColumnFind := 1;
          DMBaseDatos.QDespegables.Sql.Clear;
          DMBaseDatos.QDespegables.Sql.Add(' SELECT DISTINCT envase_e, descripcion_e ' + ' FROM frf_envases Frf_envases ' + ' ORDER BY descripcion_e ');
        end;
      kMarca:
        begin
          RejillaFlotante.ColumnResult := 0;
          RejillaFlotante.ColumnFind := 1;
          DMBaseDatos.QDespegables.Sql.Clear;
          DMBaseDatos.QDespegables.Sql.Add(' SELECT DISTINCT codigo_m, descripcion_m ' + ' FROM frf_marcas Frf_marcas ' + ' ORDER BY descripcion_m ');
        end;
      kCalibre:
        begin
          RejillaFlotante.ColumnResult := 2;
      ...
      kTipoPalet:
        begin
          ...
          RejillaFlotante.ColumnFind := 1;
          DMBaseDatos.QDespegables.SQL.Clear;
          DMBaseDatos.QDespegables.SQL.Add(' SELECT codigo_tp, descripcion_tp' + ' FROM frf_tipo_palets ' + ' ORDER BY codigo_tp');
        end;
    end;
  ...
  begin
    if printError then
      ShowError('No se han encontrado datos.');
    Consulta := False;
  end;
end;
...
begin
  ...
  if esVisible(gRF) or esVisible(gCF) then
    Exit;
  ...
  begin
    case TComponent(Sender).Tag of
      kProducto:
        STProducto_sl.Caption := desProductoAlta(producto_sl.Text);
      kMarca:
...
procedure TFMSalidas.RequiredTime(Sender: TObject; var isTime: Boolean);
begin
  isTime := false;
...
begin
  if Trim(empresa_sc.Text) <> '' then
    ConsultaCentro := ' SELECT DISTINCT empresa_c, centro_c, descripcion_c ' + ' FROM frf_centros  ' + ' WHERE empresa_c= ' + QuotedStr(empresa_sc.Text) + ' ' + ' ORDER BY descripcion_c'
  else
    ConsultaCentro := ' SELECT DISTINCT empresa_c, centro_c, descripcion_c ' + ' FROM frf_centros  ' + ' ORDER BY descripcion_c';
end;

function TFMSalidas.ConsultaProducto: string;
begin
  ConsultaProducto := ' SELECT DISTINCT producto_p, descripcion_p ' + ' FROM frf_productos Frf_productos ' + ' WHERE tipo_venta_p = ''S'' ' + ' ORDER BY descripcion_p ';
end;
...
begin
  if Trim(empresa_sc.Text) <> '' then
    ...
    ConsultaCliente := ' SELECT DISTINCT cliente_c, nombre_c ' + ' FROM frf_clientes ' + ' ORDER BY nombre_c ';
end;
...
var
  ...
  aux: string;
begin
  aux := ' SELECT DISTINCT cliente_ds,dir_sum_ds, nombre_ds ' + ' FROM frf_dir_sum ' +
         ' WHERE fecha_baja_ds is null ';
  flag := false;

  if (Trim(cliente_sal_sc.Text) <> '') then
  begin
    aux := aux + ' AND   (cliente_ds = ' + QuotedStr(cliente_sal_sc.Text) + ') ';
  end;

  ConsultaSuministro := aux + ' ORDER BY nombre_ds ';
end;

function TFMSalidas.ConsultaTransportista: string;
begin
  ConsultaTransportista := ' SELECT transporte_t, descripcion_t ' + ' FROM frf_transportistas ' + ' ORDER BY descripcion_t ';
end;
...
var
  aux: string;
begin
  aux := ' SELECT DISTINCT producto_c, calibre_c ' + ' FROM frf_calibres ';

  if (Trim(producto_sl.Text) <> '') then
  begin
    aux := aux + ' WHERE  (producto_c = ' + QuotedStr(producto_sl.Text) + ') ';
  end;

  ConsultaCalibre := aux + ' ORDER BY calibre_c ';
end;
...
var
  ...
  aux: string;
begin
  aux := ' SELECT DISTINCT producto_c, categoria_c, descripcion_c ' + ' FROM frf_categorias ';
  ...
  ConsultaCategoria := aux + ' ORDER BY descripcion_c ';
end;
...
var
  aux: string;
begin
  aux := ' SELECT DISTINCT producto_c, color_c, descripcion_c ' + ' FROM frf_colores ';
  ...
  ConsultaColor := aux + ' ORDER BY descripcion_c ';
end;
...
function TFMSalidas.GetNumeroDeAlbaran(const empresa, centro: string; const AIncrementa: boolean; const albaran: Integer = -1): Integer;
begin
  if albaran <> -1 then
  begin
    if albaran <> albaranObtenido then
    begin
      result := albaran;
      Exit;
    end;
  ...
  begin
    Close;
    SQl.Clear;
    SQL.Add(' SELECT cont_albaranes_c FROM frf_centros ' + ' WHERE empresa_c=' + quotedstr(empresa_sc.Text) + ' AND centro_c=' + quotedstr(centro_salida_sc.Text));
    try
      Open;
    except
    ...
    begin
      SQL.Clear;
      SQL.Add(' UPDATE frf_centros ' + ' SET cont_albaranes_c =' + IntToStr(result + 1) + ' WHERE empresa_c=' + quotedstr(empresa_sc.Text) + ' AND centro_c=' + quotedstr(centro_salida_sc.Text));
      try
        ExecSQL;
      except
...
begin
  if (Trim(empresa_sc.Text) <> '') and (Trim(centro_salida_sc.Text) <> '') and (Estado = teAlta) and (DSMaestro.State <> dsBrowse) then
  begin
...
begin
     //Rellenamos cliente de facturacion
  if (Length(Trim(cliente_fac_sc.Text)) > 1) and (Length(Trim(empresa_sc.Text)) = 3) and
    //(Trim(moneda_sc.Text) = '') and
    (estado <> teLocalizar) and (DSMaestro.State <> dsBrowse) then
  begin
    with DMBaseDatos.QGeneral do
    begin
      if Active then
      ...
      SQL.Clear;
      SQL.Add(' SELECT moneda_c FROM frf_clientes ' + ' WHERE cliente_c=' + quotedstr(cliente_fac_sc.Text));
      try
        Open;
      except
        Exit;
...
begin
  ...
  begin
    SQL.Text := ' select expediciones_c from frf_clientes ' + ' where cliente_c=' + QuotedStr(cliente);
    try
      try
        Open;
        if IsEmpty then
    ...
    SQL.Text := ' update frf_clientes set expediciones_c = ' + IntToStr(aux + 1) + ' where cliente_c=' + QuotedStr(cliente);
    ExecSQL;
    ;
  end;
end;
...
begin
  with DMAuxDB.QDescripciones do
  begin
    SQL.Text := ' select count(*) from frf_dir_sum ' + ' where cliente_ds=' + QuotedStr(cliente_sal_sc.Text) + ' and dir_sum_ds=' + QuotedStr(dir_sum_sc.Text) +
  ...
  *)
  ...
  if not Result then
    ...
    begin
      ...
      if not IsEmpty then
      begin
        ShowError('No se puede borrar una salida asociada con un abono -> ' + fieldByName('empresa_fal').AsString + '-' + fieldByName('fecha_fal').AsString + '-' + fieldByName('factura_fal').AsString);
        Result := True;
      end;
      Close;
    end;
...
begin
  ...
  begin
    ...
    if VerAdvertencia(Self, #13 + #10 + ' ¿Esta usted seguro de querer borrar el albarán de venta completo con su detalle asociado?', '    BORRAR SALIDA', 'Quiero borrar el albarán completo', 'Borrar Albarán') = mrIgnore then
      botonPulsado := mrYes;
...
begin
  ...
  begin
    ...
    begin
      ...
      SQL.Add('WHERE empresa_g=' + quotedstr(empresa_sc.Text) + '  and centro_salida_g=' + quotedstr(centro_salida_sc.Text) + '  and n_albaran_g=' + n_albaran_sc.Text + '  and fecha_g=:fecha ');
    ...
    SQL.Add('WHERE empresa_sl=' + quotedstr(empresa_sc.Text) + '  and centro_salida_sl=' + quotedstr(centro_salida_sc.Text) + '  and n_albaran_sl=' + n_albaran_sc.Text + '  and fecha_sl=:fecha ');
    ParamByName('fecha').asdatetime := StrToDate(fecha_sc.Text);
    try
      EjecutarConsulta(DMBaseDatos.QGeneral);
    except
    ...
    SQL.Clear;
    SQL.Add('DELETE FROM frf_salidas_l2 ');
    SQL.Add('WHERE empresa_sl2=' + quotedstr(empresa_sc.Text) + '  and centro_salida_sl2=' + quotedstr(centro_salida_sc.Text) + '  and n_albaran_sl2=' + n_albaran_sc.Text + '  and fecha_sl2=:fecha ');
    ParamByName('fecha').asdatetime := StrToDate(fecha_sc.Text);
    try
      EjecutarConsulta(DMBaseDatos.QGeneral);
    except
...
procedure TFMSalidas.Rejilla(boton: TBGridButton; VAlta: Boolean = FALSE);
begin
...
begin
  ...
  if DMConfig.EsLaFont or (cliente_sal_sc.Text = 'WEB') then
    ...
    //Sql.Add(' select sum(importe_g) as gastos '+
        ' from frf_gastos, frf_tipo_gastos ' + ' where empresa_g=:empresa ' + ' and centro_salida_g=:centro ' + ' and n_albaran_g=:referencia ' + ' and fecha_g=:fecha ' + ' and tipo_tg = tipo_g');
      ParamByName('empresa').AsString := empresa_sc.Text;
      ParamByName('centro').AsString := centro_salida_sc.Text;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if (cliente_sal_sc.Text = 'SEM') or (cliente_sal_sc.Text = 'ALP') or (cliente_sal_sc.Text = 'AMA') or (cliente_sal_sc.Text = 'UND') or (cliente_sal_sc.Text = 'DIA') or (cliente_sal_sc.Text = 'IND') or (cliente_sal_sc.Text = 'VEG') or (cliente_sal_sc.Text = 'PRX ') then
        begin
          nota_sc.Lines.Add('TEMPERATURA ENTRE 1º y 8º C');
          nota_sc.Lines.Add('TEMPERATURA SALIDA PRODUCTO INFERIOR A +4º C');
        end
        else if (cliente_sal_sc.Text = 'MON') or (cliente_sal_sc.Text = 'ECI') or (cliente_sal_sc.Text = 'IDR') or (cliente_sal_sc.Text = 'LID') or (cliente_sal_sc.Text = 'RUN') then
        begin
          nota_sc.Lines.Add('TEMPERATURA ENTRE 1º y 6º C');
          nota_sc.Lines.Add('TEMPERATURA SALIDA PRODUCTO INFERIOR A +4º C');
        end
        else if (cliente_sal_sc.Text = 'MER') then
        begin
    ...
    begin
      ...
      begin
        if (centro_salida_sc.Text = '4') and (cliente_sal_sc.Text = 'MER') and (dir_sum_sc.Text = '927') then
        begin
      ...
      else if (empresa_sc.Text = 'F17') and (centro_salida_sc.Text = '2') then
      begin
...
begin
  ...
  begin
    ...
    begin
               //COMPROBAR CLAVE AJENA
      STDir_sum_sc.Caption := desSuministro(empresa_sc.text, cliente_sal_sc.Text, dir_sum_sc.Text);
    end;
  end
  else
  begin
...
begin
  with DMAuxDB.QAux do
  begin
    SQL.Text := ' select pais_c from frf_clientes ' + ' where cliente_c=' + QuotedStr(codCliente);
    try
      try
        Open;
        if IsEmpty then
          ...
          esClienteExtranjero := (Fields[0].AsString <> 'ES') and (Fields[0].AsString <> '');
      except
        esClienteExtranjero := false;
      end;
    finally
...
begin
  ...
  begin
    ...
    unidad_precio_sl.Text := unidadFacturacion(empresa_sc.text, cliente_sal_sc.text, producto_sl.Text, envase_sl.Text);
    sUnidadPrecioLinea := Copy(unidad_precio_sl.Text, 1, 1);

    tipo_iva_sl.Text := Impuesto.sCodigo;
    case TipoIVAEnvase(empresa_sc.Text, envase_sl.Text, producto_sl.Text) of
...
// Elimina de la tabla Temporal...
    ...
    SQL.Clear;
    SQL.Add('INSERT INTO tmp_gastos (empresa_tg,centro_tg,albaran_tg,' + 'fecha_tg,tipo_tg,importe_tg,usuario_tg,ref_fac_tg, fecha_fac_tg, producto_tg, solo_lectura_tg, transporte_tg) ');
    SQL.Add('SELECT empresa_g,centro_salida_g,n_albaran_g,' + 'fecha_g,tipo_g,' + 'importe_g,' + QuotedStr(gsCodigo) + ' As usuario ');
  ...
  try
    ...
    begin
      ...
      SQL.Add('WHERE empresa_tg =' + QuotedStr(empresa_sc.Text) + ' ' + 'And centro_tg =' + QuotedStr(centro_salida_sc.Text) + ' ' + 'And albaran_tg =' + n_albaran_sc.Text + ' ' + 'And fecha_tg = :fecha ' + 'And usuario_tg = ' + QuotedStr(gsCodigo) + ' ');
      ...
      while not Eof do
      begin
        DMAuxDB.QDescripciones.SQL.Text := ' select descripcion_tg, facturable_tg from frf_tipo_gastos ' + ' where tipo_tg=' + QuotedStr(FieldByName('tipo_tg').AsString);
        try
          DMAuxDB.QDescripciones.Open;
          if not DMAuxDB.QDescripciones.IsEmpty then
          begin
            Edit;
            FieldByName('descriptipo_tg').AsString := DMAuxDB.QDescripciones.FieldByName('descripcion_tg').AsString;
            FieldByName('facturable_tg').AsString := DMAuxDB.QDescripciones.FieldByName('facturable_tg').AsString;
            Post;
          end;
        finally
...
begin
  ...
  begin
    ...
    begin
      ShowMessage('Albarán asociado a una factura. Recuerde que sólo debe modificar gastos no facturables.');
      GastosClick(False);
    end
    else
    begin
...
//var
  ...
  //cBack: TCursor;
begin
  if Application.MessageBox('¿Seguro que quiere romper el enlace entre el albarán actual y su factura asociada.?' + #13 + #10 + 'Recuerde repetir la factura para que el cambio se vea reflejado.', 'Desasociar factura.', MB_YESNO) = IDNO then
    Exit;
...
//    ShowMessage('Albarán asociado a una factura, recuerde repetirla si modifica el albarán');
  ...
  begin
    ...
    etiquetaUnidad.Caption := RVisualizacion.DataSource.DataSet.FieldByName('unidad_precio_sl').AsString;
  ...
  begin
    ...
    TextoPrecio.Caption := IntToStr(RVisualizacion.SelectedRows.Count) + ' registros selccionados.';
    PanelPrecio.Visible := true;
    PanelPrecio.Enabled := true;
    RVisualizacion.Enabled := false;
    Precio.SetFocus;
...
procedure TFMSalidas.precioKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = vk_F1 then
  begin
    SpeedButton2.Click;
...
procedure TFMSalidas.precioKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
begin
  ...
  begin
    ...
    begin
      if (TSalidasL.FieldByName('tipo_iva_sl').AsString <> Impuesto.sCodigo) or (QSalidasC.FieldByName('cliente_sal_sc').AsString <> TSalidasL.FieldByName('cliente_sl').AsString) then
      begin
        TSalidasL.Edit;
        ...
        begin
          ...
          case TipoIVAEnvase(TSalidasL.FieldByName('empresa_sl').AsString, TSalidasL.FieldByName('envase_sl').AsString, TSalidasL.FieldByName('producto_sl').AsString) of
//         0:
//            TSalidasL.FieldByName('porc_iva_sl').AsFloat := Impuesto.rSuper;
            1:
              TSalidasL.FieldByName('porc_iva_sl').AsFloat := Impuesto.rReducido;
            2:
              TSalidasL.FieldByName('porc_iva_sl').AsFloat := Impuesto.rGeneral;
            else
              TSalidasL.FieldByName('porc_iva_sl').AsFloat := Impuesto.rSuper;
          end;
...
begin
  if not DMConfig.EsLaFont then
  begin
    if (oldMatricula <> QSalidasC.FieldByName('vehiculo_sc').AsString) and (oldmatricula <> 'IGNORAR') then
    begin
...
begin
  ...
  begin
    ...
    begin
      Result := EstaContabilizada(empresa_sc.Text, DSMaestro.DataSet.FieldByName('serie_fac_sc').AsString, n_factura_sc.Text, fecha_factura_sc.Text);
    end;
  end;
end;

function TFMSalidas.unidadFacturacion(const empresa, cliente, producto, envase: string): string;
var
...
procedure TFMSalidas.QSalidasLBeforeDelete(DataSet: TDataSet);
begin
...
procedure TFMSalidas.QSalidasLCalcFields(DataSet: TDataSet);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      else if FieldByName('unidad_precio_sl').AsString = 'CAJ' then
      begin
...
begin
  ...
  btnDesadv.Enabled := (QSalidasC.FieldByName('cliente_sal_sc').AsString = 'ECI') or (QSalidasC.FieldByName('cliente_sal_sc').AsString = 'AMA') or (QSalidasC.FieldByName('cliente_sal_sc').AsString = 'MER');
end;
...
begin
  ...
  begin
    ...
    begin
      ShowMessage('Albarán asociado a una factura, no se pueden modificar los precios.');
//    ValorarAlbaran;
    end
    else
    begin
    ...
    begin
      ...
      try
        ...
        begin
          ...
          iUnidadesEnvase := unidadesEnvaseEx(DMAuxDB.QGeneral.FieldByName('empresa_sl').AsString, DMAuxDB.QGeneral.FieldByName('envase_sl').AsString, DMAuxDB.QGeneral.FieldByName('producto_sl').AsString);
          if sUnidad = '' then
          begin
            sAux := sAux + #13 + #10 + 'ERROR Falta grabar precios para -> ' + DMAuxDB.QGeneral.FieldByName('cliente_sal_sc').AsString + '-' + DMAuxDB.QGeneral.FieldByName('envase_sl').AsString + '-' + DMAuxDB.QGeneral.FieldByName('producto_sl').AsString + '-' + DateToStr(dFecha);
          end
//          else if DMAuxDB.QGeneral.FieldByName('unidad_precio_sl').AsString <> sUnidad then
//          begin
//            sAux := sAux + #13 + #10 + 'ERROR No coincide la unidad de facturacion ' + sUnidad + ' -> ' + DMAuxDB.QGeneral.FieldByName('empresa_sl').AsString + '-' + DMAuxDB.QGeneral.FieldByName('envase_sl').AsString + '-' + DMAuxDB.QGeneral.FieldByName('producto_sl').AsString + '-' + DMAuxDB.QGeneral.FieldByName('unidad_precio_sl').AsString;
//          end
          else if (sUnidad = 'UND') and (iUnidadesEnvase <> DMAuxDB.QGeneral.FieldByName('unidades_caja_sl').AsInteger) then
          begin
            sAux := sAux + #13 + #10 + 'ERROR No coincide las unidades por envase ' + DMAuxDB.QGeneral.FieldByName('unidades_caja_sl').AsString + ' -> ' + DMAuxDB.QGeneral.FieldByName('empresa_sl').AsString + '-' + DMAuxDB.QGeneral.FieldByName('envase_sl').AsString + '-' + DMAuxDB.QGeneral.FieldByName('producto_sl').AsString + '-' + IntToStr(iUnidadesEnvase);
          end
          else
          begin
            if rPrecio <> 0 then
            begin
              ...
              if sUnidad = 'UND' then
              begin
                rNeto := bRoundTo(rPrecio * (DMAuxDB.QGeneral.FieldByName('cajas_sl').AsFloat * UnidadesEnvase(DMAuxDB.QGeneral.FieldByName('empresa_sl').AsString, DMAuxDB.QGeneral.FieldByName('envase_sl').AsString, DMAuxDB.QGeneral.FieldByName('producto_sl').AsString)), 2);
              end
              else if sUnidad = 'CAJ' then
              begin
            ...
            begin
              sAux := sAux + #13 + #10 + 'ERROR Falta grabar precios para -> ' + DMAuxDB.QGeneral.FieldByName('empresa_sl').AsString + '-' + DMAuxDB.QGeneral.FieldByName('centro_salida_sl').AsString + '-' + DMAuxDB.QGeneral.FieldByName('envase_sl').AsString + '-' + DMAuxDB.QGeneral.FieldByName('producto_sl').AsString + '-' + DateToStr(dFecha);
            end;
          end;
      ...
      if sAux <> '' then
      begin
        sAux := 'Proceso finalizado con errores.' + #13 + #10 + sAux + #13 + #10 + #13 + #10 + 'Valoradas ' + IntToStr(iCount) + ' lineas de ' + IntToStr(iTotal) + '.';
...
begin
  ...
  begin
    ...
    try
      ...
      while not Eof do
      begin
        if sSuministro <> FieldByName('empresa_sc').AsString + FieldByName('centro_salida_sc').AsString + FieldByName('cliente_sal_sc').AsString + FieldByName('dir_sum_sc').AsString then
        begin
          sSuministro := FieldByName('empresa_sc').AsString + FieldByName('centro_salida_sc').AsString + FieldByName('cliente_sal_sc').AsString + FieldByName('dir_sum_sc').AsString;
          sImpuestoSuministro := ImpuestosEntre(FieldByName('empresa_sc').AsString, FieldByName('centro_salida_sc').AsString, FieldByName('cliente_sal_sc').AsString, FieldByName('dir_sum_sc').AsString);
        end;
        if sEnvase <> FieldByName('empresa_sc').AsString + FieldByName('envase_sl').AsString + FieldByName('producto_sl').AsString then
        begin
          sEnvase := FieldByName('empresa_sc').AsString + FieldByName('envase_sl').AsString + FieldByName('producto_sl').AsString;
          iIvaEnvase := TipoIvaEnvase(FieldByName('empresa_sc').AsString, FieldByName('envase_sl').AsString, FieldByName('producto_sl').AsString);
          if not ExisteEnvase(FieldByName('empresa_sc').AsString, FieldByName('envase_sl').AsString, FieldByName('producto_sl').AsString) then
          begin
            raise Exception.Create('ERROR ->  NO existe el artículo ' + FieldByName('envase_sl').AsString);
          end;
        end;
        if sEnvaseCliente <> FieldByName('empresa_sc').AsString + FieldByName('cliente_sal_sc').AsString + FieldByName('envase_sl').AsString + FieldByName('producto_sl').AsString then
        begin
          sEnvaseCliente := FieldByName('empresa_sc').AsString + FieldByName('cliente_sal_sc').AsString + FieldByName('envase_sl').AsString + FieldByName('producto_sl').AsString;
          sUnidadFactura := unidadFacturacion(FieldByName('empresa_sc').AsString, FieldByName('cliente_sal_sc').AsString, FieldByName('producto_sl').AsString, FieldByName('envase_sl').AsString);
          iUnidadesEnvase := unidadesEnvaseEx(FieldByName('empresa_sc').AsString, FieldByName('envase_sl').AsString, FieldByName('producto_sl').AsString);
        end;
        ...
        sAlbaran := FieldByName('empresa_sc').AsString + ' ' + FieldByName('centro_salida_sc').AsString + ' ' + FieldByName('n_albaran_sc').AsString + ' ' + FieldByName('fecha_sc').AsString;

        //Validar impuesto
        case iIvaEnvase of
          0:
            if FieldByName('porc_iva_sl').AsFloat <> rSuper then
            begin
              Result := False;
              sErrores := sErrores + #13 + #10 + '* Para el artículo ' + FieldByName('envase_sl').AsString + ' se esperaba un IVA de ' + FormatFloat('#,##0.00', rSuper) + ' y se ha encontrado ' + FormatFloat('#,##0.00', FieldByName('porc_iva_sl').AsFloat);
            end;
          1:
            if FieldByName('porc_iva_sl').AsFloat <> rReducido then
            begin
              Result := False;
              sErrores := sErrores + #13 + #10 + '* Para el artículo ' + FieldByName('envase_sl').AsString + ' se esperaba un IVA de ' + FormatFloat('#,##0.00', rReducido) + ' y se ha encontrado ' + FormatFloat('#,##0.00', FieldByName('porc_iva_sl').AsFloat);
            end;
          2:
            if FieldByName('porc_iva_sl').AsFloat <> rGeneral then
            begin
              Result := False;
              sErrores := sErrores + #13 + #10 + '* Para el artículo ' + FieldByName('envase_sl').AsString + ' se esperaba un IVA de ' + FormatFloat('#,##0.00', rGeneral) + ' y se ha encontrado ' + FormatFloat('#,##0.00', FieldByName('porc_iva_sl').AsFloat);
            end;
        end;
        ...
        begin
          Result := False;
          sErrores := sErrores + #13 + #10 + '* Para el artículo ' + FieldByName('envase_sl').AsString + ' se esperaba facturar por ' + sUnidadFactura + ' y se ha encontrado ' + FieldByName('unidad_precio_sl').AsString;
        end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Result := False;
              sErrores := sErrores + #13 + #10 + '* Para el artículo ' + FieldByName('envase_sl').AsString + ' se esperaba ' + IntToStr(iUnidadesEnvase) + ' unidades caja y se ha encontrado ' + FieldByName('unidades_caja_sl').AsString;
            end
            else
            begin
        ...
        begin
          Result := False;
          sErrores := sErrores + #13 + #10 + '* Falta precio para el artículo ' + FieldByName('envase_sl').AsString;
        end;
  ...
  if bActualizarUnidades then
    ActualizarUnidades(QSalidasC.fieldByName('empresa_sc').AsString, QSalidasC.fieldByName('centro_salida_sc').AsString, QSalidasC.fieldByName('fecha_sc').AsDateTime, QSalidasC.fieldByName('n_albaran_sc').AsInteger);
end;
...
begin
  ...
  begin
    if (DSMaestro.DataSet.fieldByname('cliente_sal_sc').AsString = 'ECI') or (DSMaestro.DataSet.fieldByname('cliente_sal_sc').AsString = 'AMA') or
       (DSMaestro.DataSet.fieldByname('cliente_sal_sc').AsString = 'MER') then
    begin
      VerOrdenFD.MakeDsadv(DSMaestro.DataSet.fieldByname('empresa_sc').AsString, DSMaestro.DataSet.fieldByname('centro_salida_sc').AsString, DSMaestro.DataSet.fieldByname('n_albaran_sc').AsInteger, DSMaestro.DataSet.fieldByname('fecha_sc').AsDateTime);
    end
    else
    begin
...
begin
  ...
  begin
    ...
    else if Length(sEntrega) <> 12 then
    begin
...
begin
  ...
  begin
    with QSalidasC do
    begin
      Result := FormatFloat('#,##0.00', DMMaster.GetImpFacAlbaran(FieldByName('empresa_sc').AsString, FieldByName('centro_salida_sc').AsString, FieldByName('fecha_sc').AsDateTime, FieldByName('n_albaran_sc').AsInteger));
    end;
  end;
end;
