...
type
  TFMRemesas = class(TMaestroDetalle)
    ...
    lblPlanta: TLabel;
    planta_fr: TBDEdit;
    QDetalleplanta_fr: TStringField;
    lbl1: TLabel;
    notas_r: TDBMemo;
  ...
  private
    ...
    function  ImportePendiente( const AEmpresa_, APlanta: string; const AFactura: Integer;
                                const AFecha: TDate; const AModificar: boolean;
                                var ATotalFactura: real ): real;
    function  FechaFactura( const AEmpresa_, APlanta: string; const AFactura, ARemesa: Integer;
                            const AFechaRemesa: TDate; var VFecha: TDate ): boolean;
...
uses CVariables, CGestionPrincipal, UDMBaseDatos, bSQLUtils, bNumericUtils,
     ...
     UDMCambioMoneda, CMaestro, bMath, VAriants, UDMConfig, SelectRemesaFD;
...
{*RELLENAR, CONSTANTES EN CVariables}
  ...
  planta_fr.Tag := kEmpresa; //Para mostrar rejilla flotante
...
{*}
  ...
  FocoDetalle := planta_fr;

     //Inicializar variables
  CalendarioFlotante.Date := Date;
  Rvisualizacion.Height := 240;
  ...
  begin
    ...
    SQL.Add(' WHERE empresa_fr = :empresa_r ');
    SQL.Add(' AND remesa_fr = :referencia_r ');
    SQL.Add(' AND fecha_remesa_fr = :fecha_r ');
    SQL.Add(' ORDER BY planta_fr, factura_fr ');
  ...
  begin
    ...
    SQL.Add('        sum((fecha_remesa_fr - fecha_factura_fr)* importe_cobrado_fr) dias_cobro');
    SQL.Add(' FROM frf_facturas_remesa ');
    SQL.Add(' WHERE empresa_fr = :empresa_r ');
    SQL.Add(' AND remesa_fr = :referencia_r ');
    SQL.Add(' AND fecha_remesa_fr = :fecha_r ');
  ...
  begin
    ...
    SQL.Add('          from frf_facturas_remesa ');
    SQL.Add('        where planta_fr = :planta ');
    SQL.Add('          and factura_fr = :factura ');
...
begin
  ...
  begin
    (*
       where exists
       ( select *
         from frf_facturas_remesa, frf_facturas
         where empresa_r = empresa_fr
           and referencia_r = remesa_fr
           and fecha_r = fecha_remesa_fr
           and planta_fr = empresa_f
           and factura_fr = n_factura_f
                  and fecha_factura_fr = fecha_factura_f
...
begin
  //Que no hayan campos vacios
  if Trim(planta_fr.Text) = '' then
  begin
    planta_fr.SetFocus;
    raise Exception.Create('Faltan Datos (Código de la Planta).');
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SQL.Add('   and n_factura_f = :factura ');
        SQL.Add('   and fecha_factura_f = :fecha ');
        ParamByName('planta').AsString:= planta_fr.Text;
      ...
      begin
        rCobrado:= ImportePendiente( empresa_r.Text, planta_fr.Text, iFactura, dFecha, ( QDetalle.State = dsEdit ), rTotalFactura );
...
begin
  ...
  if QDetalle.FieldByName('empresa_fr').AsString <> 'BAG' then
  begin
    QDetalle.FieldByName('planta_fr').AsString := QDetalle.FieldByName('empresa_fr').AsString;
  end
  else
  begin
    if Copy( sUltimaPlanta, 1, 1 ) = 'F' then
    begin
      QDetalle.FieldByName('planta_fr').AsString:= sUltimaPlanta;
    end;
  end;
end;

function  TFMRemesas.FechaFactura( const AEmpresa_, APlanta: string; const AFactura, ARemesa: Integer;
                                   const AFechaRemesa: TDate; var VFecha: TDate ): boolean;
begin
  with QFechaFactura do
  begin
    ...
    if ARemesa = 0 then
      ...
      ParamByName('fecha').AsDate:= AFechaRemesa + 30;
    Open;
    if not IsEmpty then
    begin
      if ARemesa = 0 then
      ...
      begin
        ...
        begin
          if FieldByName('fecha').AsDateTime <= AFechaRemesa then
          begin
            VFecha:= FieldByName('fecha').AsDateTime;
            result:= True;
          end
          ...
          begin
            if FieldByName('tipo_factura_f').AsInteger = 381 then
            begin
              VFecha:= FieldByName('fecha').AsDateTime;
              result:= True;
            end;
...
(*TODO*)
//Parametros remesa
function TFMRemesas.ImportePendiente( const AEmpresa_, APlanta: string; const AFactura: Integer;
                                      const AFecha: TDate; const AModificar: boolean;
                                      var ATotalFactura: real ): real;
...
begin
  ...
  begin
    with QCobradoFacturaRemesa do
    begin
      ParamByName('empresa').AsString:= AEmpresa_;
      ParamByName('planta').AsString:= APlanta;
      ParamByName('factura').AsInteger:= AFactura;
...
begin
 ...
 begin
  ...
  begin
    if FechaFactura( empresa_r.Text, planta_fr.Text, iFactura, iRemesa, TDate(QRemesasExpfecha_r.AsDateTime), TDate(dFecha) ) then
    begin
      sFecha:= FormatDateTime('dd/mm/yyyy', dFecha );
      rImporte:= ImportePendiente( empresa_r.Text, planta_fr.Text, iFactura, dFecha, ( QDetalle.State = dsEdit ), rTotalFactura );
      fecha_factura_fr.Text:= sFecha;
...
begin
  sUltimaPlanta:= DataSet.FieldByname('planta_fr').AsString;
  QDetalleTotal.Close;
  QDetalleTotal.Open;
  PutTotalesFacturas;
end;
...
begin
  with QImporteFacturaRemesa do
  begin
    ParamByName('planta').AsString:= QDetalleplanta_fr.AsString;
