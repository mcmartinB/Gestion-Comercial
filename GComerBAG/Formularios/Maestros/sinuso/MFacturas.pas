...
type
  TFMFacturas = class(TMaestro)
    ...
    LPorc_impuesto_f: TLabel;
    qryGiros: TQuery;
    dsGiros: TDataSource;
    lblRemesaGiro: TLabel;
    edtRemesaGiro: TBDEdit;
    lblGiro: TLabel;
    edtGiro: TBDEdit;
    lbl1: TLabel;
    edtFechaRemesa: TBDEdit;
    chkDescuento: TCheckBox;
    lblCodigo1: TnbLabel;
    lblCodigo2: TnbLabel;
    importe_fal: TBDEdit;
    descuento_fal: TBDEdit;
    porc_descuento_fal: TBDEdit;
  ...
  private
    ...
    procedure DescuentoCliente( const AAlta: Boolean );
...
uses bMath, UDMAuxDB, CVariables, CGestionPrincipal, UDMBaseDatos,
     UDMFacturacion, CAuxiliarDB, Principal, bDialogs, LFRepticionFacturas,
...
begin
  ...
  begin
    ...
    SQL.Add(' where planta_fr = :empresa_f ');
  ...
  begin
    SQL.Clear;
    SQL.Add(' select n_remesa_fgc, fecha_remesa_fgc, n_giro_fgd ');
    SQL.Add(' from fct_remesa_giro_d, fct_remesa_giro_c ');
    SQL.Add(' where empresa_fgd = :empresa_f ');
    SQL.Add(' and n_factura_fgd = :n_factura_f ');
    SQL.Add(' and fecha_factura_fgd = :fecha_factura_f ');
    SQL.Add(' and empresa_fgc = empresa_fgd ');
    SQL.Add(' and n_remesa_fgc = n_remesa_fgd ');
    if not Prepared then
      Prepare;
  end;
...
function FiltroGiro( const ARemesa, AFecha, AGiro: string ): string;
begin
  Result:= '';
  with DMAuxDB.QAux do
  begin
    SQL.Clear;
    SQL.Add('  select empresa_fgc, n_factura_fgd, fecha_factura_fgd ');
    SQL.Add('    from fct_remesa_giro_c, fct_remesa_giro_d ');
    SQL.Add('    where empresa_fgc = empresa_fgd ');
    SQL.Add('    and n_remesa_fgc = n_remesa_fgd ');

    if ARemesa <> '' then
      SQL.Add('    and n_remesa_fgc = '  + ARemesa );
    if AFecha <> '' then
      SQL.Add('    and fecha_remesa_fgc = '  + QuotedStr(AFecha) );
    if AGiro <> '' then
      SQL.Add('    and n_giro_fgd = '  + AGiro );
    Open;
    while not Eof do
    begin
      if Result <> '' then
        Result:= Result + ' or ';
      Result:= Result + ' ( empresa_f = ' + QuotedStr( FieldByName('empresa_fgc').AsString ) +
                         ' and n_factura_f = ' + FieldByName('n_factura_fgd').AsString +
                         ' and fecha_factura_f = ' + QuotedStr( FieldByName('fecha_factura_fgd').AsString ) + ' ) ';
      Next;
    end;
    Close;
  end;
...
begin
  ...
  if ( trim( edtRemesaGiro.Text ) <> '' ) or ( Trim( edtFechaRemesa.Text ) <> '' ) or ( trim( edtGiro.Text ) <> '' )  then
  begin
    sAux:= FiltroGiro( trim( edtRemesaGiro.Text ), Trim( edtFechaRemesa.Text ), Trim( edtGiro.Text ) );
    if flag then
    begin
      where := where + ' and ';
    end;
...
begin
  ...
  begin
    FLRepticionFacturas := TFLRepticionFacturas.Create(nil);
    FLRepticionFacturas.Height:= 0;
    FLRepticionFacturas.Width:= 0;
    FLRepticionFacturas.Enabled := False;
    FLRepticionFacturas.eEmpresa.Text := QFacturas.FieldByName('empresa_f').AsString;
    FLRepticionFacturas.eCliente.Text := QFacturas.FieldByName('cliente_fac_f').AsString;
    FLRepticionFacturas.eFacturaHasta.Text := QFacturas.FieldByName('n_factura_f').AsString;
    FLRepticionFacturas.eFacturaDesde.Text := QFacturas.FieldByName('n_factura_f').AsString;
    FLRepticionFacturas.eFechaHasta.Text := QFacturas.FieldByName('fecha_factura_f').AsString;
    FLRepticionFacturas.eFechaDesde.Text := QFacturas.FieldByName('fecha_factura_f').AsString;
    FLRepticionFacturas.printOriginal.Checked := True;
    FLRepticionFacturas.printEmpresa.Checked := False;
    FLRepticionFacturas.cbxSoloVer.Checked := True;
    try
      FLRepticionFacturas.RepetirFactura;
    finally
      FreeAndNil(FLRepticionFacturas);
...
begin
  ...
  begin
    pBotonera.Enabled := cbxContabilizado_f.Enabled;
    pgFactura.Enabled := cbxContabilizado_f.Enabled;
  end
  else
  begin
  ...
  case TButton(Sender).Tag of
    ...
    2:
      begin
        ...
        stEnvase.Caption:= desEnvaseP(empresa_f.Text, envase_fal.Text, producto_fal.Text);
      end;
    3:
      begin
...
var
  //neto_fal, descuento_fal, importe_fal, porc_descuento_fal
...
begin
  ...
  *)
  ...
  begin
    ...
    rAuxNeto:= bRoundTo( ABS( StrToFloatDef( importe_neto_f.Text, 0 ) ) - ABS( rAuxNeto ), 2 );
    if rAuxNeto < 0 then
    begin
  ...
  //Todas las lineas tienen que el mismo descuento
...
begin
  case TBEdit(Sender).Tag of
    ...
    kEnvase: stEnvase.Caption:= desEnvaseP(empresa_f.Text, envase_fal.Text, producto_fal.Text);
  end;
end;
...
begin
  ...
  begin
    ...
    if InputQuery('Buscar facturas de la remesa','Código de la planta:', sEmpresaRemesa) then
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SQL.Add('select *');
          SQL.Add('from frf_facturas_remesa');
          SQL.Add('where planta_fr = :empresa');
...
begin
  ...
  edtRemesaGiro.Enabled:= AEnable;
  edtFechaRemesa.Enabled:= AEnable;
  edtGiro.Enabled:= AEnable;
  LContabilizado_f.Enabled:= AEnable;
  importe_cobrado.Enabled:= AEnable;
  DBGRemesas.Enabled:= not AEnable;
end;

function TFMFacturas.GetDescuento: real;
begin
    ...
    DMAuxDB.QAux.ParamByName('cliente').AsString:= cliente_fac_f.Text;
    DMAuxDB.QAux.ParamByName('fecha').AsDateTime:= StrToDate( fecha_factura_f.Text );
    DMAuxDB.QAux.Open;
...
procedure TFMFacturas.DescuentoCliente( const AAlta: Boolean );
begin
  DMAuxDB.QAux.SQL.Clear;
  DMAuxDB.QAux.SQL.Add(' select max( nvl(descuento_fal,0) ) descuento, count(*) descuentos ');
  DMAuxDB.QAux.SQL.Add(' from frf_fac_abonos_l ');
  DMAuxDB.QAux.SQL.Add(' where empresa_fal = :empresa ');
  DMAuxDB.QAux.SQL.Add(' and factura_fal = :factura ');
  DMAuxDB.QAux.SQL.Add(' and fecha_fal = :fecha ');
  DMAuxDB.QAux.ParamByName('empresa').AsString:= empresa_f.Text;
  DMAuxDB.QAux.ParamByName('factura').AsInteger:= StrToInt( n_factura_f.Text );
  DMAuxDB.QAux.ParamByName('fecha').AsDateTime:= StrToDate( fecha_factura_f.Text );
  DMAuxDB.QAux.Open;
  ...
  begin
    if AAlta then
    begin
      chkDescuento.Enabled:= DMAuxDB.QAux.FieldByName('descuentos').AsInteger = 0;
    end
    else
    begin
      chkDescuento.Enabled:= DMAuxDB.QAux.FieldByName('descuentos').AsInteger = 1;
    end;
    chkDescuento.Checked:= DMAuxDB.QAux.FieldByName('descuento').AsFloat <> 0;
  end;
...
procedure TFMFacturas.chkDescuentoClick(Sender: TObject);
begin
  if chkDescuento.Checked then
  begin
    porc_descuento_fal.Text:= FloatToStr( rPorcDescuento );
