...
var
  QRLFacturas: TRIFacturas;

implementation

uses LSelecFacturas, CAuxiliarDB, UDMAuxDB, StrUtils, CVariables,
     bMath, bSQLUtils, UDMBaseDatos, bTextUtils;

{$R *.DFM}

constructor TRIFacturas.Create(AOwner: TComponent);
begin
  inherited;
  definitivo := False;
  printEmpresa := True;
...
procedure TRIFacturas.QuickReport1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  salto := 0;
  AlbaranNum := -1;
...
procedure TRIFacturas.CuadroFormaPago;
var
...
procedure TRIFacturas.BandaTotalesBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRIFacturas.CabeceraFacturaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  ...
  begin
    ...
    begin
      qlProvincia.Enabled := false;
      qlCif.Caption := 'V.A.T. : ' +
        self.DataSet.FieldByName('pais_c').AsString +
        self.DataSet.FieldByName('nif_tfc').AsString; ;
    end;
  end;
...
procedure TRIFacturas.BandaLineaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
function TRIFacturas.Unidades(cad: string): string;
begin
     //Cantidad  cajas
...
procedure TRIFacturas.CabeceraGastosAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  CabeceraGastos.Enabled := false;
end;

procedure TRIFacturas.BandaGastosBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRIFacturas.BonnyBandBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if Hoja = 1 then
  begin
...
procedure TRIFacturas.PiePaginaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  psEtiqueta.Left:= 12;
  ...
  begin
    ...
    qrmReponsabilidadEnvase.Lines.Add('"De conformidad con lo establecido en la ley 11/1997,  de 24 de abril, de envases y residuos de envases y,  el artículo 18.1 del  Real Decreto 782/1998,  de 30 de Abril  que desarrolla la Ley 11/1997;');
    qrmReponsabilidadEnvase.Lines.Add('el responsable de la entrega del residuo de envase o envase usado para su correcta  gestión ambiental de aquellos envases no identificados mediante el Punto Verde (sistema integrado de gestión');
    qrmReponsabilidadEnvase.Lines.Add('de Ecoembes), será el poseedor final del mismo".');
  end
  else
  begin
    qrmReponsabilidadEnvase.Lines.Clear;
    qrmReponsabilidadEnvase.Lines.Add('"In accordance with which it  is established in law 11/1997 dated April 24th about  packaging and packaging waste,  according to article 18.1 of  Royal decree 782/1998 dated  April 30th that develops');
    qrmReponsabilidadEnvase.Lines.Add('law 11/1997; the responsible for the delivery of packaging and packaging waste used for proper enviroment management  from those packaging non identified by the green point (inegrated management');
    qrmReponsabilidadEnvase.Lines.Add('system Ecoembes), will be the final holder".');
  end;
end;

procedure TRIFacturas.CabeceraLineaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if detalles >= 0 then
  begin
...
procedure TRIFacturas.CabeceraFactura2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRIFacturas.BandaTotalesAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  Hoja := 0;
  sigue.Enabled := false;
...
procedure TRIFacturas.CabeceraFacturaAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
...
procedure TRIFacturas.BandaLineaAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
...
procedure TRIFacturas.BandaGastosAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  if CurrentY > 2645 then
    ForceNewPage;
...
procedure TRIFacturas.QRLFacturasStartPage(Sender: TCustomQuickRep);
begin
     //Nueva hoja
  Inc(Hoja);
  Bandas := 0;
...
procedure TRIFacturas.CabeceraFactura2AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  Inc(NumeroHoja);
end;

procedure TRIFacturas.PonProducto;
...
procedure TRIFacturas.Configurar(const AEmpresa, ACliente: string);
var
  bAux: Boolean;
  sAux: string;
  ssEmpresa: string;
begin
  ...
  begin
    ...
    begin
      imgQRImgLogo.Picture.LoadFromFile( gsDirActual + '\recursos\LogoPicaroGourmet.jpg');
    end;
    Exit;
  end
  else
...
procedure TRIFacturas.PedidoPrint(sender: TObject; var Value: string);
begin
  Value := '';
...
procedure TRIFacturas.AlbaranPrint(sender: TObject;
  var Value: string);
begin
  (*ALBARAN*)
...
procedure TRIFacturas.PsQRLabel33Print(sender: TObject;
  var Value: string);
begin
...
procedure TRIFacturas.PsQRLabel36Print(sender: TObject;
  var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
    Value := ''
...
procedure TRIFacturas.PsQRLabel34Print(sender: TObject;
  var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
    Value := 'Datum'
...
procedure TRIFacturas.PsQRLabel35Print(sender: TObject;
  var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
    Value := 'Kunden Nr.'
...
procedure TRIFacturas.alb2Print(sender: TObject; var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
    Value := 'Referenz'
  else
...
procedure TRIFacturas.pro2Print(sender: TObject; var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
    Value := 'Produkt'
  else
...
procedure TRIFacturas.can2Print(sender: TObject; var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
    Value := 'Menge'
  else
...
procedure TRIFacturas.uni2Print(sender: TObject; var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
    Value := 'Einheiten'
  else
...
procedure TRIFacturas.pre2Print(sender: TObject; var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
    Value := 'E´Preis'
  else
...
procedure TRIFacturas.qrlImp2AuxPrint(sender: TObject; var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
    Value := 'Preis Eur'
  else
...
procedure TRIFacturas.ConfiguraNeto;
begin
...
procedure TRIFacturas.ConfiguraIVA;
begin
...
procedure TRIFacturas.ConfiguraTotal;
begin
  if RImportesCabecera.rTotalIva <> 0 then
  begin
...
procedure TRIFacturas.ConfiguraTotalEuros;
begin
...
procedure TRIFacturas.ConfiguracionCuadroTotales;
begin
  ConfiguraNeto;
  ConfiguraIVA;
  ConfiguraTotal;
...
procedure TRIFacturas.ConfigurarGastosYDescuentos;
begin
...
procedure TRIFacturas.HojaNumPrint(sender: TObject; var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
...
procedure TRIFacturas.plinPrint(sender: TObject; var Value: string);
begin
  if DMFacturacion.QCabeceraFactura.FieldByName('pais_tfc').AsString = 'ALEMANIA' then
    Value := ' Zahlung :'
  else
...
procedure TRIFacturas.DescriptiomPrint(sender: TObject;
  var Value: string);
begin
...
procedure TRIFacturas.LFecha2Print(sender: TObject; var Value: string);
begin
  Value := DMFacturacion.QCabeceraFactura.FieldByName('fecha_tfc').AsString;
end;

procedure TRIFacturas.LabelFechaPrint(sender: TObject; var Value: string);
begin
  Value := DMFacturacion.QCabeceraFactura.FieldByName('fecha_tfc').AsString;
end;

procedure TRIFacturas.BandaErrorCopiaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
function TRIFacturas.CambioFacturacion: boolean;
begin
  with DMAuxDB.QAux do
  begin
    SQL.Clear;
...
procedure TRIFacturas.bndComisionAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  bndComision.Enabled := false;
end;

procedure TRIFacturas.bndDescuentoAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  bndDescuento.Enabled := false;
end;

procedure TRIFacturas.bndDescuentoComisionBeforePrint(
...
procedure TRIFacturas.bndDescuentoComisionAfterPrint(
...
function TRIFacturas.NotaFactura: string;
begin
  with DMAuxDB.QAux do
  begin
    SQL.Clear;
...
procedure TRIFacturas.BandaNotasBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRIFacturas.qrlMNetoPrint(sender: TObject; var Value: String);
begin
  Value:= DMFacturacion.QCabeceraFactura.FieldByName('cod_moneda_tfc').AsString;
end;

procedure TRIFacturas.BandaObservacionesBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
