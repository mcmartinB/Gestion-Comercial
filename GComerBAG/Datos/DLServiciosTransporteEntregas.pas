...
type
  ...
  public
    { Public declarations }
    function ObtenerDatos( const AEmpresa, ACentroLlegada, ATransportista, AProducto: string;
...
begin
  ...
  begin
    ...
    while not Eof do
    begin
      if ( dFecha = FieldByName('fecha_llegada_ec').AsDateTime ) and
      ...
      begin
        ...
        dFecha:= FieldByName('fecha_llegada_ec').AsDateTime;
...
begin
  ...
  begin
    ...
    while not Eof do
    begin
      if ( sSemana = AnyoMes( FieldByName('fecha_llegada_ec').AsDateTime ) ) and
         ( iTransporte = FieldByName('transporte_ec').AsInteger ) and
         ( sMatricula = FieldByName('vehiculo_ec').AsString ) then
      begin
        Inc( iViajes );
      ...
      begin
        ...
        sSemana:= AnyoMes( FieldByName('fecha_llegada_ec').AsDateTime );
        iTransporte:= FieldByName('transporte_ec').AsInteger;
        sMatricula:= FieldByName('vehiculo_ec').AsString;
        iViajes:= 1;
        rKilos:= FieldByName('kilos_el').AsFloat;
...
begin
  ...
  begin
    ...
    while not Eof do
    begin
      if ( sSemana = AnyoSemana( FieldByName('fecha_llegada_ec').AsDateTime ) ) and
         ( iTransporte = FieldByName('transporte_ec').AsInteger ) and
         ( sMatricula = FieldByName('vehiculo_ec').AsString ) then
      begin
        Inc( iViajes );
      ...
      begin
        ...
        sSemana:= AnyoSemana( FieldByName('fecha_llegada_ec').AsDateTime );
        iTransporte:= FieldByName('transporte_ec').AsInteger;
        sMatricula:= FieldByName('vehiculo_ec').AsString;
        iViajes:= 1;
        rKilos:= FieldByName('kilos_el').AsFloat;
...
function TDMLServiciosTransporteEntregas.ObtenerDatos( const AEmpresa, ACentroLlegada, ATransportista, AProducto: string;
                                                        const AFechaIni, AFechaFin: TDateTime;
                                                        const AVehiculo: boolean;
                                                        const APortes, AAgrupacion: integer ): boolean;
begin
  ...
  begin
    ...
    if AVehiculo then
      SQL.Add(' select transporte_ec, vehiculo_ec, fecha_llegada_ec, vehiculo_ec matricula, ')
    else
      SQL.Add(' select transporte_ec, '''' vehiculo_ec, fecha_llegada_ec,  vehiculo_ec matricula, ');
