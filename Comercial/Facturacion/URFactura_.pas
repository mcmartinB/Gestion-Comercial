...
uses Windows, SysUtils, Messages, Classes, Graphics, Controls,
  StdCtrls, ExtCtrls, Forms, QuickRpt, QRCtrls, UDFactura, DB, kbmMemTable, BonnyQuery;

type
  TRFactura_ = class(TQuickRep)
    BonnyBand: TQRBand;
    PiePagina: TQRBand;
    QRImageCab: TQRImage;
    PsEmpresa: TQRLabel;
  ...
  private
    ...
    function  GetKilosNeto(const ACodFactura, AEmpresa: string; const AFactura: integer; const Afecha: TDateTime ): Real;
...
var
  RFactura_: TRFactura_;

implementation

uses bMath, CFactura, UDMAuxDB, bSQLUtils, CVariables, bTextUtils, CGlobal;

{$R *.DFM}

constructor TRFactura_.Create(AOwner: TComponent);
begin
  inherited;
  definitivo := False;
  bProforma:= False;
...
procedure TRFactura_.BonnyBandBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if Hoja = 1 then
  begin
...
procedure TRFactura_.CabeceraFactura2AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  Inc(NumeroHoja);
end;

procedure TRFactura_.CabeceraLineaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if detalles >= 0 then
  begin
...
procedure TRFactura_.CabeceraFactura2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFactura_.CabeceraFacturaBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
var i:Integer;
begin
...
procedure TRFactura_.CabeceraFacturaAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
...
procedure TRFactura_.qrgCabAlbaranBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrgCabAlbaran.Height:= 0;
end;

procedure TRFactura_.BandaLineaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFactura_.BandaLineaAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
...
procedure TRFactura_.CabeceraGastosAfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
  CabeceraGastos.Enabled := false;
end;

procedure TRFactura_.bndDescuentoComisionBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFactura_.bndDescuentoComisionAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
...
procedure TRFactura_.BandaTotalesBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  CuadroFormaPago;
...
procedure TRFactura_.BandaTotalesAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  Hoja := 0;
  sigue.Enabled := false;
...
procedure TRFactura_.BandaErrorCopiaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFactura_.BandaNotasBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if not bBruto then
  begin
...
procedure TRFactura_.BandaObservacionesBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFactura_.PiePaginaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFactura_.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  ...
  FacturaNum := -2;
  Hoja := 0;
  RFactura_.ReportTitle := StringReplace('Facturas ' + DFactura.mtFacturas_Cab.fieldbyname('cod_cliente_fc').AsString, '.', '', [rfReplaceAll, rfIgnoreCase]);
  AsignarTemporales;
end;

procedure TRFactura_.QuickRepStartPage(Sender: TCustomQuickRep);
begin
     //Nueva hoja
  Inc(Hoja);
  Bandas := 0;
...
procedure TRFactura_.ConfigurarGastosYDescuentos;
begin
  bndDescuentoComision.Enabled := false;
...
procedure TRFactura_.ConfiguracionCuadroTotales;
var i: Integer;
begin
...
procedure TRFactura_.PonProducto;
var
...
begin
  ...
  begin
    ...
    begin
         //PRODUCTO
      if ((Trim(DFactura.mtFacturas_Det.FieldByName('cod_producto_fd').AsString) = 'E') or
        (Trim(DFactura.mtFacturas_Det.FieldByName('cod_producto_fd').AsString) = 'T')) and
      ...
      begin
       ...
       //******************************************************
    if (DFactura.mtFacturas_Det.FieldByName('cod_producto_fd').AsString = 'T') or
      (DFactura.mtFacturas_Det.FieldByName('cod_producto_fd').AsString = 'E') then
    begin
      sProducto := sProducto + ' (' + DFactura.mtFacturas_Det.FieldByName('calibre_fd').AsString + ')';
    end;

    if (DFactura.mtFacturas_Det.FieldByName('cod_producto_fd').AsString = 'F') and
...
function TRFactura_.Unidades(cad: string): string;
begin
   //Cantidad  cajas
...
procedure TRFactura_.CuadroFormaPago;
var
...
function TRFactura_.CambioFacturacion: Boolean;
begin
...
procedure TRFactura_.Configurar(const AEmpresa, ACliente: string);
var
  bAux: Boolean;
  sAux: string;
  ssEmpresa: string;
...
procedure TRFactura_.LabelFechaPrint(sender: TObject; var Value: String);
begin
  Value := DFactura.mtFacturas_Cab.fieldbyname('fecha_factura_fc').AsString;
end;

procedure TRFactura_.LFecha2Print(sender: TObject; var Value: String);
begin
  Value := DFactura.mtFacturas_Cab.fieldbyname('fecha_factura_fc').AsString;
end;

procedure TRFactura_.AlbaranPrint(sender: TObject; var Value: String);
begin
  (*ALBARAN*)
...
procedure TRFactura_.PedidoPrint(sender: TObject; var Value: String);
begin
  Value := '';
...
procedure TRFactura_.CabeceraTotales(const AIndice: Integer; AFlag: Boolean);
begin
...
procedure TRFactura_.ValoresTotales(const AIndice: Integer; AFlag: Boolean);
var
...
procedure TRFactura_.ConfiguraIVA(const AFlag: Boolean);
...
begin
  ...
  if (DFactura.mtFacturas_Cab.FieldByName('importe_impuesto_fc').AsFloat <> 0) or
     ((gProgramVersion = pvBAG) and (PaisCliente( DFactura.mtFacturas_Cab.FieldByName('cod_empresa_fac_fc').AsString,
                                                  DFactura.mtFacturas_Cab.FieldByName('cod_cliente_fc').AsString ) = 'ES')) then
  begin
...
procedure TRFactura_.QuickRepAfterPrint(Sender: TObject);
begin
  DFactura.mtFacturas_Bas.Filter := '';
  DFactura.mtFacturas_Bas.Filtered := False;
...
procedure TRFactura_.bndComisionBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFactura_.bndDescuentoBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
...
procedure TRFactura_.bndComisionAfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
  bndDescuentoComision.Enabled := (DetallesComision = 0) and (DetallesDescuento = 0) and (DetallesEurosKg = 0);
end;

procedure TRFactura_.bndDescuentoAfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
  bndDescuentoComision.Enabled := (DetallesComision = 0) and (DetallesDescuento = 0) and (DetallesEurosKg = 0);
end;

procedure TRFactura_.BandaGastosBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Inc(DetallesGasto, -1);
...
procedure TRFactura_.HojaNumPrint(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
...
procedure TRFactura_.plinPrint(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := ' Zahlung :'
  else
...
procedure TRFactura_.DescriptiomPrint(sender: TObject; var Value: String);
begin
...
procedure TRFactura_.PsQRLabel33Print(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := 'Rechnung Nr.'
  else
...
procedure TRFactura_.PsQRLabel36Print(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := ''
  else
...
procedure TRFactura_.PsQRLabel34Print(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := 'Datum'
  else
...
procedure TRFactura_.PsQRLabel35Print(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := 'Kunden Nr.'
  else
...
procedure TRFactura_.qrlAlbaran2Print(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := 'Referenz'
  else
...
procedure TRFactura_.qrlProducto2Print(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := 'Produkt'
  else
...
procedure TRFactura_.qrlCantidad2Print(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := 'Menge'
  else
...
procedure TRFactura_.qrlUnidad2Print(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := 'Einheiten'
  else
...
procedure TRFactura_.qrlPrecio2Print(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := 'E´Preis'
  else
...
procedure TRFactura_.qrlImporte2Print(sender: TObject; var Value: String);
begin
  if DFactura.mtFacturas_Cab.FieldByName('des_pais_fc').AsString = 'ALEMANIA' then
    Value := 'Preis Eur'
  else
...
procedure TRFactura_.AsignarTemporales;
begin
...
procedure TRFactura_.ValoresTotalesEur(const AIndice: Integer; AFlag: Boolean);
var
  iInicio, iAltura, iIncCuadro, iIncTexto: Integer;
begin
...
procedure TRFactura_.DesactivarBases;
var i: integer;
    Etiqueta:  String;
begin
  for i:= 0 to 2 do
...
procedure TRFactura_.ActivarBases;
var i: integer;
    Etiqueta: String;
begin
  i := 0;
...
procedure TRFactura_.CreaQGrupoSAT;
begin
...
function TRFactura_.EsGrupoSAT: boolean;
begin
  with QGrupoSAT do
  begin
    if Active then
...
procedure TRFactura_.bndEurosKgBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFactura_.bndEurosKgAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  bndDescuentoComision.Enabled := (DetallesComision = 0) and (DetallesDescuento = 0) and (DetallesEurosKg = 0);
end;

procedure TRFactura_.bndPieAlbaranBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
function TRFactura_.GetKilosNeto(const ACodFactura, AEmpresa: string; const AFactura: integer; const Afecha: TDateTime ): Real;
begin
