...
type
  TFPrincipal = class(TForm)
    ...
    mnuEntEntregasSinAsociar: TMenuItem;
    ...
    procedure EnvasesComerciales1Click(Sender: TObject);
...
procedure TFPrincipal.EnvasesComerciales1Click(Sender: TObject);
begin
...
begin
  //Sólo podemos salir si el formulario activo es el principal
...
//  ConfigurarOpcion(  mnuFchProductoEDI, True , 'Códigos Facturación EDI', '');
  ...
  ConfigurarOpcion(  mnuFchPaises, True, 'Países', '');
  ...
  ConfigurarOpcion(  mnuFchTiposVia, True, 'Tipo de Vías', '');
...
begin
  ...
  ConfigurarOpcion(  mnuEntResEntradasTransportistas, True, 'Resumen Entradas Transportista', '');
  ConfigurarOpcion(  mnuEntEntradasRecoleccion, True, 'Resumen Entradas por Plantación', '');
  ...
  ConfigurarOpcion(  mnuEntVerificarEscandallos, True, 'Verificar Grabación de Escandallos', '');
end;
...
begin
  ...
  ConfigurarOpcion(  mnuEntFacturasPlatano, false, 'Facturas Despacho Plátano Canario', '');
  ...
  ConfigurarOpcion(  mnuEntSepStockAlmacen,  not DMConfig.EsLaFont, '-', 'Confeccionado Almacén');
...
//  ConfigurarOpcion(  mnuEntPaletsProveedorVolcados,  not DMConfig.EsLaFont, 'Palets Volcados', '');
  ...
  ConfigurarOpcion(  mnuStockFrutaConfeccionadaEnvases,  not DMConfig.EsLaFont, 'Stock Fruta Confeccionada por Artículos', '');
...
//  ConfigurarOpcion(  mnuEntBeneficioProducto, DMConfig.EsLaFont, 'Beneficio Producto', '');
  ...
  ConfigurarOpcion(  mnuEntSepLiquidacion, True, '-', 'Liquidación' );
...
//  ConfigurarOpcion(  mnuEntLiquidacionEntrega, DMConfig.EsLaFont, 'Liquidación Entrega' );
//  ConfigurarOpcion(  mnuEntPrevisionCostesProducto, DMConfig.EsLaFont, 'Previsión Costes Producto' );

//  if DMConfig.EsLaFont then
//    ConfigurarOpcion(  mniEntMargenBeneficio, True , 'Margen Beneficio', '')
//  else
//    ConfigurarOpcion(  mniEntMargenBeneficio, True , 'Parte confección', '');
...
//  ConfigurarOpcion(  mnuEntCuadreSemanalRFAlmacen, True, 'Cuadre Semanal RF Almacén' );
...
begin
  ConfigurarOpcion(  mnuAlmacen, True, '&Almacén', '');

  ConfigurarOpcion(  mnuAlmSepAlmacen, True, '-', 'Gestión del Inventario');
  ConfigurarOpcion(  mnuAlmInventario, True, 'Inventario de Almacén', '');
  ConfigurarOpcion(  mnuAlmResumenInventario, True, 'Resumen de Inventarios', '');
  ConfigurarOpcion(  mnuAlmConfeccionadoEnvases, True, 'Confeccionado por Artículos', '');
  ConfigurarOpcion(  mnuAlmConfeccionadoAgrupacion, True, 'Confeccionado por Agrupación', '');
  ConfigurarOpcion(  mnuParteProduccion, True, 'V2.- Parte Producción', '');
  ...
  ConfigurarOpcion(  mnuAlmSepSobrepesos, True, '-', 'Sobrepeso del Artículo');
  ConfigurarOpcion(  mnuAlmSobrepesoEnvase, True, 'Sobrepeso del Artículo', '');
  ConfigurarOpcion(  mnuAlmVerificarGrabacionEnvasado, True, 'Verificar la Grabación del Sobrepeso', '');
  ...
  ConfigurarOpcion(  mnuAlmSepCosteEnvase, True, '-', 'Costes del Artículo');
  ConfigurarOpcion(  mnuAlmCostesEnvase, True, 'Costes por KG Artículo (Mensual)', '');
  ConfigurarOpcion(  mnuAlmVerificarCosteEnvases, True, 'Verificar Grabación Costes Artículo', '');
  ...
  ConfigurarOpcion(  mnuAlmPreciosDiariosrEnvases, false, 'Precios Diarios por Artículos', '');
  ConfigurarOpcion(  mnuAlmPrecioVentaCliente, DMConfig.EsLaFont, 'Precio Venta por Cliente y Artículo', '');
end;
...
begin
  ConfigurarOpcion(  mnuLiquidacion, DMConfig.EsLaFont, '&Liquidación', '');
...
begin
  ConfigurarOpcion(  mnuTransitos, True, '&Tránsitos', '');

  ConfigurarOpcion(  mnuTraSepTransistos, True, '-', 'Gestión de Tránsitos');
  ConfigurarOpcion(  mnuTraTransitos, True, 'Tránsitos', '');
  ConfigurarOpcion(  mnuTraLisTransitos, True, 'Listado Tránsitos', '');
  ConfigurarOpcion(  mnuTraConsultaTransitos, True, 'Consulta de Tránsitos', '');
  ConfigurarOpcion(  mnuTraAsignarKilosTransitos, DMConfig.EsLaFont, 'Asignar Kilos de Tránsitos', '');
  ConfigurarOpcion(  mnuTraLisEnvasesTransitos, True, 'Resumen de Artículos de Tránsitos', '');
  ConfigurarOpcion(  mnuTraServiciosTransportistas, True, 'Resumen Servicios por Transportista', '');
  ConfigurarOpcion(  mnuTraListadoFacturasdeGastosTransitos, True, 'Listado Facturas de Gastos Tránsitos', '');

  ConfigurarOpcion(  mnuTraSepDeposito, DMConfig.EsLaFont, '-', 'Depósito de Aduanas');
  ConfigurarOpcion(  mnuTraListadoDepositoAduanas, DMConfig.EsLaFont, 'Listado Depósito de Aduanas', '');
  ConfigurarOpcion(  mnuTraLFacturasTransportesDeposito, DMConfig.EsLaFont, 'Listado Facturas de Transporte Depósito de Aduanas', '');
  ConfigurarOpcion(  mnuTraCrearFicherosAplicacionAtlantis, DMConfig.EsLaFont, 'Crear Ficheros Aplicación Atlantis', '');

  ConfigurarOpcion(  mnuTraSepPOSEI, DMConfig.EsLaFont, '-', 'Ayuda POSEI');
  ConfigurarOpcion(  mnuTraCalculoAyudaPOSEI, DMConfig.EsLaFont, 'Cálculo Ayuda POSEI', '');
  ConfigurarOpcion(  mnuTraInformeAyudaPOSEI, DMConfig.EsLaFont, 'Informe Ayuda POSEI', '');

  ConfigurarOpcion(  mnuTraSepRecepcionTransitos, not DMConfig.EsLaFont, '-', 'Recepción de Tránsitos');
  ConfigurarOpcion(  mnuTraRecepcionTransitos, not DMConfig.EsLaFont, 'Recepción de Tránsitos', '');
end;
...
begin
  ...
  ConfigurarOpcion(  mnuPedAccesoWeb, True, 'Proceso de Pedidos Aplicación WEB', '');
  ConfigurarOpcion(  mnuPedConfirmaRecepcion, True, 'Confirmación de Recepción de Mercancías En Mercadona', '');
end;
...
begin
  ...
  ConfigurarOpcion(  mnuSalSepSalidas, True, '-', 'Gestión Salidas');
  ConfigurarOpcion(  mnuSalTablaSalidas, True, 'Tabla Dinámica de Salidas', '');
  ConfigurarOpcion(  mnuSalResumenSalidasDinamico, True, 'Resumen Salidas Dinámico', '');
  ConfigurarOpcion(  mnuSalSalidas, True, 'Salidas', '');
  ConfigurarOpcion(  mnuSalEnvioAlbaranes, True, 'Envío de Albaranes por Correo Electrónico', '');
  ...
  ConfigurarOpcion(  mnuSalResumenAlmacen, True, 'Resumen de Almacén / Salidas por Periodo', '');
  ...
  ConfigurarOpcion(  mnuSalResSalidasEnvase, True, 'Listado de Salidas por Artículo', '');
  ...
  ConfigurarOpcion(  mnuSalTransitosEnvComer, True, 'Tránsitos Envases Comerciales', '');
  ...
  ConfigurarOpcion(  mnuSalListadoFOBSalidas, DMConfig.EsLaFont, 'Listado Salidas FOB por Artículos', '');
  ...
  ConfigurarOpcion(  mnuSalFOBClientes,DMConfig.EsLaFont, 'FOB Facturación Clientes', '');

  ConfigurarOpcion(  mnuSalSepFederacion, DMConfig.EsLaFontEx, '-', 'Asignar Federación');
  ConfigurarOpcion(  mnuSalSemanalExpediciones, DMConfig.EsLaFontEx, 'Declaración Semanal de Expediciones', '');
  ConfigurarOpcion(  mnuSalSubFederacion, DMConfig.EsLaFontEx, 'Asignar Federación', '');
  ConfigurarOpcion(  mnuSalPorcentajesFederacion, DMConfig.EsLaFontEx, 'Porcentajes Federación', '');
  ConfigurarOpcion(  mnuSalAsignarFederacion, DMConfig.EsLaFontEx, 'Asignar Federación', '');
  ConfigurarOpcion(  mnuSalModificacionFederacion, DMConfig.EsLaFontEx, 'Modificación Federación', '');
  ConfigurarOpcion(  mnuSalLisSalidasFederacion, DMConfig.EsLaFontEx, 'Resumen Salidas por Federación', '');
...
begin
  ...
  ConfigurarOpcion(  mnuFacturacion, bPuedoFacturar, '&Facturación', '');
  ...
  ConfigurarOpcion(  mnuProcFacturacion, bPuedoFacturar , 'Facturación Automática ', '');
  ConfigurarOpcion(  mnuRepFacturacion, bPuedoFacturar , 'Repetición Facturas ', '');
  ConfigurarOpcion(  mnuFacturarEdi, bPuedoFacturar , 'Fichero Facturas/Abonos EDI ', '');
  ConfigurarOpcion(  mnuContFacturas, bPuedoFacturar and bPuedoContabilizar and gbEscritura, 'Contabilización de Facturas ', '');
  ...
  ConfigurarOpcion(  mnuAnuFacturas, bPuedoFacturar , 'Anulación de Facturas ', '');
  ...
  ConfigurarOpcion(  mnuFacNotificacinVentasCredito, bPuedoFacturar, 'Notificación de Ventas a Crédito', '');
  ...
  ConfigurarOpcion(  mnuFacDiasMediosPagoClientes, bPuedoFacturar, 'Días Medios de Pago de Clientes', '');
...
begin
  ConfigurarOpcion(  mnuDatosTransmision, not DMConfig.EsLaFont, 'Tra&nsmisión', '');
  ...
  ConfigurarOpcion(  mnuDatDiarioTransitos, True, 'Tránsitos', '');
...
begin
  ...
  ConfigurarOpcion(  mnuUtiConversionDivisa, DMConfig.EsLaFont, 'Conversión Divisas', '');
  ConfigurarOpcion(  mnuUtiEuroConversor, DMConfig.EsLaFont, 'Euro Conversor', '');
  ConfigurarOpcion(  mnuUtiCompruebaGastos, DMConfig.EsLaFont, 'Comprobación de Gastos', '');
  ConfigurarOpcion(  mnuUtiSelectImpresoras, True, 'Seleccion de Impresoras', '');
  ConfigurarOpcion(  mnuUtiSelectCuentaCorreo, DMConfig.EsLaFont, 'Selección Cuenta de Correo', '');
  ConfigurarOpcion(  mnuUtiAdministracion, UpperCase( Copy( gsCodigo, 1, 4 ) ) = 'INFO', 'Administración (INFO)', '');
