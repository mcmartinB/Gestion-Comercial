...
type
  ...
  public
    ...
    procedure   CargaSQL(const AVenta, AOrigen, ACategoria, AAgrupacion: Boolean );
  end;
...
procedure TQRLInformeVentasSemanal.CargaSQL(const AVenta, AOrigen, ACategoria, AAgrupacion: Boolean );
begin
  with Query1 do
  begin
    SQL.Clear;
    SQL.Add(' select empresa_sl empresa, ');
    SQL.Add('        case when producto_sl = ' + QuotedStr('E') + ' then  ' + QuotedStr('T') + ' else producto_sl end  producto, ');
    if AAgrupacion then
      SQL.Add('        agrupa_comercial_e envase, ')
    else
    ...
    if AVenta then
      SQL.Add(' and centro_salida_sl = :venta ');
    if AOrigen then
...
begin
  if orden = 2 then
    Value := Query2.FieldByName('envase').Text + ' ' + DesEnvase(Query2.FieldByName('empresa').AsString, Query2.FieldByName('envase').Text)
  else
    Value := Query1.FieldByName('envase').Text + ' ' + DesEnvase(Query1.FieldByName('empresa').AsString, Query1.FieldByName('envase').Text);
end;
...
procedure TQRLInformeVentasSemanal.QRLabel1Print(sender: TObject;
  var Value: string);
begin
  Value := 'FIN PRODUCTO ' + QuotedStr(Copy(aux_grupo, 4, 1));
end;

procedure TQRLInformeVentasSemanal.QRLabel2Print(sender: TObject;
  var Value: string);
begin
  Value := 'FIN PRODUCTO ' + QuotedStr(Copy(grupo, 4, 1));
end;
