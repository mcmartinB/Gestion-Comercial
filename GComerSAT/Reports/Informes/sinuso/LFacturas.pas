...
begin
  ...
  begin
    ...
    rKilosPlataforma:= rKilosPlataforma + DMFacturacion.QLineaFactura.FieldBYName('kilos_tf').AsFloat;
...
procedure TQRLFacturas.Configurar(const AEmpresa: string);
var
  bAux: Boolean;
...
procedure TQRLFacturas.BandaNotasBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   qrlObservacionFactura.Caption:= NotaFactura;
   PrintBand:= qrlObservacionFactura.Caption <> '';
end;

procedure TQRLFacturas.UnidadPrint(sender: TObject; var Value: String);
begin
(*
...
procedure TQRLFacturas.qrgCabAlbaranBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
function TQRLFacturas.CostePlataforma( const AEmpresa, ACentro: string; const AAlbaran: integer;
                                       const AFecha: TDateTime; var APrecioPlataforma: real ): boolean;
var
  sCliente: string;
begin
...
procedure TQRLFacturas.PrecioUnidadPrint(sender: TObject;
  var Value: String);
begin
  if bPlataforma then
  begin
...
procedure TQRLFacturas.precio_neto_tfPrint(sender: TObject;
  var Value: String);
begin
  if bPlataforma then
  begin
...
procedure TQRLFacturas.bndPieAlbaranBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if bPlataforma then
  begin
  ...
  begin
    if bndPieAlbaran.Frame.DrawBottom = True then
      bndPieAlbaran.Height:= 2
    else
      bndPieAlbaran.Height:= 0;

    qrlCostePlataforma.Caption:= '';
    qrlUnidadPlataforma.Caption:= '';
    qrlKilosPlataforma.Caption:= '';
    qrlPrecioPlataforma.Caption:= '';
    qrlImportePlataforma.Caption:= '';
...
procedure TQRLFacturas.cod_postalPrint(sender: TObject; var Value: String);
begin
  if Trim(BDPais.DataSet.FieldByName('pais_tfc').AsString) = 'ESPAÑA' then
  begin
