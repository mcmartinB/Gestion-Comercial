...
type
  ...
  private
    ...
    procedure QuerySalidas( const ACliente, APais: string);
    Procedure QueryGastos;
    Procedure QueryCosteEnvase;
    procedure QueryDescuentos;
    Procedure QueryProductoBase;

    function  ObtenerSalidas( const Aempresa, ACentro, AProducto, ACategoria, ACliente, APais: string;
  ...
  public
    ...
    function ObtenerDatosListado( const Aempresa, ACentro, AProducto, ACategoria, ACliente, APais: string;
...
procedure TFFobCalibresData.QuerySalidas( const ACliente, APais: string);
begin
  with QSalidas do
  begin
    SQL.Clear;
  ...
  if ( length(APais) = 2 ) or (UPPERCASE(APais) = 'NACIONAL')  or (UPPERCASE(APais) = 'EXTRANJERO') then
  begin
    QSalidas.SQL.Add(' from frf_salidas_c, frf_salidas_l, frf_clientes  ');
  end
  else
  begin
...
function TFFobCalibresData.ObtenerDatosListado( const Aempresa, ACentro, AProducto, ACategoria, ACliente, APais: string;
                             const AFechaDesde, AFechaHasta: TDateTime;
                             const bGastos, bEnvase, bEnvasado: Boolean ): boolean;
begin
  scentroOrigen:= ACentro;
  kbmSalidasLin.MasterSource:= DSSalidas;
  result:= ObtenerSalidas( Aempresa, ACentro, AProducto, ACategoria, ACliente, APais, AFechaDesde, AFechaHasta );
  if result then
  begin
    if bGastos then
      PutGastosAlbaranes;
...
function TFFobCalibresData.ObtenerSalidas( const AEmpresa, ACentro, AProducto, ACategoria, ACliente, APais: string;
                                           const AFechaDesde, AFechaHasta: TDateTime ): boolean;
begin
  QuerySalidas( ACliente, APais );
  with QSalidas do
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          kbmSalidasLin.FieldByName('gastos').AsFloat:= bRoundTo( FieldByName('gastos').AsFloat *  rChange, -2 );
          kbmSalidasLin.Post;
        end;
      end
      else
...
procedure TFFobCalibresData.GetDescuento( var AComision, ADescuento: Real );
begin
...
procedure TFFobCalibresData.GetDescuentoAux( var AComision, ADescuento: Real );
begin
  with QDescuentos do
  begin
...
begin
  ...
  begin
    ...
    try
      Open;
      AEnvase := FieldByName('coste_ec').AsFloat;
