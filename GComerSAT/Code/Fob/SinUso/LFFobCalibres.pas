...
uses
  ...
  BGrid, Derror, DBTables, ExtCtrls, DBCtrls, nbEdits, nbCombos;
...
begin
  ...
  try
    if FFobCalibresData.ObtenerDatosListado( eEmpresa.Text, eCentro.Text, eProducto.Text,
                                          eCategoria.Text, sCliente, sPais, dFechaDesde, dFechaHasta,
...
procedure TFFobCalibres.cbxNacionalidadChange(Sender: TObject);
begin
...
procedure TFFobCalibres.cbxNacionalidadKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if Key = vk_return then PostMessage(Handle, WM_NEXTDLGCTL, 0, 0);
end;

procedure TFFobCalibres.edtClienteChange(Sender: TObject);
begin
  ...
  begin
    stCliente.Caption:= desCliente( eEmpresa.Text, edtCliente.Text);
  end;
end;
