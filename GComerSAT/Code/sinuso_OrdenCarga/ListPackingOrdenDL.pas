...
type
  TDLListPackingOrden = class(TDataModule)
    ...
    QListadoOrdenCargaenvase: TStringField;
    QListadoOrdenCargaorden: TIntegerField;
    QListadoOrdenCargafecha: TDateField;
    QListadoOrdenCargasscc: TStringField;
    QListadoOrdenCargaalbaran: TIntegerField;
    QListadoOrdenCargacliente: TStringField;
    QListadoOrdenCargacajas: TFloatField;
    QListadoOrdenCargakilos: TFloatField;
    QListadoOrdenCargatransportista: TSmallintField;
...
begin
  ...
  begin
    SQL.Clear;
    SQL.Add(' select envase_pl envase, orden_occ orden, fecha_occ fecha, ean128_pl sscc, ');
    ...
    if AEnvase <> '' then
      SQL.Add('        else cajas_pl * ( select peso_neto_e from frf_envases where empresa_e = :empresa and producto_base_e = producto_base_pl and envase_e = :envase ) end ) kilos, ')
    else
      SQL.Add('        else cajas_pl * ( select peso_neto_e from frf_envases where empresa_e = :empresa and producto_base_e = producto_base_pl and envase_e = envase_pl ) end ) kilos, ');
    ...
    if AProducto <> '' then
      SQL.Add('   and producto_base_pl = :producto ');
    if AEnvase <> '' then
      SQL.Add('   and envase_pl = :envase ');
    if ACliente <> '' then
      SQL.Add('   and cliente_sal_occ = :cliente ');

    SQL.Add(' group by envase_pl, orden_occ, fecha_occ, ean128_pl, n_albaran_occ, cliente_sal_occ, transporte_occ ');
