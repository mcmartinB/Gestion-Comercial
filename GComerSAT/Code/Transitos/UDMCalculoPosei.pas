...
type
  TDMCalculoPosei = class(TDataModule)
    ...
    kmtOficialdes_producto: TStringField;
    kmtOficialcod_empresa_fac: TStringField;
    kmtOficialn_factura_fc: TIntegerField;
    kmtOficialfecha_factura: TDateField;
    kmtOficialnif_cliente: TStringField;
    kmtOficialcod_cliente: TStringField;
    kmtOficialdes_cliente: TStringField;
    kmtOficialanyo: TIntegerField;
    kmtOficialsemestre: TIntegerField;
    kmtInformen_albaran: TIntegerField;
    kmtOficialcodigo_pais: TStringField;
    kmtOficialdes_provincia: TStringField;
    kmtOficialdes_pais: TStringField;
    kmtOficialagrupacion: TIntegerField;
    kmtOficialdes_agrupacion: TStringField;
  private
    { Private declarations }
    iAnyo, iSemestre, iLinea: integer;
    sProducto, sAgrupacion, sDefinitivo: String;
    dFechaIni, dFechaFin: TDateTime;
    rKilosAsignados, rKilosPosei, rKilosAlbaran, rKilosParaAsignar, rKilosRestoTrans: real;
    rPrecioAlbaran: real;
    bExisteProdLlanos: boolean;

    procedure CalcularPosei(var VLabel: TLabel);
    procedure PreparaQuerys;
    function BorrarPosei: boolean;
    procedure InsertarPosei (const AKilos, AKilosAlb: Real);
    procedure InsertarPoseiAlbaran_1 (const AKilosPosei, AKilosAlb: Real);
    ...
    function CalcularPrecioAlbaran( const AEmpresa, ACentro, AAlbaran, AFecha, AProducto, AEnvase: string; AEsPacking: boolean): real;
    ...
    procedure InsertarkmtOficial;
    procedure ModificarkmtControl;
    procedure ModificarkmtOficial;

    function BuscarSigLinea: integer;

    function EsProdLlanos( const AEmpresa, ACentro, AProducto:string; const AFechaDesde, AFechaFin: TDateTime): boolean;
...
begin
  ...
  begin
    ...
    SQL.Add('        referencia_tc referencia, fecha_tc fecha, NVL(fecha_entrada_tc, fecha_tc) fecha_entrada, producto_tl producto,                                     ');
    ...
    SQL.Add('   and excluir_posei_tc = 0                                                                                                                                ');
...
begin
  ...
  begin
    ...
    SQL.Add('        CASE WHEN (cliente_pl[1,1] = ''0'' or cliente_pl IN (''RET'',''REA'',''REP'',''GAN'',''EG'',''BAA'')) THEN ''RETIRADA'' ELSE cliente_pl END cliente,     ');
    ...
    SQL.Add(' group by orden_pl, empresa_pl, producto_pl, envase_pl, peso_neto_e, cliente                                                                                     ');
    SQL.Add(' order by orden_pl, empresa_pl, producto_pl, envase_pl, peso_neto_e, cliente                                                                                     ');
  ...
  begin
    ...
    SQL.Add(' select empresa_sc, centro_salida_sc, n_albaran_sc, fecha_sc, empresa_fac_sc, serie_fac_sc, fecha_factura_sc, n_factura_sc,                             ');
    SQL.Add('        CASE WHEN (cliente_sal_sc[1,1] = ''0'' or cliente_sal_sc IN (''RET'',''REA'',''REP'',''GAN'',''EG'',''BAA'')) THEN ''RETIRADA'' ELSE cliente_sal_sc END cliente,    ');
    SQL.Add('        producto_sl, sum(cajas_sl) cajas_sl, sum(kilos_sl) kilos_sl, sum(kilos_posei_sl) kilos_posei_sl                                                 ');
    SQL.Add('  from frf_salidas_c                                                                                                                                    ');
    SQL.Add('  join frf_salidas_l on empresa_sl = empresa_sc and centro_salida_sl = centro_salida_sc and n_albaran_sl = n_albaran_sc and fecha_sl = fecha_sc         ');
    SQL.Add(' where empresa_sc = :empresa                                                                                                                            ');
    ...
    SQL.Add('   and producto_sl = :producto                                                                                                                          ');
    SQL.Add('   and fecha_sc between :fechaini and :fechafin                                      ');  //Fecha Entrada Transito + 15
//    SQL.Add('   and fecha_factura_sc between :fechaini_tran and :fechafin_tran                    ');  //Facturado en el mismo periodo
    SQL.Add(' group by empresa_sc, centro_salida_sc, n_albaran_sc, fecha_sc, empresa_fac_sc, serie_fac_sc, fecha_factura_sc, n_factura_sc, cliente, producto_sl      ');
...
begin
   ...
   // PROCESO
  // 1º Buscamos cargas directas en el Packing List (lo que realmente se ha vendido)
  // 2º Buscamos en albaranes de venta, ordenado por fecha para ir cogiendo kilos

  // Borrar Información
  if not BorrarPosei then
  begin
    ShowMessage(' ERROR. No se puede borrar datos de POSEI para el Año ' + IntToStr(iAnyo) + ' y Semestre ' + IntToStr(iSemestre) );
    Exit;
  end;
  ...
  begin
    ...
    bExisteProdLlanos := EsProdLlanos( QTransito.FieldByName('empresa').AsString, '1', QTransito.FieldByName('producto').AsString, dFechaIni, dFechaFin );
  ...
  begin
    ...
    begin
      ...
      rKilosRestoTrans := bRoundTo(QTransito.FieldByName('kilos').AsFloat - rKilosAsignados, 2);
...
procedure TDMCalculoPosei.BuscarAlbaran;
var rKilosDisponibles, rKilosParaAsignarAlb: real;
begin
  ...
  if dFechaFin >= (QTransito.FieldByName('fecha_entrada').AsDateTime + 15) then
    QAlbaranResto.ParamByName('fechafin').AsDateTime := QTransito.FieldByName('fecha_entrada').AsDateTime + 15
  else
    QAlbaranREsto.ParamByName('fechafin').AsDateTime := dFechaFin;
//  QAlbaranResto.ParamByName('fechaini_tran').AsDateTime := dFechaIni;
//  QAlbaranResto.ParamByName('fechafin_tran').AsDateTime := dFechaFin;

  QAlbaranResto.Open;

  while (not QAlbaranResto.Eof) and (bRoundTo(rKilosRestoTrans, 2) > 0) do
  begin
    ...
    rKilosDisponibles := bRoundTo(QAlbaranResto.FieldByName('kilos_posei_sl').AsFloat - rKilosPosei, 2);
    if rKilosDisponibles > 0 then
    begin
      if rKilosRestoTrans >= rKilosDisponibles then
        rKilosParaAsignar := rKilosDisponibles
      else
        rKilosParaAsignar := rKilosRestoTrans;

      //Aplico el porcentaje de kilos, para calcular el importe aplicado a kilos_sl
      rKilosParaAsignarAlb := bRoundTo((rKilosParaAsignar / QAlbaranResto.FieldByName('kilos_posei_sl').AsFloat) * QAlbaranResto.FieldByName('kilos_sl').AsFloat, 2);
      ...
      rPrecioAlbaran := CalcularPrecioAlbaran(QAlbaranResto.FieldByName('empresa_sc').AsString, QAlbaranResto.FieldByName('centro_salida_sc').AsString,
                                              ...
                                              QAlbaranResto.FieldByName('producto_sl').AsString, '', false);
      ...
      //Si es un producto que solo se cosecha en Tenerife, incluimos datos de RETIRADA en el calculo del POSEI
      if not bExisteProdLlanos then
      begin
        if ((QFactura.FieldByName('fecha_factura_fc').AsDateTime >= dFechaIni) and (QFactura.FieldByName('fecha_factura_fc').AsDateTime <= dFechaFin) or
           (QFactura.IsEmpty)) then
        begin
          try
            //Insertamos en Tabla Posei
            InsertarPoseiAlbaran_1 (rKilosParaAsignar, rKilosParaAsignarAlb);
            rKilosAsignados := bRoundTo(rKilosAsignados + QPosei.FieldByName('kilos_albaran_p').AsFloat, 2);
            rKilosRestoTrans := bRoundTo(rKilosRestoTrans - QPosei.FieldByName('kilos_albaran_p').AsFloat, 2);
            Inc(iLinea);
      ...
      begin
        if ((QFactura.FieldByName('fecha_factura_fc').AsDateTime >= dFechaIni) and (QFactura.FieldByName('fecha_factura_fc').AsDateTime <= dFechaFin) or
           (QFactura.IsEmpty)) and (QAlbaranResto.FieldByName('cliente').AsString <> 'RETIRADA') then
        begin
          try
            //Insertamos en Tabla Posei
            InsertarPoseiAlbaran_1 (rKilosParaAsignar, rKilosParaAsignarAlb);
  ...
  begin
    ...
    begin
      ...
      rPrecioAlbaran := CalcularPrecioAlbaran(kmtAlbPartidos.FieldByName('empresa_alb').AsString, kmtAlbPartidos.FieldByName('centro_alb').AsString,
                                            ...
                                            kmtAlbPartidos.FieldByName('producto_alb').AsString, '', false);
      //Buscar Factura Venta
      if QFactura.Active then
        QFactura.Close;
...
begin
  ...
  begin
    rPrecioAlbaran := CalcularPrecioAlbaran(QAlbaranDirecto.FieldByName('empresa_sc').AsString, QAlbaranDirecto.FieldByName('centro_salida_sc').AsString,
                                              ...
                                              QAlbaranDirecto.FieldByName('producto_sl').AsString, '', false);
    //Buscar Factura Venta
    if QFactura.Active then
      QFactura.Close;
    ...
    rKilosParaAsignar := QAlbaranResto.FieldByName('kilos_posei_sl').AsFloat - rKilosPosei;
{
    ...
    begin
      ...
      begin
        ...
        if QAlbaranResto.FieldByName('cajas_sl').AsFloat = 0 then
          kmtAlbPartidos.FieldByName('cajas_alb').AsFloat := rKilosParaAsignar / QAlbaranResto.FieldByName('kilos_posei_sl').AsFloat
        else
...
function TDMCalculoPosei.CalcularPrecioAlbaran (const AEmpresa, ACentro, AAlbaran, AFecha, AProducto, AEnvase: string; AEsPacking: boolean): real;
...
procedure TDMCalculoPosei.CargarDatosOficial;
begin
  // Cargamos tabla temporal
  if kmtOficial.Active then
    kmtOficial.Close;
  ...
  while not DMAuxDB.QAux.Eof do
  begin
    if DMAuxDB.QAux.FieldByName('cod_factura_fc').AsString <> '' then
    begin
      if kmtOficial.Locate( 'agrupacion;codigo_factura', VarArrayOf([DMAuxDB.QAux.FieldByName('codigo_a').AsString, DMAuxDB.QAux.FieldByName('cod_factura_fc').AsString]), []) then
        ModificarkmtOficial
      else
        InsertarkmtOficial
    end;
  ...
  kmtOficial.SortOn('anyo;semestre;producto;fecha_factura;n_factura',[]);
...
function TDMCalculoPosei.EsProdLlanos(const AEmpresa, ACentro, AProducto: String; const AFechaDesde, AFechaFin: TDateTime): boolean;
begin
  with QAux do
  try
    if Active then
...
var rKilosPacking, rKilosPosei, rKilosAAsignar, rKilosTotal, rKilosTotalAlb: real;
begin
  ...
  begin
    ...
    QAlbaran.Open;
    while (not QAlbaran.Eof) and (bRoundTo(rKilosPacking, 2) < QPacking.FieldByName('kilos_pl').AsFloat ) do//and (QPacking.FieldByName('kilos_pl').AsFloat <= QAlbaran.FieldByName('kilos_posei').AsFloat) do
    begin
      ...
      rKilosAAsignar := bRoundTo(QAlbaran.FieldByName('kilos_posei').AsFloat - rKilosPosei, 2);
      if rKilosAAsignar > 0 then
      begin
        ...
        //Aplico el porcentaje de kilos, para calcular el importe aplicado a kilos_sl
        rKilosTotalAlb := bRoundTo((rKilosTotal / QAlbaran.FieldByName('kilos_posei').AsFloat) * QAlbaran.FieldByName('kilos').AsFloat, 2);
        ...
        rPrecioAlbaran := CalcularPrecioAlbaran(QAlbaran.FieldByName('empresa_sc').AsString, QAlbaran.FieldByName('centro_salida_sc').AsString,
                                                ...
                                                QAlbaran.FieldByName('producto_sl').AsString, QAlbaran.FieldByName('envase_sl').AsString, true) ;
        //Buscar Factura Venta
        if QFactura.Active then
          QFactura.Close;
        ...
        begin
          ...
          begin
            try
              //Insertamos en Tabla Posei
              InsertarPosei (rKilosTotal, rKilosTotalAlb);
              rKilosAsignados := bRoundTo(rKilosAsignados + QPosei.FieldByName('kilos_albaran_p').AsFloat, 2);
              rKilosPacking := bRoundTo(rKilosPacking + QPosei.FieldByName('kilos_albaran_p').AsFloat, 2);
              Inc(iLinea);
        ...
        begin
          if ((QFactura.FieldByName('fecha_factura_fc').AsDateTime >= dFechaIni) and (QFactura.FieldByName('fecha_factura_fc').AsDateTime <= dFechaFin) or
             (QFactura.IsEmpty)) and (QPacking.FieldByName('cliente').AsString <> 'RETIRADA') then
          begin
            try
              //Insertamos en Tabla Posei
              InsertarPosei (rKilosTotal, rKilosTotalAlb);
...
begin
  ...
  kmtInforme.FieldByName('n_albaran').AsInteger := DMAuxDB.QAux.FieldByName('n_albaran_p').AsInteger;
...
procedure TDMCalculoPosei.InsertarkmtOficial;
begin
  kmtOficial.Insert;

  kmtOficial.FieldByName('definitivo').AsString := DMAuxDB.QAux.FieldByName('definitivo_p').AsString;
  kmtOficial.FieldByName('anyo').AsInteger := DMAuxDB.QAux.FieldByName('anyo_p').AsInteger;
  kmtOficial.FieldByName('semestre').AsInteger := DMAuxDB.QAux.FieldByName('semestre_p').AsInteger;
  kmtOficial.FieldByName('agrupacion').AsInteger := DMAuxDB.QAux.FieldByName('codigo_a').AsInteger;
  kmtOficial.FieldByName('des_agrupacion').AsString := DMAuxDB.QAux.FieldByName('nombre_a').AsString;
  kmtOficial.FieldByName('producto').AsString := DMAuxDB.QAux.FieldByName('producto_p').AsString;
  kmtOficial.FieldByName('des_producto').AsString := DMAuxDB.QAux.FieldByName('des_producto').AsString;
  kmtOficial.FieldByName('cod_empresa_fac').AsString := DMAuxDB.QAux.FieldByName('cod_empresa_fac_fc').AsString;
  kmtOficial.FieldByName('n_factura').AsString :=  DMAuxDB.QAux.FieldByName('n_factura_fc').AsString;
  kmtOficial.FieldByName('fecha_factura').AsString := DMAuxDB.QAux.FieldByName('fecha_factura_fc').AsString;
  kmtOficial.FieldByName('codigo_factura').AsString := DMAuxDB.QAux.FieldByName('cod_factura_fc').AsString;
  kmtOficial.FieldByName('codigo_pais').AsString := DMAuxDB.QAux.FieldByName('codigo_pais').AsString;
  kmtOficial.FieldByName('des_pais').AsString := DMAuxDB.QAux.FieldByName('des_pais').AsString;
  kmtOficial.FieldByName('des_provincia').AsString := DMAuxDB.QAux.FieldByName('des_provincia').AsString;
  kmtOficial.FieldByName('nif_cliente').AsString := DMAuxDB.QAux.FieldByName('nif_c').AsString;
  kmtOficial.FieldByName('cod_cliente').AsString := DMAuxDB.QAux.FieldByName('cliente_c').AsString;
  kmtOficial.FieldByName('des_cliente').AsString := DMAuxDB.QAux.FieldByName('nombre_c').AsString;
  kmtOficial.FieldByName('cajas').AsFloat := DMAuxDB.QAux.FieldByName('cajas_albaran_p').AsFloat;
  kmtOficial.FieldByName('kilos').AsFloat := DMAuxDB.QAux.FieldByName('kilos_albaran_p').AsFloat;
  kmtOficial.FieldByName('importe').AsFloat := DMAuxDB.QAux.FieldByName('importe_neto_albaran_p').AsFloat;

  kmtOficial.Post;

end;

procedure TDMCalculoPosei.InsertarPosei ( const AKilos, AKilosAlb: Real);
begin
  try
    QPosei.Insert;
    ...
    QPosei.FieldByName('articulo_albaran_p').AsString := QPacking.FieldByName('envase_pl').AsString;
{
    ...
    begin
      ...
      if QPacking.FieldByName('cliente').AsString <> 'RETIRADA' then
        QPosei.FieldByName('importe_neto_albaran_p').AsFloat := rPrecioAlbaran * AKilosAlb
      else
...
//    end;
    ...
    if QFactura.FieldByName('cod_factura_fc').AsString <> '' then
    begin
    ...
    begin
      if QPacking.FieldByName('cliente').AsString = 'RETIRADA' then
      begin
        QPosei.FieldByName('codigo_factura_p').AsString := 'RETIRADA';
        QPosei.FieldByName('fecha_factura_p').AsString :=  '';
        QPosei.FieldByName('importe_total_factura_p').AsString := '';
      end
      else
      begin
        QPosei.FieldByName('codigo_factura_p').AsString := 'PDTE. FACTURAR';
        QPosei.FieldByName('fecha_factura_p').AsString :=  '';
        QPosei.FieldByName('importe_total_factura_p').AsString := '';
      end;
    end;
...
procedure TDMCalculoPosei.InsertarPoseiAlbaran_1(const AKilosPosei, AKilosAlb: Real);
begin
  try
    QPosei.Insert;
    ...
    if QAlbaranResto.FieldByName('cajas_sl').AsInteger = 0 then
      QPosei.FieldByName('cajas_albaran_p').AsFloat := AKilosPosei / QAlbaranResto.FieldByName('kilos_posei_sl').AsFloat
    else
      QPosei.FieldByName('cajas_albaran_p').AsFloat := AKilosPosei / (QAlbaranResto.FieldByName('kilos_posei_sl').AsFloat / QAlbaranResto.FieldByName('cajas_sl').AsFloat);
    QPosei.FieldByName('kilos_albaran_p').AsFloat := AKilosPosei;
    if QAlbaranResto.FieldByName('cliente').AsString <> 'RETIRADA' then
      QPosei.FieldByName('importe_neto_albaran_p').AsFloat := rPrecioAlbaran * AKilosAlb
    else
      QPosei.FieldByName('importe_neto_albaran_p').AsFloat := 0;
...
procedure TDMCalculoPosei.ModificarkmtOficial;
begin
  kmtOficial.Edit;

  kmtOficial.FieldByName('cajas').AsFloat := kmtOficial.FieldByName('cajas').AsFloat + DMAuxDB.QAux.FieldByName('cajas_albaran_p').AsFloat;
  kmtOficial.FieldByName('kilos').AsFloat := kmtOficial.FieldByName('kilos').AsFloat + DMAuxDB.QAux.FieldByName('kilos_albaran_p').AsFloat;
  kmtOficial.FieldByName('importe').AsFloat := kmtOficial.FieldByName('importe').AsFloat + DMAuxDB.QAux.FieldByName('importe_neto_albaran_p').AsFloat;
