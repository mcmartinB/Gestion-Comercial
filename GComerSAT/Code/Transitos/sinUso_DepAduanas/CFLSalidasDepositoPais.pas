...
type
  TFLSalidasDepositoPais = class(TForm)
...
uses UDMAuxDB, Principal, CVariables, DPreview,
   ...
   CDLSalidasDepositoPais, CRLSalidasDepositoPais;

{$R *.DFM}

//                       ****  BOTONES  ****

function  TFLSalidasDepositoPais.LeerParametros: Boolean;
begin
  result:= False;
...
procedure TFLSalidasDepositoPais.BBAceptarClick(Sender: TObject);
var
  rKilos: real;
begin
  ...
  begin
    //Recoger datos
    if DLSalidasDepositoPais.AbrirQuery( sEmpresa, sCentro, sTransito, dFechaDesde, dFechaHasta, rKilos ) then
    begin
      //Previsualizar informe
      try
        Previsualizar( sEmpresa, sCentro, dFechaDesde, dFechaHasta, rKilos );
      finally
        DLSalidasDepositoPais.CerrarQuery;
      end;
    end
    else
    begin
...
procedure TFLSalidasDepositoPais.BBCancelarClick(Sender: TObject);
begin
...
procedure TFLSalidasDepositoPais.FormCreate(Sender: TObject);
begin
  FormType := tfOther;
  BHFormulario;
  ...
  DLSalidasDepositoPais:= TDLSalidasDepositoPais.Create( self );
end;

procedure TFLSalidasDepositoPais.FormActivate(Sender: TObject);
begin
  ActiveControl := eEmpresa;
end;

procedure TFLSalidasDepositoPais.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFLSalidasDepositoPais.FormClose(Sender: TObject;
...
begin
  ...
  FreeAndNil( DLSalidasDepositoPais );
  Action := caFree;
end;

//                      ****  FUNCIONES ESPECIFICAS  ****

procedure TFLSalidasDepositoPais.ADesplegarRejillaExecute(Sender: TObject);
begin
...
procedure TFLSalidasDepositoPais.PonNombre(Sender: TObject);
begin
    if (gRF <> nil) then
