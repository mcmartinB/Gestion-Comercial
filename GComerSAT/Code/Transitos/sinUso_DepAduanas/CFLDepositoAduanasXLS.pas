...
type
  TFLDepositoAduanasXLS = class(TForm)
...
uses UDMAuxDB, Principal, CVariables, DPreview,
   ...
   CDLDepositoAduanasXLS, CRLDepositoAduanasXLS;

{$R *.DFM}

//                       ****  BOTONES  ****

function  TFLDepositoAduanasXLS.LeerParametros: Boolean;
begin
  result:= False;
...
procedure TFLDepositoAduanasXLS.BBAceptarClick(Sender: TObject);
begin
  ...
  begin
    //Recoger datos
    if DLDepositoAduanasXLS.AbrirQuery( sEmpresa, sCentro, dFechaDesde, dFechaHasta ) then
    begin
      //Previsualizar informe
      try
        Previsualizar( self, sEmpresa, sCentro, dFechaDesde, dFechaHasta );
      finally
        DLDepositoAduanasXLS.CerrarQuery;
      end;
    end
    else
    begin
...
procedure TFLDepositoAduanasXLS.BBCancelarClick(Sender: TObject);
begin
...
procedure TFLDepositoAduanasXLS.FormCreate(Sender: TObject);
begin
  FormType := tfOther;
  BHFormulario;
  ...
  *)
  ...
  DLDepositoAduanasXLS:= TDLDepositoAduanasXLS.Create( self );
end;

procedure TFLDepositoAduanasXLS.FormActivate(Sender: TObject);
begin
  ActiveControl := eEmpresa;
end;

procedure TFLDepositoAduanasXLS.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFLDepositoAduanasXLS.FormClose(Sender: TObject;
...
begin
  ...
  FreeAndNil( DLDepositoAduanasXLS );
  Action := caFree;
end;

//                      ****  FUNCIONES ESPECIFICAS  ****

procedure TFLDepositoAduanasXLS.ADesplegarRejillaExecute(Sender: TObject);
begin
...
procedure TFLDepositoAduanasXLS.PonNombre(Sender: TObject);
begin
    if (gRF <> nil) then
