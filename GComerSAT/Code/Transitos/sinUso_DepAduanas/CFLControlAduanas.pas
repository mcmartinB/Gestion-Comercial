...
type
  TFLControlAduanas = class(TForm)
    ...
    cbbTransitos: TComboBox;
    lblSeleccionar: TLabel;
    lblSalidas: TLabel;
    cbbSalidas: TComboBox;
    ...
    procedure cbbTransitosChange(Sender: TObject);
...
uses UDMAuxDB, Principal, CVariables, DPreview,
   ...
   CDLControlAduanas, CRLControlAduanas;

{$R *.DFM}

//                       ****  BOTONES  ****

function  TFLControlAduanas.LeerParametros: Boolean;
begin
  result:= False;
...
procedure TFLControlAduanas.BBAceptarClick(Sender: TObject);
begin
  ...
  begin
    //Recoger datos
    if DLControlAduanas.AbrirQuery( sEmpresa, sCentro, sTransito, dFechaDesde, dFechaHasta,
                                    cbbTransitos.ItemIndex, cbbSalidas.ItemIndex ) then
    begin
      //Previsualizar informe
      try
        Previsualizar( sEmpresa, sCentro, dFechaDesde, dFechaHasta );
      finally
        DLControlAduanas.CerrarQuery;
      end;
    end
    else
    begin
...
procedure TFLControlAduanas.BBCancelarClick(Sender: TObject);
begin
...
procedure TFLControlAduanas.FormCreate(Sender: TObject);
begin
  FormType := tfOther;
  BHFormulario;
  ...
  DLControlAduanas:= TDLControlAduanas.Create( self );
end;

procedure TFLControlAduanas.FormActivate(Sender: TObject);
begin
  ActiveControl := eEmpresa;
end;

procedure TFLControlAduanas.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFLControlAduanas.FormClose(Sender: TObject;
...
begin
  ...
  FreeAndNil( DLControlAduanas );
  Action := caFree;
end;

//                      ****  FUNCIONES ESPECIFICAS  ****

procedure TFLControlAduanas.ADesplegarRejillaExecute(Sender: TObject);
begin
...
procedure TFLControlAduanas.PonNombre(Sender: TObject);
begin
    if (gRF <> nil) then
...
procedure TFLControlAduanas.cbbTransitosChange(Sender: TObject);
begin
  cbbSalidas.Enabled:= cbbTransitos.ItemIndex <> 1;
end;
