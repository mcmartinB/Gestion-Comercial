...
type
  TFLSalidasDDASinFactura = class(TForm)
    ...
    cbbPortePagado: TComboBox;
    chkFacturaComercial: TCheckBox;
    lbl1: TLabel;
    lblNombre2: TLabel;
    lbl2: TLabel;
...
uses UDMAuxDB, Principal, CVariables, DPreview, CAuxiliarDB, UDMBaseDatos,
     bDialogs, bMath, bTimeUtils, CDLSalidasDDASinFactura, CRLSalidasDDASinFactura;

{$R *.DFM}

//                       ****  BOTONES  ****

function  TFLSalidasDDASinFactura.LeerParametros: Boolean;
begin
  result:= False;
...
procedure TFLSalidasDDASinFactura.BBAceptarClick(Sender: TObject);
begin
  ...
  begin
    //Recoger datos
    if DLSalidasDDASinFactura.AbrirQuery( sEmpresa, sCentro, dFechaDesde, dFechaHasta,
                                cbbFactura.ItemIndex, cbbPortePagado.ItemIndex,
                                chkFacturaComercial.Checked   ) then
    begin
      //Previsualizar informe
      try
        Previsualizar( sEmpresa, sCentro, dFechaDesde, dFechaHasta,
                       cbbFactura.ItemIndex, cbbPortePagado.ItemIndex,
                       chkFacturaComercial.Checked );
      finally
        DLSalidasDDASinFactura.CerrarQuery;
      end;
    end
    else
    begin
...
procedure TFLSalidasDDASinFactura.BBCancelarClick(Sender: TObject);
begin
...
procedure TFLSalidasDDASinFactura.FormCreate(Sender: TObject);
begin
  FormType := tfOther;
  BHFormulario;
  ...
  *)
  ...
  DLSalidasDDASinFactura:= TDLSalidasDDASinFactura.Create( self );
end;

procedure TFLSalidasDDASinFactura.FormActivate(Sender: TObject);
begin
  ActiveControl := eEmpresa;
end;

procedure TFLSalidasDDASinFactura.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFLSalidasDDASinFactura.FormClose(Sender: TObject;
...
begin
  ...
  FreeAndNil( DLSalidasDDASinFactura );
  Action := caFree;
end;

//                      ****  FUNCIONES ESPECIFICAS  ****

procedure TFLSalidasDDASinFactura.ADesplegarRejillaExecute(Sender: TObject);
begin
...
procedure TFLSalidasDDASinFactura.PonNombre(Sender: TObject);
begin
    if (gRF <> nil) then
