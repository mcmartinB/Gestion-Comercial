...
type
  TFLDepositosAduanas = class(TForm)
...
uses UDMAuxDB, Principal, CVariables, DPreview,
   ...
   CDLDepositosAduanas, CRLDepositosAduanas;

{$R *.DFM}

//                       ****  BOTONES  ****

function  TFLDepositosAduanas.LeerParametros: Boolean;
begin
  result:= False;
...
procedure TFLDepositosAduanas.BBAceptarClick(Sender: TObject);
begin
  ...
  begin
    //Recoger datos
    if DLDepositosAduanas.AbrirQuery( sEmpresa, sCentro, dFechaDesde, dFechaHasta, rgDatos.ItemIndex ) then
    begin
      //Previsualizar informe
      try
        Previsualizar( self, sEmpresa, sCentro, dFechaDesde, dFechaHasta );
      finally
        DLDepositosAduanas.CerrarQuery;
      end;
    end
    else
    begin
...
procedure TFLDepositosAduanas.BBCancelarClick(Sender: TObject);
begin
...
procedure TFLDepositosAduanas.FormCreate(Sender: TObject);
begin
  FormType := tfOther;
  BHFormulario;
  ...
  *)
  ...
  DLDepositosAduanas:= TDLDepositosAduanas.Create( self );
end;

procedure TFLDepositosAduanas.FormActivate(Sender: TObject);
begin
  ActiveControl := eEmpresa;
end;

procedure TFLDepositosAduanas.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFLDepositosAduanas.FormClose(Sender: TObject;
...
begin
  ...
  FreeAndNil( DLDepositosAduanas );
  Action := caFree;
end;

//                      ****  FUNCIONES ESPECIFICAS  ****

procedure TFLDepositosAduanas.ADesplegarRejillaExecute(Sender: TObject);
begin
...
procedure TFLDepositosAduanas.PonNombre(Sender: TObject);
begin
    if (gRF <> nil) then
