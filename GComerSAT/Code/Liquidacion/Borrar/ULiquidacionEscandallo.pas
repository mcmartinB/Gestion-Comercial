...
procedure LiquidacionPorEscandallo(const AEmpresa, ACentro, AProducto, ACosechero,
  AFechaIni, AFechaFin, ACosteComercial,
  ACosteProduccion, ACosteAdministrativo: string;
  const AAbonos, ADefinitiva: boolean);
...
uses CVariables, UDMBaseDatos, CGestionPrincipal, bSQLUtils, DError,
  UDMAuxDB, DPreview, CReportes, bMath, RLiquidacionCosecheros,
  bTimeUtils, ULiquidacionComun, RResumenLiqCos, UCosteEnvasadoComunEx;
...
begin
  try
    ULiquidacionComun.CrearTablasTemporales;
    Previsualizar(AEmpresa, ACentro, AProducto, ACosechero, AFechaIni, AFechaFin,
      ACosteComercial, ACosteProduccion, ACosteAdministrativo, AAbonos, ADefinitiva);
  finally
    BorrarTemporal('tmp_fob');
    ULiquidacionComun.DestruirTablasTemporales;
  end;
end;
...
procedure MarcarComoDefinitiva( const AEmpresa, ACentro, AProducto, AFechaIni, AFechaFin, ACosechero,
                                ...
                                const AAbonos, ADefiniva: boolean;
...
begin
  ...
  begin
    ...
    if AAbonos then
      ...
      ParamByName('abonos').AsInteger:= 0;
...
begin
  try
    ULiquidacionComun.CalcularFechaInicio(AEmpresa, ACentro, AProducto, ACosechero, AFechaIni, AFechaFin, fechainicio, fechafin);
    ...
    begin
      ...
      FreeAndNil(QRLiquidacionCosecheros);
      Exit;
    end;
  ...
  *)
    ...
    try
      NuevoCalcularFOB(gsCodigo, AEmpresa, ACentro, AProducto, AFechaIni, AFechaFin, AAbonos);
  ...
  try
    ULiquidacionComun.PreparaLiquidacion(AEmpresa, ACentro, AProducto, ACosechero, AFechaIni,
      AFechaFin, ACosteComercial, ACosteProduccion, ACosteAdministrativo);
    QRLiquidacionCosecheros.UnSoloCosechero := Trim(ACosechero) <> '';
    UCosteEnvasadoComunEx.CrearTablaTemporal;
    Preview(QRLiquidacionCosecheros);
    ...
    try
      QRResumenLiqCos:= TQRResumenLiqCos.Create( nil );
      PonLogoGrupoBonnysa(QRResumenLiqCos, AEmpresa );
      QRResumenLiqCos.lblCentro.Caption:= ACentro;
      QRResumenLiqCos.lblNomCentro.Caption:= DesCentro( AEmpresa, ACentro );
      QRResumenLiqCos.lblProducto.Caption:= AProducto;
      QRResumenLiqCos.lblNOmProducto.Caption:= DesProducto( AEmpresa, AProducto );
      QRResumenLiqCos.lblPeriodo.Caption:= 'Desde el ' + AFechaIni + ' al ' + AFechaFin;
      Preview(QRResumenLiqCos);
    except
      FreeAndNil( QRResumenLiqCos );
      Raise;
    end;
  except
    FreeAndNil( QRLiquidacionCosecheros );
