...
type
  TFLDetalleLiquidacionFruta = class(TForm)
...
uses CVariables, CAuxiliarDB, CGestionPrincipal, bSQLUtils,
  UDMAuxDB, bMath,  bDialogs, RDetalleLiquidacionFruta, UDMBaseDatos;


{$R *.DFM}

procedure TFLDetalleLiquidacionFruta.BBCancelarClick(Sender: TObject);
begin
...
function TFLDetalleLiquidacionFruta.ValidarParametros: Boolean;
var
  aux: string;
...
procedure TFLDetalleLiquidacionFruta.BBAceptarClick(Sender: TObject);
begin
  ...
  begin
     if not DetalleLiquidacionFruta( self, sEmpresa, sCentro, sProducto, sCosechero, dFechaIni, dFechaFin ) then
     begin
...
procedure TFLDetalleLiquidacionFruta.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  BEMensajes('');
  gRF := nil;
  gCF := nil;
...
procedure TFLDetalleLiquidacionFruta.FormCreate(Sender: TObject);
begin
  FormType := tfOther;
  BHFormulario;
...
procedure TFLDetalleLiquidacionFruta.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFLDetalleLiquidacionFruta.ADesplegarRejillaExecute(Sender: TObject);
begin
...
procedure TFLDetalleLiquidacionFruta.PonNombre(Sender: TObject);
begin
    if (gRF <> nil) then
...
procedure TFLDetalleLiquidacionFruta.EProductoChange(Sender: TObject);
begin
  PonNombre(Sender);
end;
