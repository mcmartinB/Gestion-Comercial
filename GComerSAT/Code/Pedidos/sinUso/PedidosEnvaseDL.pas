...
begin
  ...
  begin
    ...
    begin
      ...
      QKilosAlbaranEx.ParamByName('envase').AsString:= AEnvase;
  ...
  begin
    ...
    begin
      ...
      QUnidadesAlbaranEx.ParamByName('envase').AsString:= AEnvase;
  ...
  begin
    ...
    begin
      ...
      QCajasAlbaranEx.ParamByName('envase').AsString:= AEnvase;
...
begin
  ...
  if bProducto and ( ( AProducto <> 'E' ) and ( AProducto <> 'T' ) ) then
  begin
...
begin
  ...
  tTemporal.FieldDefs.Add('envase', ftString, 3, False);
  ...
  tListado.FieldDefs.Add('envase', ftString, 3, False);
  ...
  QKilosAlbaranEx.SQL.Add( 'select sum(kilos_sl) unidades');
  ...
  QKilosAlbaranEx.SQL.Add( 'and envase_sl = :envase ');
  QKilosAlbaranEx.Prepare;

  QCajasAlbaranEx.SQL.Clear;
  QCajasAlbaranEx.SQL.Add( 'select sum(cajas_sl) unidades');
  ...
  QCajasAlbaranEx.SQL.Add( 'and envase_sl = :envase ');
  QCajasAlbaranEx.Prepare;

  QUnidadesAlbaranEx.SQL.Clear;
  QUnidadesAlbaranEx.SQL.Add( 'select sum(cajas_sl * unidades_caja_sl ) unidades ');
  ...
  QUnidadesAlbaranEx.SQL.Add( 'and envase_sl = :envase ');
...
begin
  ...
  sAux.Add('   case when producto_pdd = ''E'' then ''T'' else producto_pdd  end producto_pdd,  ');
  ...
  begin
    if ( AProducto = 'T' ) or ( AProducto = 'E' ) then
       sAux.Add(' and ( producto_pdd = ''T'' or producto_pdd = ''E'' ) ')
    else
...
begin
  sAux:= TStringList.Create;
  sAux.Add(' SELECT case when producto_sl = ''E'' then ''T'' else producto_sl end producto, ');
  ...
  begin
    if ( AProducto = 'T' ) or ( AProducto = 'E' ) then
      sAux.Add(' and ( producto_sl = ''T'' or producto_sl = ''E'' ) ')
    else
