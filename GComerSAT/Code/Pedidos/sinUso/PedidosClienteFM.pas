...
type
  TFMPedidosCliente = class(TMaestroDetalle)
    PMaestro: TPanel;
...
uses PedidosClienteQM, PedidosClienteDM, CGestionPrincipal, UDMBaseDatos, UDMAuxDB, CAuxiliarDB,
     Principal,DPreview, bSQLUtils, bNumericUtils, CReportes, UDMConfig;

{$R *.DFM}

procedure TFMPedidosCliente.AbrirTablas;
begin
     // Abrir tablas/Querys
...
procedure TFMPedidosCliente.CerrarTablas;
begin
  BorrarTemporal('temporal');

  CloseAuxQuerys;
  bnCloseQuerys([DMPedidosCliente.TDetalle, DMPedidosCliente.QMaestro]);
end;

//************************** CREAMOS EL FORMULARIO ************************

procedure TFMPedidosCliente.FormCreate(Sender: TObject);
begin
  CrearModuloDeDatos;
  ...
  //Fuente de datos
 {+}DataSetMaestro := DMPedidosCliente.QMaestro;
...
procedure TFMPedidosCliente.FormActivate(Sender: TObject);
begin
...
procedure TFMPedidosCliente.FormDeactivate(Sender: TObject);
begin
  gRF := nil;
  gCF := nil;
end;

procedure TFMPedidosCliente.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFMPedidosCliente.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFMPedidosCliente.Filtro;
var Flag: Boolean;
  i: Integer;
begin
  where := ''; Flag := false;
...
procedure TFMPedidosCliente.AnyadirRegistro;
var Flag: Boolean;
  i: Integer;
begin
  Flag := false;
...
procedure TFMPedidosCliente.ValidarEntradaMaestro;
var i: Integer;
begin
  for i := 0 to ListaComponentes.Count - 1 do
  begin
  ...
  if not DMPedidosCliente.DirSumValida( empresa_pdc.Text, cliente_pdc.Text, dir_suministro_pdc.Text ) then
  begin
  ...
  begin
    DSMaestro.DataSet.FieldByName('pedido_pdc').AsInteger:=
      DMPedidosCliente.NuevaPedido( empresa_pdc.Text, centro_pdc.Text );
  end;
end;

procedure TFMPedidosCliente.Previsualizar;
var
  QMPedidosCliente: TQMPedidosCliente;
begin
  DMPedidosCliente.QueryListado( empresa_pdc.Text, centro_pdc.Text, cliente_pdc.Text, '', ref_pedido_pdc.Text,
                          StrToDate( fecha_pdc.Text), StrToDate( fecha_pdc.Text) );
  DMPedidosCliente.QListado.Open;
  DMPedidosCliente.QNotas.Open;
  if not DMPedidosCliente.QListado.IsEmpty then
  begin
    QMPedidosCliente := TQMPedidosCliente.Create(Application);
    try
      PonLogoGrupoBonnysa(QMPedidosCliente, empresa_pdc.Text);
      QMPedidosCliente.lblPeriodo.Caption:= '';
      QMPedidosCliente.lblProducto.Caption:= '';
      QMPedidosCliente.ReportTitle:= 'PEDIDOS';
      QMPedidosCliente.bNotas:= True;

      Preview(QMPedidosCliente);
    except
      QMPedidosCliente.Free;
    end;
  end;
  DMPedidosCliente.QNotas.Close;
  DMPedidosCliente.QListado.Close;
end;
...
procedure TFMPedidosCliente.ARejillaFlotanteExecute(Sender: TObject);
begin
...
procedure TFMPedidosCliente.AntesDeInsertar;
begin
...
procedure TFMPedidosCliente.AntesDeLocalizar;
begin
...
procedure TFMPedidosCliente.AntesDeModificar;
var i: Integer;
begin
  for i := 0 to ListaComponentes.Count - 1 do
  begin
...
procedure TFMPedidosCliente.AntesDeVisualizar;
var i: Integer;
begin
  for i := 0 to ListaComponentes.Count - 1 do
  begin
...
procedure TFMPedidosCliente.VerDetalle;
var i: integer;
begin
  LineasObligadas := false;
...
procedure TFMPedidosCliente.EditarDetalle;
var i: integer;
begin
...
procedure TFMPedidosCliente.ValidarEntradaDetalle;
var i: Integer;
begin
  for i := 0 to ListaDetalle.Count - 1 do
  begin
...
procedure TFMPedidosCliente.RellenaClaveDetalle;
begin
  ...
  DSDetalle.DataSet.FieldByName('linea_pdd').AsInteger:=
    DMPedidosCliente.NuevaLinea(DSMaestro.DataSet.FieldByName('empresa_pdc').AsString,
    DSMaestro.DataSet.FieldByName('centro_pdc').AsString,
    DSMaestro.DataSet.FieldByName('pedido_pdc').AsInteger);
end;
...
procedure TFMPedidosCliente.RequiredTime(Sender: TObject;
  var isTime: Boolean);
begin
  isTime := false;
...
procedure TFMPedidosCliente.DetalleAltas;
var
   //aux1,aux2:String;
  aux3: TEstadoDetalle;
begin
...
procedure TFMPedidosCliente.empresa_pdcChange(Sender: TObject);
begin
...
procedure TFMPedidosCliente.centro_pdcChange(Sender: TObject);
begin
  STCentro.Caption := desCentro(empresa_pdc.Text,centro_pdc.Text);
end;

procedure TFMPedidosCliente.cliente_pdcChange(Sender: TObject);
begin
  STCliente.Caption := desCliente(empresa_pdc.Text,cliente_pdc.Text);
  STSuministro.Caption := desSuministroEx(empresa_pdc.Text,cliente_pdc.Text,dir_suministro_pdc.Text);
end;

procedure TFMPedidosCliente.dir_suministro_pdcChange(Sender: TObject);
begin
  STSuministro.Caption := desSuministroEx(empresa_pdc.Text,cliente_pdc.Text,dir_suministro_pdc.Text);
end;

procedure TFMPedidosCliente.envase_pddChange(Sender: TObject);
var
  sUnidad: String;
begin
  ...
  begin
    sUnidad:= DMPedidosCliente.Unidad( empresa_pdc.Text, producto_pdd.Text, envase_pdd.Text, cliente_pdc.Text );
    if sUnidad <> '' then
    begin
...
procedure TFMPedidosCliente.producto_pddChange(Sender: TObject);
begin
  STProducto.Caption := desProducto(empresa_pdc.Text,producto_pdd.Text);
end;

procedure TFMPedidosCliente.cbxUnidadesChange(Sender: TObject);
begin
...
procedure TFMPedidosCliente.unidad_pddChange(Sender: TObject);
begin
...
procedure TFMPedidosCliente.aux_anulado_pdcChange(Sender: TObject);
begin
  motivo_anulacion_pdc.Enabled:= ( TEdit( Sender ).Text <> '0' );
end;

procedure TFMPedidosCliente.anulado_pdcClick(Sender: TObject);
begin
