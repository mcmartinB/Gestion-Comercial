...
type
  TDBalanceCompra = class(TDataModule)
...
var
  DBalanceCompra: TDBalanceCompra;
...
function ObtenerDatos( const AOwner: TComponent;  const Empresa, ACentro, AEntrega: String ): boolean;
begin
  DBalanceCompra:= TDBalanceCompra.Create( AOwner );
  with DBalanceCompra do
  begin
    ...
    if result then
    begin
      QGastosEntregas.ParamByName('entrega').AsString:= AEntrega;
      QGastosEntregas.Open;
      QSalidasEntregas.ParamByName('entrega').AsString:= AEntrega;
...
procedure CerrarDatos;
begin
  FreeAndNil( DBalanceCompra );
end;

procedure TDBalanceCompra.ValorarSalidas;
begin
  ...
  mtSalidasValoradas.SortOn('empresa;cliente;producto;envase;fecha',[]);
end;

function TDBalanceCompra.GetPrecioFob: Real;
var
...
procedure TDBalanceCompra.DataModuleCreate(Sender: TObject);
begin
  with QEntregas do
  begin
    SQL.Clear;
  ...
  begin
    ...
    SQL.Add(' where codigo_ec = :entrega ');
    SQL.Add(' union ');
    SQL.Add(' select codigo_eca, empresa_eca, cliente_eca, producto_eca, envase_eca, fecha_eca, porcentaje_eca  ');
  ...
  mtSalidasValoradas.FieldDefs.Add('envase', ftString, 9, False);
  ...
  mtSalidasValoradas.IndexFieldNames:= 'empresa;cliente;producto;envase;fecha';
...
procedure TDBalanceCompra.DataModuleDestroy(Sender: TObject);
begin
  ClientDataSet.Close;
...
procedure TDBalanceCompra.PrepareQGastos( const ANo036: boolean );
begin
...
function TDBalanceCompra.ObtenerDatos(const AEmpresa, ACentroOrigen, ACentroDestino,
...
procedure TDBalanceCompra.RellenaDatosFaltantes( const AEmpresa: string; const ACategoria: string;
                                 const AGasto, ANo036, ACosteEnvase, ASecciones: boolean );
var
...
procedure TDBalanceCompra.GetKilosAlbaran(var AKilosTotal, AKilosTransito, AKilosProdTotal, AKilosProdTransito: Real;
      const AEmpresa, ACentro: string; const AAlbaran: Integer; const AFecha: TDateTime; const AProducto: string);
begin
  with QKilos do
  begin
...
procedure TDBalanceCompra.GetGastosAlbaran(
...
procedure TDBalanceCompra.GetCostesEnvasado( const AEmpresa, ACentro, AEnvase: string;
