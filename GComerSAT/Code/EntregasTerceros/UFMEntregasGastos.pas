...
type
  TFMEntregasGastos = class(TForm)
    ...
    procedure producto_geChange(Sender: TObject);
...
begin
  ...
  MDEntregas.QGastosEntregas.FieldByname('pagado_ge').AsInteger := 0;
  producto_ge.SetFocus;
  producto_ge.Change;
  tipo_ge.Change;
end;
...
procedure TFMEntregasGastos.producto_geChange(Sender: TObject);
begin
  stProducto.Caption:= desProducto(MDEntregas.QEntregasC.FieldByname('empresa_ec').AsString, producto_ge.Text );
  if stProducto.Caption = '' then
...
begin
  ...
  if producto_ge.Text <> '' then
  begin
    DMBaseDatos.QDespegables.SQL.Add('and producto_ge = :producto ');
  end;
  DMBaseDatos.QDespegables.SQL.Add(' ) ');
  ...
  if producto_ge.Text <> '' then
  begin
    DMBaseDatos.QDespegables.ParamByName('producto').AsString:= producto_ge.Text;
  end;
