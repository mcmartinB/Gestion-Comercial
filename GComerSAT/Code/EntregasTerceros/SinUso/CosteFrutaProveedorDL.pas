...
type
  ...
  private
    ...
    procedure ObtenerGastosEx( const AEntrega, AProducto, AVariedad, ACalibre, ATipoGasto: string; const AMasImporte: Boolean;
                               var AKilosCal, AKilosTot, AImporteLinea, AFacturas, AGastos: real );
    procedure RellenarTablaDeGastos( const ATipoGasto: string; const AMasImporte: Boolean; AMensaje: TLabel );

  public
    { Public declarations }
    function DatosQueryFrutaProveedor( const AEmpresa, ACentro, AProducto, AProveedor, AEntrega, ATipoGasto: string;
    ...
    procedure ObtenerGastos( const AEntrega, AProducto, AVariedad, ACalibre, ATipoGasto: string; const AMasImporte: Boolean;
                             var AKilosCal, AKilosTot, AImporteLinea, AFacturas, AGastos: real );
  end;
...
(*****************)
  ...
  begin
    SQL.Clear;
    SQL.Add(' select sum( case when tipo_ge = ''110'' then importe_ge else 0 end ) facturas,  ');
    SQL.Add('        sum( case when tipo_ge = :tipo then importe_ge else 0 end ) gastos,  ');
    SQL.Add('        sum( case when tipo_ge <> :tipo then importe_ge else 0 end ) resto_gastos  ');
...
begin
  ...
  begin
    ...
    SQL.Add('          where p.empresa_p = :empresa ');
    SQL.Add('            AND p.proveedor_p = ec.proveedor_ec ');
    SQL.Add('        ) nombre_p, ');
    SQL.Add('        ( select descripcion_p ');
    SQL.Add('          from frf_productos p ');
    SQL.Add('          where p.empresa_p = :empresa ');
    SQL.Add('          AND p.producto_p = el.producto_el ');
    ...
    SQL.Add('          where pp.empresa_pp = :empresa ');
    SQL.Add('          AND pp.proveedor_pp = el.proveedor_el ');
...
function TDLCosteFrutaProveedor.DatosQueryFrutaProveedor( const AEmpresa, ACentro, AProducto, AProveedor, AEntrega, ATipoGasto: string;
                                                          const AMasImporte: Boolean; const AFechaDesde, AFechaHasta: string;
                                                          const AAgruparVariedad, AAgruparCalibre: Boolean;  AMensaje: TLabel ): boolean;
begin
  ...
  begin
    ...
    RellenarTablaDeGastos( ATipoGasto, AMasImporte, AMensaje );

    result:= not IsEmpty;
  end;
end;

procedure TDLCosteFrutaProveedor.RellenarTablaDeGastos( const ATipoGasto: string; const AMasImporte: Boolean; AMensaje: TLabel );
var
  ...
  begin
    ...
    begin
      ...
      ObtenerGastos( FieldByname('entrega').AsString, FieldByname('producto').AsString, FieldByname('variedad').AsString,
        FieldByname('calibre').AsString, ATipoGasto, AMasImporte, rKilosCal, rKilosTot, rImporteLinea, rFacturas, rGastos );
...
procedure TDLCosteFrutaProveedor.ObtenerGastos( const AEntrega, AProducto, AVariedad, ACalibre, ATipoGasto: string; const AMasImporte: Boolean;
                                             var AKilosCal, AKilosTot, AImporteLinea, AFacturas, AGastos: real );
begin
  ...
  begin
    ObtenerGastosEx( AEntrega, AProducto, AVariedad, ACalibre, ATipoGasto, AMasImporte, AKilosCal, AKilosTot, AImporteLinea, AFacturas, AGastos );
    with TMemGastos do
    begin
      Insert;
...
procedure TDLCosteFrutaProveedor.ObtenerGastosEx( const AEntrega, AProducto, AVariedad, ACalibre, ATipoGasto: string; const AMasImporte: Boolean;
                                               var AKilosCal, AKilosTot, AImporteLinea, AFacturas, AGastos: real );
begin
  with QGastos do
  begin
    ...
    ParamByName('producto').AsString:= AProducto;
    if  ATipoGasto <> '' then
      ParamByName('tipo').AsString:= ATipoGasto
    else
      ParamByName('tipo').AsString:= '110';
    Open;
    ...
    begin
      AFacturas:= FieldByname('facturas').AsFloat;
      AGastos:= FieldByname('resto_gastos').AsFloat;
    end;
    Close;
  end;
  ...
  begin
    ...
    if ( ATipoGasto = '' ) or AMasImporte then
    begin
