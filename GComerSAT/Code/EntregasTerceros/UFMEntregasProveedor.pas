...
procedure TFMEntregasProveedor.PintaDestrioCalidad;
var
  rAux: Real;
begin
...
begin
  ...
  begin
    DSMaestro.DataSet.FieldByName('codigo_ec').AsString:=
      GetCodigoEntrega( empresa_ec.Text, fecha_origen_ec.Text );
  ...
  if CDBEntregas.ExisteAlbaranProveedor( empresa_ec.Text, proveedor_ec.Text, almacen_ec.Text, fecha_carga_ec.Text, albaran_ec.Text ) then
  begin
...
begin
      ...
      begin
        ...
        begin
          sAux:= sAux + '* Para el centro ' + QuotedStr( centro_llegada_ec.Text ) + ' el dia ' + QuotedStr( fecha_eca.Text ) +
                 ' hay salidas con los artículos/productos ' + QuotedStr( FieldByname('envase_sl').AsString + '/' + FieldByname('producto_sl').AsString  );
          Next;
          while not Eof do
          begin
        ...
        raise Exception.Create('No existe salidas para el centro ' + QuotedStr( centro_llegada_ec.Text ) + ' del dia ' + QuotedStr(fecha_eca.Text) +
                    ' del artículo/producto ' + envase_eca.Text + '/' + producto_eca.Text +
  ...
  begin
    ...
    begin
      ShowMEssage('Falta el código del operador/envase o es incorrecto.');
      Abort;
    end;
...
procedure TFMEntregasProveedor.almacen_ecChange(Sender: TObject);
begin
  lblAlmacen.Caption := desProveedorAlmacen(empresa_ec.Text, proveedor_ec.Text, almacen_ec.Text);
end;
...
procedure TFMEntregasProveedor.btnFecha_CargaClick(Sender: TObject);
begin
...
procedure TFMEntregasProveedor.producto_elChange(Sender: TObject);
begin
...
procedure TFMEntregasProveedor.ssEnvaseAntesEjecutar(Sender: TObject);
begin
    ssEnvase.SQLAdi := '';
...
procedure TFMEntregasProveedor.btnGastosClick(Sender: TObject);
begin
...
procedure TFMEntregasProveedor.producto_elEnter(Sender: TObject);
begin
...
procedure TFMEntregasProveedor.codigo_ecChange(Sender: TObject);
begin
  if pMantenimientoGastos.Visible then
  begin
...
procedure TFMEntregasProveedor.PageControlChange(Sender: TObject);
begin
...
procedure TFMEntregasProveedor.cosechero_erChange(Sender: TObject);
begin
  if PDetalle2.Visible then
  begin
    lblCosechero.Caption := desCosechero(empresa_ec.Text, cosechero_er.Text);
...
procedure TFMEntregasProveedor.plantacion_erChange(Sender: TObject);
begin
  if PDetalle2.Visible then
...
procedure TFMEntregasProveedor.btnClienteClick(Sender: TObject);
begin
  if cliente_eca.Focused then
    DespliegaRejilla( btnCliente, [empresa_ec.text, cliente_eca.Text] );
end;

procedure TFMEntregasProveedor.btnProducto_ecaClick(Sender: TObject);
begin
  if producto_eca.Focused then
    DespliegaRejilla( btnProducto, [empresa_ec.text, producto_eca.Text] );
end;

procedure TFMEntregasProveedor.btnFechaCalidadClick(Sender: TObject);
begin
  if fecha_eca.Focused then
    DespliegaCalendario( btnFechaCalidad );
end;

procedure TFMEntregasProveedor.cliente_ecaChange(Sender: TObject);
begin
  stCliente.Caption:= desCliente( cliente_eca.Text );
end;

procedure TFMEntregasProveedor.envase_ecaChange(Sender: TObject);
begin
  stenvase.Caption:= desEnvase( empresa_ec.Text, envase_eca.Text );
end;

procedure TFMEntregasProveedor.envase_ecaExit(Sender: TObject);
begin
...
procedure TFMEntregasProveedor.producto_ecaChange(Sender: TObject);
begin
  stProducto.Caption:= desProductoAlta( producto_eca.Text );
end;

procedure TFMEntregasProveedor.btnBalanceClick(Sender: TObject);
begin
  if Trim(codigo_ec.Text) <> '' then
...
procedure TFMEntregasProveedor.DSDetalle3StateChange(Sender: TObject);
begin
...
procedure TFMEntregasProveedor.btnOperadorClick(Sender: TObject);
begin
  if cod_operador_em.Focused then
    DespliegaRejilla( btnOperador, [] );
end;

procedure TFMEntregasProveedor.btnEnvaseOperadorClick(Sender: TObject);
begin
  if cod_producto_em.Focused then
    DespliegaRejilla( btnEnvaseOperador, [cod_operador_em.text] );
end;

procedure TFMEntregasProveedor.cod_operador_producto_emChange(Sender: TObject);
begin
