...
procedure PreparaQuerysSobrepeso( const AEmpresa: string );
begin
  QTargetCab.SQL.Clear;
  QTargetCab.SQL.Add('select * ');
  QTargetCab.SQL.Add('from frf_env_sobrepeso');
  QTargetCab.SQL.Add('where empresa_es = ''###'' ');

  QSourceCab.SQL.Clear;
  QSourceCab.SQL.Add('select * ');
  QSourceCab.SQL.Add('from frf_env_sobrepeso');
  if AEmpresa <> '' then
  begin
    QSourceCab.SQL.Add('where empresa_es = :empresa');
    QSourceCab.SQL.Add('  and anyo_es = :anyo');
    QSourceCab.SQL.Add('  and mes_es = :mes');
  end
  else
  begin
    QSourceCab.SQL.Add('where anyo_es = :anyo');
    QSourceCab.SQL.Add('  and mes_es = :mes');
  end;
end;
...
begin
  case ATipo of
    kSincroSobrepesos: PreparaQuerysSobrepeso( AEmpresa );
    kSincroCosteEnvase: PreparaQuerysCosteEnvase( AEmpresa );
