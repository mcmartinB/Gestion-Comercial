...
type
  ...
  private
    ...
    function Grabar(filtro, campo: string; valor: real; fecha: TDate): boolean;
...
begin
  ...
  begin
    ...
    begin
      SQL.Clear;
      SQL.Add(' select unique envase_sl from frf_salidas_l ');
      SQL.Add(' where empresa_sl = :empresa ');
      //if Trim(productos) <> '' then
      //  SQL.Add(' AND producto_sl MATCHES ' + QuotedStr('[' + productos + ']'));
  ...
  case TComponent(Sender).Tag of
    ...
    begin
      if eProducto.Text = '' then
        ...
        txtProducto.Caption := desproducto(Eempresa.Text, eProducto.Text);
    end;
  end;
end;
...
var ini, tot,i : integer;
    a,m,d,a1,m1,d1: Word;
    f1,f2: TDateTime;
    aux: string;
begin
     result:=True;
     //Sacamos la semana inicial y final del rango de fechas seleccionado
     f1:=StrToDate(MEDesde.Text);
     DecodeDate(f1,a,m,d);
     d:=DayOfWeek(f1);
     d:= d - 1;
     if d = 0 then d:= 7;
     ...
     f2:=StrToDate(MEHasta.Text);
     DecodeDate(f2,a1,m1,d1);
     d1:=DayOfWeek(f2);
     d1:= d1 - 1;
     if d1 = 0 then d1:= 7;

     //Calculamos el numero de semanas que hay en el rango de fechas seleccionado
     tot:=0;
     //Si el primer dia es lunes no pasa nada
     if d <> 1 then
     begin
       tot:= tot +1;
       f1:= f1 + (8 - d);
     end;
     //Si el ultimo dia es domingo no pasa nada
     if d1 <> 7 then
     begin
       tot:= tot + 1;
       f2:= f2 - d1;
     ...
     //Numero de semanas
     tot:=tot + trunc((f2-f1)/7);

     //Comprobar que la semana corresponde al año
     if (ini = 1) and (m = 12)  then
       begin
        Inc(a);
       end;
     if (ini > 51) and (m = 1)then
       begin
         Dec(a);
       end;


    //insertamos en la tabla temporal en el campo semana todas las semanas que
     //componen el rango de fechas seleccionado con el formato aaaass
     for i:=0 to tot do
      begin
        if ini > 52 then
         begin
          inc(a);//incrementamos en uno el año, puesto que si la semana es mayor que 52 cambiamos de año
          ini:=1;
         end;
        aux:=IntToStr(ini);
        if ini < 10 then
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('SELECT SUM(KILOS_SL) KILOS, YearAndWeek(FECHA_SC) FECHA ' + //YEARANDWEEK(FECHA_SC) SEMANA '+
      ' FROM FRF_SALIDAS_C, FRF_SALIDAS_L ' +
      ' WHERE EMPRESA_SC = ' + QuotedStr(EEmpresa.Text) +
      ' AND   CLIENTE_SAL_SC = ' + QuotedStr(ECliente.Text) +
                //' AND CLIENTE_SAL_SC NOT IN("MER","AL","CF","D","DW","ED","FG","FN","GL","HB","HBA","HSP","IB","LE","PL","UL","WS") AND Categoria_sl = "1" ' +
      ' AND   FECHA_SC BETWEEN ' + QuotedStr(MEDesde.Text) + ' AND ' + QuotedStr(MEHasta.Text));
    if envase <> '' then SQL.Add(' AND   ENVASE_SL ' + SQLEqualS(envase));
    ...
    if Trim(eproducto.Text) <> '' then
      SQL.Add(' AND producto_sl = :producto ');
        ...
        (*BEEF SQL.Add(' AND ( UPPER(calibre_sl[1,1]) = "G" AND ENVASE_SL <> "001" ) ' );*)
        ...
        begin
         if ant <> act then
          begin
           ...
           if not Grabar(IntToStr(ant), 'kilos',sumkilos,fech) then
            begin
             Result:=False;
             Exit;
            end;
         ...
         sumkilos:=sumKilos + FieldByName('kilos').AsFloat;
         fech:=FieldByName('fecha').AsDateTime;
         Next;
         Act:=Semana(FieldByName('fecha').AsDateTime);
        end;

        if not Grabar(IntToStr(act), 'KILOS',sumkilos,fech) then
         begin
          Result:=False;
          Exit;
         end;
...
begin
  ...
  begin
    ...
    if ( Trim(eProducto.Text) <> '' ) or ( Trim( eCategoria.Text ) <> '' ) or
      ( trim ( envase ) <> '' ) then
    begin
...
function TFLVentasSem.Grabar(filtro, campo: string; valor: real; fecha: TDate): boolean;
var a, m, d: word;
begin
...
begin
  ...
  QRLVentasSem.producto := txtProducto.Caption; //desProducto(EEmpresa.text,eProducto.Text);
  QRLVentasSem.envase := envase;
  QRLVentasSem.lblEnvase.Caption := envase + ' / ' + desEnvase(EEmpresa.Text, envase);
