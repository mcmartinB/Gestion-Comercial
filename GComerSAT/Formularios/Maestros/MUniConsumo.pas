...
procedure TFMUniConsumo.AntesDeLocalizar;
begin
  cbxVer.ItemIndex := 0;
  cbxVer.Enabled := True;
end;
...
begin
  ...
  begin
    ...
    if cbxVer.ItemIndex = 1 then
      where := where + ' fecha_baja_uc is null'
    else
      where := where + ' fecha_baja_uc is not null';
  end;
...
begin
  //buscar descripcion
  descripcion_pb.text := desProducto(empresa_uc.Text, producto_uc.Text);
end;
