...
type
  TFMAgrupacionesEnvase = class(TMaestro)
    ...
    pgcCostes: TPageControl;
    tsCosProducto: TTabSheet;
    dbgrdCostes: TDBGrid;
...
{+   Eliminar linea y funcion si no se va a usar }
  ...
  begin
    SQL.clear;
    SQL.Add(' select  empresa_cap empresa, fecha_ini_cap fecha, centro_cap centro,  ');
    SQL.Add('         producto_cap producto, substr(descripcion_p,1,20) des_producto,  ');
    SQL.Add('         coste_directo_cap + coste_material_cap + coste_seccion_cap coste_total ');
    SQL.Add(' from frf_costes_agrupa_prod ');
    SQL.Add('       join frf_productos on producto_p = producto_cap ');
    SQL.Add(' where agrupacion_cap = :agrupacion_ae ');
...
procedure TFMAgrupacionesEnvase.QAgrupacionesAfterOpen(DataSet: TDataSet);
begin
  qryCostesProducto.Open;
end;

procedure TFMAgrupacionesEnvase.QAgrupacionesBeforeClose(
  DataSet: TDataSet);
begin
  qryCostesProducto.Close;
end;

procedure TFMAgrupacionesEnvase.btnCostesClick(Sender: TObject);
begin
  if QAgrupaciones.IsEmpty then
    ShowMessage('Seleccione primero una agrupacion.')
  else
  begin
    CostesAgrupaProductoFD.ExecuteCosteAgrupacion( self, agrupacion_ae.Text, qryCostesProducto.FieldByName('empresa').AsString,
      qryCostesProducto.FieldByName('centro').AsString, qryCostesProducto.FieldByName('producto').AsString );
    qryCostesProducto.Close;
    qryCostesProducto.Open;
  end;
end;
