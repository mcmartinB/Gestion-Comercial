...
uses DConfigPrinter, CVariables, DConfigMail, UDMConfig, UDFactura,
     UFDEnviarCorreo, QRPDFFilt, UDMBaseDatos, UDMAuxDB, AdvertenciaFD;
      ...
      case FilterIndex of
        1: {XLS}
          begin
            ...
            {
            if ficheroEnUso( Filename ) then
              ShowMessage('Fichero abierto, hay que cerrarlo si quieres volver a grabarlo.')
            else
            }
...
//            begin
//              if Pos('.CSV',Uppercase( FileName ) ) = 0 then
//                FileName := FileName + '.csv';
//            end;
//              TQuickRep(qrprinter.master).ExportToFilter(TQRCommaSeparatedFilter.Create(Filename));
//          end;
...
begin
  ...
  begin
    ...
    QuickMail.Subject:= 'Envío del ' + TQuickRep(QRPrinter.master).ReportTitle + ' de '+
                        desCliente( sClienteConfig );
  end
  else
  begin
    QuickMail.Subject:= 'Envío del ' + TQuickRep(QRPrinter.master).ReportTitle;
  end;
...
begin
  ...
  sDir:= ExtractFilePath(AFileName+ '.pdf');
  ...
  begin
    ShowMessage('NO existe el directorio "' + Sdir + '" donde guardar los PDFs temporales.');
  end;
...
begin
  ...
  begin
    if (Copy( LowerCase(TQuickRep(QRPrinter.master).Name), 1, 9)  = 'qralbaran') or
       (LowerCase(QRPreview1.QRPrinter.Master.Name) = 'qrlfacturaproforma' ) or
      ...
      if LowerCase(TQuickRep(QRPrinter.master).Name) = 'qrlalbarantransito2' then
        ...
        if LowerCase(TQuickRep(QRPrinter.master).Name) = 'qrlfacturamanual' then
          ...
          begin
            bEnviado:= UDFactura.EnviarYGuardarFacturas(TQuickRep(qrprinter.master));
          end
          else
          begin
