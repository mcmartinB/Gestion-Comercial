...
type
  ...
  private
    ...
    function SacaCantidad(empresa, unidad, envase: string; cajas, unidades: Integer; kilos: Real): Real;
...
uses UDMAuxDB, CAuxiliarDB, CVariables, LFacturaProforma, Printers, StrUtils,
     UDMBaseDatos, bSQLUtils, UDMCambioMoneda;
...
begin
  if ConsultaOpen(DMAuxDB.QGeneral,
    ...
    '            incoterm_sc incoterm, plaza_incoterm_sc plaza ' +
    ' from frf_salidas_c ' +
...
begin
  registros := ConsultaOpen(DMAuxDB.QGeneral,
    ...
    '        kilos_sl kilos, unidad_precio_sl unidad' +
    ' from frf_salidas_l ' +
    ' where empresa_sl= ' + SQLString(empresa) +
    '   and centro_salida_sl= ' + SQLString(centro) +
    '   and fecha_sl= ' + SQLDate(fecha) +
    ...
    begin
      ...
      begin
        ...
        StringGrid1.Cells[4, fila] := FloatToStr(SacaCantidad(empresa, FieldByName('unidad').AsString,
          ...
          FieldByName('cajas').AsInteger,
          FieldByName('unidades').AsInteger,          
...
function isBeef(const producto, calibre, envase: string): Boolean;
begin
  if ((producto = 'T') or (producto = 'E')) and
    (Copy(UPPERCASE(calibre), 1, 1) = 'G') and
    (envase <> '001') then
    result := true
  else
    result := false;
...
begin
  if ConsultaOpen(DMBaseDatos.QGeneral,
    ...
    begin
      ...
      begin
        if isBeef(producto, calibre, envase) then
        begin
          result := 'BEEF'
        end
        else
  ...
  if ((producto = 'T') or (producto = 'E')) and (result <> 'BEEF') then
  begin
    result := result + '(' + calibre + ')';
  end;
  DMBaseDatos.QGeneral.Cancel;
  DMBaseDatos.QGeneral.Close;
end;
...
function TFDFacturaProforma.SacaCantidad(empresa, unidad, envase: string; cajas, unidades: Integer; kilos: Real): Real;
begin
  if unidad = 'UND' then
  begin
...
begin
  ...
  QRLFacturaProforma.albaran.Caption := numeroAlbaran;
  if (QRLFacturaProforma.albaran.Caption <> '') and (numeroAlbaran <> '') then
  ...
  begin
    QRLFacturaProforma.mascara := '#,##0.00';
    QRLFacturaProforma.mascara2 := '#,##0.000';
  end;
...
begin
  ...
  begin
    if StringGrid1.cells[7, conta] <> '' then
      acum := acum + StrToFloat(StringReplace(StringGrid1.cells[7, conta], '.', '', [rfReplaceAll, rfIgnoreCase]));
  end;
  ...
  exTotalEUR.text := FormatFloat('#,##0.00', UDMCambioMoneda.ChangeToEuro( moneda, exFecha.Text, acum + iva));
end;
