    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            AddMsg( AMsg, 'ERROR: No coincide los kilos para el artículo ' + FieldByname('envase_sl').AsString +
        ...
        begin
          ...
          begin
            AddMsg( AMsg, 'ERROR: No coincide las unidades por caja para el artículo ' + FieldByname('envase_sl').AsString +
...
begin
  ...
  begin
    ShowMessage('Factura contabilizada, solo se permitira modificar un número limitado de campos.');
...
begin
  ...
  begin
    ...
    begin
      ShowMessage( 'Albarán asociado a una factura contabilizada, ' + gsNombre + ' recuerde no modificar kilos e importes. ');
    end
    else
    Begin
...
begin
  ...
  begin
    ...
    if MessageDlg('El artículo seleccionado es de unidades variable y la unidades por cajas son 0, ¿es correcto?.',
...
begin
  ...
  begin
    if ( Date = StrToDate('2/7/2012') ) and ( gsCodigo = 'informix' ) then
    begin
      ShowMessage( 'Albarán asociado a una factura conatbilizada, ' + gsNombre + ' recuerde no modificar kilos e importes. ');
    end
    else
    Begin
      raise Exception.Create('No se puede modificar una salida asociada a una factura contabilizada.');
...
begin
  ...
  *)
  ...
  begin
    ...
    if not IsEmpty then
    begin
      Result:= MessageDlg('Salida asociada a un deposito de aduanas, tránsito: -> ' + fieldByName('empresa_dac').AsString + ' - ' +
...
begin
  ...
  begin
    raise Exception.Create(' ATENCION! No se puede borrar un albaran de salida con información en Deposito de Aduanas. ');
  end;
  ...
  (*
    ...
    begin
      ...
      begin
        Close;
        ref_transitos_sl.SetFocus;
        raise Exception.Create('No se puede asignar a un salida producto de un tránsito con fecha posterior.');
      end;
    end;
   *)
    Close;
...
begin
  ...
  begin
    ...
    begin
      if estoyFacturada then
      begin
        ShowMessage('Albarán asociado a una factura, no se puede modificar los precios.');
      end
      else
        ValorarAlbaran;
    end;
          ...
          0: if FieldByName('porc_iva_sl').AsFloat <> rSuper then
             begin
               ...
               sErrores:= sErrores + #13 + #10 +
                 '* Para el artículo ' + FieldByName('envase_sl').AsString +
