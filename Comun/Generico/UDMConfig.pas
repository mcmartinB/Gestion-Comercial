...
type
  ...
  public
    ...
    function DatosPorDefectoUsu: boolean;
...
begin
  ...
  begin
    SQL.Clear;
    SQL.Add('select * from cnf_datos_por_defecto');
...
//  DatosPorDefecto;
  DatosVersion;
  DatosInstalacion;
  if not DatosPorDefectoUsu then      //Si no existe datos por usuario
    DatosPorDefecto;
  CargaDirectorios;
...
function TDMConfig.DatosPorDefectoUsu: boolean;
begin
  gsDefEmpresa := '';
  gsDefCentro := '';
  gsDefProducto := '';
...
begin
  ...
  begin
    ...
    if not result then
    begin
      result:= ( UPPERCASE( gsCodigo )  = 'TFE017' ) or //eCHANDI
               ( UPPERCASE( gsCodigo )  = 'LLA012' ) or //Veronica
               ( UPPERCASE( gsCodigo )  = 'TFE007' );   //Ana Donate
    end;
  end;
end;
  ...
  begin
    ...
    begin
      result:= ( UPPERCASE( gsCodigo )  = 'LLANOS7' ) or
               ...
               ( UPPERCASE( gsCodigo )  = 'TFE017' ) or
               ( UPPERCASE( gsCodigo )  = 'LLA030' ); //eCHANDI
    end;
  end;
end;
