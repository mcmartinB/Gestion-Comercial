unit EstadoTesoreriaClientesFL;
...
type
  TFLEstadoTesoreriaClientes = class(TForm)
  ...
  private
    ...
    bExcluirCliente, bFechaTesoreria: boolean;
    iMercado, iMoneda, iEstadoCobro: integer;
    dFechaIni, dFechaFin, dFechaCobro: TDateTime;
...
procedure TFLEstadoTesoreriaClientes.BBAceptarClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      ...
      DMEstadoCobroCliente.PendienteCobro( sEmpresa, sCliente, sTipoCliente, sPais,
                                           dFechaIni, dFechaFin, dFechaCobro,
                                           iMercado, iMoneda, iEstadoCobro,
                                           bExcluirCliente, bFechaTesoreria,
                                           bVerFacturas );
      *)
    end;
  end;
...
procedure TFLEstadoTesoreriaClientes.btnResumenClick(Sender: TObject);
begin
  if CerrarForm(true) then
  begin
    Self.ActiveControl := nil;
...
procedure TFLEstadoTesoreriaClientes.BBCancelarClick(Sender: TObject);
begin
...
procedure TFLEstadoTesoreriaClientes.FormCreate(Sender: TObject);
(*
var
...
begin
  ...
  *)
  ...
  eFechaCobro.Text := DateTostr(Date);
...
procedure TFLEstadoTesoreriaClientes.FormActivate(Sender: TObject);
begin
  ActiveControl := edtEmpresa;
end;

procedure TFLEstadoTesoreriaClientes.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFLEstadoTesoreriaClientes.FormClose(Sender: TObject;
...
procedure TFLEstadoTesoreriaClientes.ADesplegarRejillaExecute(Sender: TObject);
var
  sAux: string;
begin
  RejillaFlotante.DataSource := DMBaseDatos.DSQDespegables;
...
procedure TFLEstadoTesoreriaClientes.PonNombre(Sender: TObject);
begin
    if (gRF <> nil) then
...
function TFLEstadoTesoreriaClientes.ParametrosOK: boolean;
begin
  Result := false;
  ...
  if not TryStrToDate(eFechaCobro.Text, dFechaCobro ) then
  begin
  ...
  iMercado:= rgMercado.ItemIndex;
  iMoneda:= rgMoneda.ItemIndex;
  iEstadoCobro:= cbbEstadoSaldo.ItemIndex;
  bFechaTesoreria:= rgTipoListado.ItemIndex < 2;
...
procedure TFLEstadoTesoreriaClientes.cbbEstadoSaldoChange(Sender: TObject);
begin
  //
end;

procedure TFLEstadoTesoreriaClientes.btn1Click(Sender: TObject);
begin
...
procedure TFLEstadoTesoreriaClientes.btnLimpiarClick(Sender: TObject);
begin
  txtListaCliente.Caption:= '';
end;

procedure TFLEstadoTesoreriaClientes.edtTipoClienteChange(Sender: TObject);
begin
...
procedure TFLEstadoTesoreriaClientes.edtClienteChange(Sender: TObject);
begin
  if edtCliente.Text = '' then
  begin
...
procedure TFLEstadoTesoreriaClientes.rgTipoListadoClick(Sender: TObject);
begin
  case rgTipoListado.ItemIndex of
    0: Caption:= '   SALDO CLIENTES - AVISO PENDIENTE COBRO SEGUN TESORERÍA';
    1: Caption:= '   SALDO CLIENTES - PREVISIÓN COBRO SEGUN TESORERÍA';
    2: Caption:= '   SALDO CLIENTES - PENDIENTE COBRO SEGUN FORMA PAGO';
    3: Caption:= '   SALDO CLIENTES - PREVISIÓN COBRO SEGUN FORMA PAGO';
  end;
end;

(*
procedure TFLEstadoTesoreriaClientes.btnCorreosClick(Sender: TObject);
VAR
  sAux, eMail: string;
  aux: integer;
begin
