...
type
  TQRAsignacionFruta = class(TQuickRep)
    bndDetalle: TQRBand;
    bndTitulo: TQRBand;
...
procedure TQRAsignacionFruta.QRLabel25Print(sender: TObject;
  var Value: String);
begin
...
procedure TQRAsignacionFruta.bndDetalleBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  rImporte:= 0;
end;

procedure TQRAsignacionFruta.bndLineasBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  rAux: Real;
begin
...
procedure TQRAsignacionFruta.bndPieLineasBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlblImporteLineas.Caption:= FormatFloat( '#,##0.00', rImporte );
end;

procedure TQRAsignacionFruta.Desproveedor_ecPrint(sender: TObject;
  var Value: String);
begin
  Value:= desProveedor( DataSet.FieldByName('empresa_ac').AsString, Value );
end;

procedure TQRAsignacionFruta.QRDBText8Print(sender: TObject;
  var Value: String);
begin
  Value:= desProveedorAlmacen( DataSet.FieldByName('empresa_ac').AsString, DataSet.FieldByName('proveedor_ac').AsString, Value );
end;

procedure TQRAsignacionFruta.qrdbtxtempresa_ac1Print(sender: TObject;
  var Value: String);
begin
    Value:= desEmpresa( Value );
end;

procedure TQRAsignacionFruta.qrdbtxtproducto_al1Print(sender: TObject;
  var Value: String);
begin
