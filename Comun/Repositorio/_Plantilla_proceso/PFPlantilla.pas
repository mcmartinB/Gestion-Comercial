...
var
  FLPlantilla: TFLPlantilla;

implementation

uses UDMAuxDB, Principal, CVariables, DPreview, CReportes,
     CAuxiliarDB, UDMBaseDatos, UDMCambioMoneda, LQPlantilla;

{$R *.DFM}

//                         ****  BOTONES  ****

procedure TFLPlantilla.BBAceptarClick(Sender: TObject);
begin
  if not CamposVacios then
  begin
    if not LQPlantilla.Imprimir ( sEmpresa, sCentro, sCliente, sProducto, dIni, dFin ) then
        ShowMessage('Sin datos.');
  end;
end;

procedure TFLPlantilla.BBCancelarClick(Sender: TObject);
begin
...
//                          **** FORMULARIO ****
procedure TFLPlantilla.FormCreate(Sender: TObject);
begin
  FormType := tfOther;
  BHFormulario;
  ...
  LQPlantilla.InicializarReport;
end;

procedure TFLPlantilla.FormActivate(Sender: TObject);
begin
  Top := 1;
  ActiveControl := EEmpresa;
end;

procedure TFLPlantilla.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFLPlantilla.FormClose(Sender: TObject;
...
procedure TFLPlantilla.ADesplegarRejillaExecute(Sender: TObject);
begin
...
procedure TFLPlantilla.PonNombre(Sender: TObject);
begin
...
function TFLPlantilla.CamposVacios: boolean;
begin
  Result := True;
