object FPTransitoActivar: TFPTransitoActivar
  Left = 817
  Top = 305
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = '    ACTIVACI'#211'N TRANSITO'
  ClientHeight = 234
  ClientWidth = 500
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  HelpFile = 'help/LVentasBruto.rtf'
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object btnEntrada: TBCalendarButton
    Left = 223
    Top = 119
    Width = 17
    Height = 21
    Action = ADesplegarRejilla
    Glyph.Data = {
      
      FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    Control = edtEntrada
    Calendar = CalendarioFlotante
    CalendarAlignment = taUpLeft
    CalendarWidth = 197
    CalendarHeigth = 153
  end
  object lblEntrada: TLabel
    Left = 51
    Top = 120
    Width = 90
    Height = 19
    AutoSize = False
    Caption = ' Fecha Entrada'
    Color = cl3DLight
    ParentColor = False
    Layout = tlCenter
  end
  object btnAceptar: TSpeedButton
    Left = 274
    Top = 179
    Width = 89
    Height = 25
    Action = BAceptar
    Glyph.Data = {
      
      FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    ParentShowHint = False
    ShowHint = True
  end
  object btnCancelar: TSpeedButton
    Left = 371
    Top = 179
    Width = 89
    Height = 25
    Action = BCancelar
    Glyph.Data = {
      
      FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    ParentShowHint = False
    ShowHint = True
  end
  object lblRefTransito: TLabel
    Left = 51
    Top = 73
    Width = 90
    Height = 19
    AutoSize = False
    Caption = ' Ref. Tr'#225'nsito'
    Color = cl3DLight
    ParentColor = False
    Layout = tlCenter
  end
  object lblEmpresa: TLabel
    Left = 51
    Top = 29
    Width = 90
    Height = 19
    AutoSize = False
    Caption = ' Empresa'
    Color = cl3DLight
    ParentColor = False
    Layout = tlCenter
  end
  object lblCentroDestino: TLabel
    Left = 51
    Top = 51
    Width = 90
    Height = 19
    AutoSize = False
    Caption = ' Centro Origen'
    Color = cl3DLight
    ParentColor = False
    Layout = tlCenter
  end
  object lblFechaTransito: TLabel
    Left = 51
    Top = 96
    Width = 90
    Height = 19
    AutoSize = False
    Caption = ' Fecha Tr'#225'nsito'
    Color = cl3DLight
    ParentColor = False
    Layout = tlCenter
  end
  object Label1: TLabel
    Left = 51
    Top = 147
    Width = 374
    Height = 13
    Caption = 
      'Pone los palets asignados al transito en estado de stock, prepar' +
      'ados para usar.'
  end
  object Label2: TLabel
    Left = 51
    Top = 161
    Width = 272
    Height = 13
    Caption = 'Usar esta opci'#243'n cuando se reciba fisicamente el tr'#225'nsito.'
  end
  object eRefTransito: TBEdit
    Left = 141
    Top = 72
    Width = 50
    Height = 21
    TabStop = False
    ColorEdit = clMoneyGreen
    Enabled = False
    ReadOnly = True
    MaxLength = 6
    TabOrder = 5
    OnChange = PonNombre
  end
  object eEmpresa: TBEdit
    Left = 141
    Top = 28
    Width = 33
    Height = 21
    TabStop = False
    ColorEdit = clMoneyGreen
    Zeros = True
    Enabled = False
    ReadOnly = True
    MaxLength = 3
    TabOrder = 1
    OnChange = PonNombre
  end
  object stEmpresa: TStaticText
    Left = 176
    Top = 30
    Width = 267
    Height = 17
    AutoSize = False
    BorderStyle = sbsSunken
    TabOrder = 2
  end
  object stCentroOrigen: TStaticText
    Left = 176
    Top = 52
    Width = 267
    Height = 17
    AutoSize = False
    BorderStyle = sbsSunken
    TabOrder = 4
  end
  object edtCentroOrigen: TBEdit
    Left = 141
    Top = 50
    Width = 23
    Height = 21
    TabStop = False
    ColorEdit = clMoneyGreen
    Enabled = False
    ReadOnly = True
    MaxLength = 1
    TabOrder = 3
    OnChange = PonNombre
  end
  object eFechaTransito: TBEdit
    Left = 141
    Top = 95
    Width = 81
    Height = 21
    TabStop = False
    ColorEdit = clMoneyGreen
    InputType = itDate
    Enabled = False
    ReadOnly = True
    MaxLength = 10
    TabOrder = 6
  end
  object CalendarioFlotante: TBCalendario
    Left = 488
    Top = 8
    Width = 197
    Height = 153
    Date = 36717.371983657410000000
    ShowToday = False
    TabOrder = 0
    Visible = False
    WeekNumbers = True
  end
  object edtEntrada: TBEdit
    Left = 141
    Top = 119
    Width = 81
    Height = 21
    ColorEdit = clMoneyGreen
    InputType = itDate
    MaxLength = 10
    TabOrder = 7
  end
  object edtHora: TBEdit
    Left = 243
    Top = 120
    Width = 50
    Height = 21
    ColorEdit = clMoneyGreen
    MaxLength = 5
    TabOrder = 8
    OnChange = PonNombre
  end
  object ListaAcciones: TActionList
    Images = DMBaseDatos.ImgBotones
    Left = 8
    Top = 44
    object BAceptar: TAction
      Caption = 'Aceptar'
      ShortCut = 112
      OnExecute = BBAceptarClick
    end
    object BCancelar: TAction
      Caption = 'Cancelar'
      ShortCut = 27
      OnExecute = BBCancelarClick
    end
    object ADesplegarRejilla: TAction
      ImageIndex = 0
      ShortCut = 113
      OnExecute = ADesplegarRejillaExecute
    end
  end
end
