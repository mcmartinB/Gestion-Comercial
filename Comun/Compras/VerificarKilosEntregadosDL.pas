unit VerificarKilosEntregadosDL;
...
type
  TDLVerificarKilosEntregados = class(TDataModule)
    qryAux: TQuery;
    qryLocal: TQuery;
    qryRemoto: TQuery;
  ...
  private
    ...
    iTipoRango: Integer;
    strEmpresa, sProducto, sCategoria, sAnoSemana, sProveedor, sEntrega: string;
    dIni, dFin: TDateTime;
  ...
  public
    { Public declarations }
    procedure KilosEntregasPlatano( const AEmpresa, AProducto, ACategoria, AProveedor: string;
                                    const ATipoRango: Integer; const ASemana,  AEntrega: string; const AFechaIni, AFechaFin: TDateTime;
                                    const ASobrepeso: real; const APlanta, APorCategoria, APorVariedad, AVerEntregas: boolean );
  end;
...
uses
  Variants, Dialogs, LiquidaIncidenciasQL, VerificarKilosEntregadosQL, UDMBaseDatos, bMath;

procedure TDLVerificarKilosEntregados.DataModuleCreate(Sender: TObject);
begin
  //
  ...
  kmtKilos.FieldDefs.Add('kilos_faltan', ftFloat, 0, False);
  ...
  qryEnvase.sql.Add(' where empresa_e = :empresa ');
...
procedure TDLVerificarKilosEntregados.DataModuleDestroy(Sender: TObject);
begin
  //
  kmtKilos.Close;
end;

function TDLVerificarKilosEntregados.ConectarRemoto: boolean;
var
  sBD: string;
  bAux1: Boolean;
begin
...
procedure TDLVerificarKilosEntregados.DesconectarRemoto;
begin
  if DMBaseDatos.dbF17.Connected then
    DMBaseDatos.dbF17.Connected:= False;
  if DMBaseDatos.dbF18.Connected then
...
procedure TDLVerificarKilosEntregados.AnyadirKilosRemoto;
begin
...
procedure TDLVerificarKilosEntregados.AnyadirKilosSalidasVerde;
var
...
begin
  ...
  begin
    ...
    if iTipoRango = 0 then
    begin
      Sql.Add(' and anyo_Semana_ec = :semana ' );
    end
    else
    if iTipoRango = 1 then
    begin
      Sql.Add(' and codigo_ec = :entrega ' );
    end
    else
    if iTipoRango = 2 then
    begin
      Sql.Add(' and fecha_llegada_ec between :ini and :fin ' );
    end;
    ...
    if iTipoRango = 0 then
    begin
      ParamByName('semana').AsString:= sAnoSemana;
    end
    else
    if iTipoRango = 1 then
    begin
...
procedure TDLVerificarKilosEntregados.AnyadirLineaKilosSalidasVerde;
var
  sEmpresa: string;
begin
  if bPlanta then
...
procedure TDLVerificarKilosEntregados.AnyadirKilosAlmacen;
var
  sqlProveedor, sqlCategoria, sqlEntrega, sqlVariedad: string;
begin
  with qryRemoto do
...
procedure TDLVerificarKilosEntregados.AnyadirLineaKilosAlmacen;
var
  sEmpresa: string;
begin
  if bPlanta then
...
procedure TDLVerificarKilosEntregados.AnyadirKilosRf;
var
  sqlProveedor, sqlCategoria, sqlEntrega, sqlVariedad: string;
begin
  with qryRemoto do
...
procedure TDLVerificarKilosEntregados.AnyadirLineaKilosRF;
var
  sEmpresa: string;
  rAux: Real;
begin
  ...
  begin
    ...
    if Abs( kmtKilos.FieldByName('porcen_diff_krf').AsFloat  ) > rSobrepeso then
      kmtKilos.FieldByName('diff_krf').AsFloat:= 1
    else
      kmtKilos.FieldByName('diff_krf').AsFloat:= 0;

    rAux:= kmtKilos.FieldByName('kilos_almacen').AsFloat * ( 1 + ( rSobrepeso/100 ) );
    rAux:= kmtKilos.FieldByName('kilos_rf').AsFloat - rAux;
    if rAux < 0 then
      kmtKilos.FieldByName('kilos_faltan').AsFloat:= Abs( rAux )
    else
      kmtKilos.FieldByName('kilos_faltan').AsFloat:= 0;
    kmtKilos.Post;
...
procedure TDLVerificarKilosEntregados.AnyadirLineaKilosLocal;
var
...
procedure TDLVerificarKilosEntregados.AnyadirKilosLocal;
var
  sqlProveedor, sqlCategoria, sqlEntrega, sqlVariedad: string;
begin
  with qryLocal do
...
procedure TDLVerificarKilosEntregados.AnyadirKilos;
begin
  AnyadirKilosLocal;
  AnyadirKilosRemoto;
end;

procedure TDLVerificarKilosEntregados.KilosEntregasPlatano( const AEmpresa, AProducto, ACategoria, AProveedor: string;
                                                const ATipoRango: Integer; const ASemana,  AEntrega: string; const AFechaIni, AFechaFin: TDateTime;
                                                const ASobrepeso: real; const APlanta, APorCategoria, APorVariedad, AVerEntregas: boolean );
begin
  sProducto:= AProducto;
  sCategoria:= ACategoria;
  iTipoRango:= ATipoRango;
  sAnoSemana:= ASemana;
  sEntrega:= AEntrega;
  dIni:= AFechaIni;
  dFin:= AFechaFin;
  ...
  begin
    ...
    if iTipoRango = 0 then
    begin
      Sql.Add(' where anyo_Semana_ec = :semana ' );
    end
    else
    if iTipoRango = 1 then
    begin
    ...
    if iTipoRango = 2 then
    begin
      Sql.Add(' where fecha_llegada_ec between :ini and :fin ' );
    end;
    ...
    begin
      ...
      begin
        Sql.Add(' and empresa_ec in (''F17'',''F21'',''F23'',''F24'',''F42'') ');
      end
      else
      begin
    ...
    begin
      ...
      try
        ...
        kmtKilos.Sort([]);
        VerificarKilosEntregadosQL.PrevisualizarKilos( AEmpresa, AProducto, ACategoria, ATipoRango, ASemana, AEntrega, AFechaIni, AFechaFin, APorCategoria, AVerEntregas );
      finally
        kmtKilos.Close;
      end;
    end
