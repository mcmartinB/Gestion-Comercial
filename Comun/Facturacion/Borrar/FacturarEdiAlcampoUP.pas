...
function CodInternoProveedor: string;
var
  sAux: string;
begin
  ...
  begin
    SQL.Clear;
    SQL.Add('select cip_c from frf_clientes where empresa_c = :empresa and nif_c = :cliente ') ;
    ParamByName('empresa').AsString:= DPFacturarEdi.QCabEdi.FieldByName('empresa_fec').AsString;
    ParamByName('cliente').AsString:= DPFacturarEdi.QCabEdi.FieldByName('nif_fec').AsString;
    Open;
    sAux:= FieldByName('cip_c').AsString;
    Close;
  end;
  Result:=  sAux + '|';
  (*
  ...
  begin
    cabeceras.Add(
        ...
        *)
        ...
        '188'+QCabEdi.FieldByName('pedido_fec').AsString + '000|' +
        ...
        CodInternoProveedor //Departamento
    );
  end;
end;
