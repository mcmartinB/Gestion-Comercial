...
uses
  UDMAuxDB, FacturarEdiDP, CGlobal, SysUtils;
...
begin
  ...
  begin
    cabeceras.Add(
        ...
        'ES' + QCabEdi.FieldByName('nif_fec').AsString + '|' +
        ...
        '|' + //CPEXTRA
        QCabEdi.FieldByName('fecha_albaran_fec').AsString + '|' + //FALBARAN
        '|' + //FPEDIDO
        FormatDateTime( 'dd/mm/yyyy', QCabEdi.FieldByName('fecha_albaran_fec').AsDateTime + 1 ) + '|' + //FENTMER
        '|' + //RRSOCIAL
        '|' + //DUMMY
        ''  //DEPTO
    );
...
begin
  result:= DPFacturarEdi.QLinEdi.FieldByName('medida_fel').AsString + '|'
  //¿¿¿???
  (*
  if CGlobal.gProgramVersion = CGlobal.pvBAG then
