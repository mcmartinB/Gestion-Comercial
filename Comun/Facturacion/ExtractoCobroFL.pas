...
uses UDMAuxDB, Principal, CVariables, CReportes, UDMConfig, CGlobal,
  CAuxiliarDB, ExtractoCobroQL, DPreview, UDMBaseDatos;

{$R *.DFM}

//                         ****  BOTONES  ****

procedure TFLExtractoCobro.BBAceptarClick(Sender: TObject);
begin
...
procedure TFLExtractoCobro.BBCancelarClick(Sender: TObject);
begin
...
procedure TFLExtractoCobro.FormCreate(Sender: TObject);
(*
var
...
procedure TFLExtractoCobro.FormActivate(Sender: TObject);
begin
  ActiveControl := EEmpresa;
end;

procedure TFLExtractoCobro.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFLExtractoCobro.FormClose(Sender: TObject;
...
procedure TFLExtractoCobro.ADesplegarRejillaExecute(Sender: TObject);
begin
...
procedure TFLExtractoCobro.PonNombre(Sender: TObject);
begin
    if (gRF <> nil) then
...
function TFLExtractoCobro.CamposVacios: boolean;
var
...
procedure TFLExtractoCobro.AbrirQuery;
begin
  ...
  begin
    ...
    QEstadoCobros.SQL.Add(' SELECT cod_cliente_fc cliente_fac_f, n_factura_fc n_factura_f, cod_factura_fc ,  ' +
      '                       fecha_factura_fc fecha_factura_f, importe_total_fc importe_total_f,  ' +
    ...
    QEstadoCobros.SQL.Add(' FROM tfacturas_cab join frf_clientes on cliente_c = cod_cliente_fc   ');
    QEstadoCobros.SQL.Add('                    left join frf_clientes_tes on empresa_ct = cod_empresa_fac_fc and cliente_ct = cod_cliente_fc ');
    QEstadoCobros.SQL.Add('                    left join frf_bancos on banco_ct = banco_b ');
    QEstadoCobros.SQL.Add('                    left join frf_forma_pago on forma_pago_ct = codigo_fp ');
    ...
    QEstadoCobros.SQL.Add('  ORDER BY pais_c, cliente_fac_f, prevision_cobro_f, cod_factura_fc, n_factura_f  ');
    try
  ...
  begin
    QEstadoCobros.sql.Clear;
        QEstadoCobros.SQL.Add(' SELECT cod_cliente_fc cliente_fac_f, n_factura_fc n_factura_f, cod_factura_fc, fecha_factura_fc fecha_factura_f, importe_total_fc importe_total_f,  ' +
      '        moneda_fc moneda_f, nvl(prevision_cobro_fc,fecha_factura_fc) prevision_cobro_f,  ' +
      '        nvl(prevision_cobro_fc,fecha_factura_fc) + 60 fecha_afp_f, ');
...
procedure TFLExtractoCobro.cbbEstadoCobroChange(Sender: TObject);
begin
  //
end;
