...
uses
  ...
  DError, CVariables, StrUtils, DBActns, StdActns, UFMServicioVenta;

type
  TFMGastosServicioVenta = class(TForm)
    AGastos: TActionList;
    ...
    LProducto: TLabel;
    producto_gsv: TBDEdit;
    BGBProducto: TBGridButton;
    stProducto: TStaticText;
    Label2: TLabel;
    fecha_fac_gsv: TBDEdit;
    btnFecha_fac_gsv: TBCalendarButton;
    Calendario: TBCalendario;
    Label3: TLabel;
    nota_gsv: TBDEdit;
    pagado_gsv: TDBCheckBox;
    QGastos: TQuery;
    ...
    procedure tipo_gsvRequiredTime(Sender: TObject; var isTime: Boolean);
    ...
    procedure tipo_gsvChange(Sender: TObject);
    procedure producto_gsvChange(Sender: TObject);
    ...
    procedure btnFecha_fac_gsvClick(Sender: TObject);
...
var
  FMGastosServicioVenta: TFMGastosServicioVenta;
  opcc: Integer;
...
procedure TFMGastosServicioVenta.CargaParametros( const AEmpresa: string; const AServicio: integer; const AFechaServicio: TDateTime );
begin
  sEmpresa:= AEmpresa;
  iServicio:= AServicio;
  ...
  with QGastos do
  begin
    ParamByName('empresa_sv').AsString:= AEmpresa;
    ParamByName('servicio_sv').AsInteger:= AServicio;
  end;
end;

procedure TFMGastosServicioVenta.FormShow(Sender: TObject);
begin
  QGastos.Open;
...
procedure TFMGastosServicioVenta.FormCreate(Sender: TObject);
begin
  Salir := false;
...
procedure TFMGastosServicioVenta.FormActivate(Sender: TObject);
begin
  gRF := RejillaFlotante;
  gCF := nil;
end;
...
procedure TFMGastosServicioVenta.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if salir then
  begin
    QGastos.Close;
...
procedure TFMGastosServicioVenta.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFMGastosServicioVenta.AAceptarExecute(Sender: TObject);
begin
...
procedure TFMGastosServicioVenta.ACancelarExecute(Sender: TObject);
var
  sAux: string;
begin
  if RejillaFlotante.Visible then
...
procedure TFMGastosServicioVenta.ASalirExecute(Sender: TObject);
begin
  if Salir then
    Close;
end;

function  TFMGastosServicioVenta.ValidarDatos: boolean;
begin
  result:= false;
  if stProducto.Caption = '' then
  begin
    MessageDlg('Debe introducir un producto valido o dejarlo vacio...', mtError, [mbOK], 0);
    producto_gsv.SetFocus;
  end
  else
  ...
  begin
    ...
    begin
      ...
      SQL.Add('WHERE tipo_tg = ' + QuotedStr(tipo_gsv.Text));
      SQL.Add('AND gasto_transito_tg = 0');
      Open;
      if (FieldByName('cuenta').AsInteger = 0) then
      begin
        MessageDlg('El Tipo de Gastos introducido no es de salida o no existe ...', mtError, [mbOK], 0);
        tipo_gsv.SetFocus;
      end
      else
      begin
...
procedure TFMGastosServicioVenta.AceptarAltas;
var
  sProducto, stipo: string;
begin
  if ValidarDatos then
  begin
    if producto_gsv.Text = '' then
      QGastos.FieldByName('producto_gsv').Value:= NULL;
    QGastos.Post;
    QGastos.DisableControls;
    try
      sProducto:= QGastos.FieldByname('producto_gsv').AsString;
      sTipo:= QGastos.FieldByname('tipo_gsv').AsString;
      QGastos.Close;
      QGastos.Open;
      While ( ( QGastos.FieldByname('producto_gsv').AsString <> sProducto ) OR ( QGastos.FieldByname('tipo_gsv').AsString <> sTipo ) ) AND
        not QGastos.Eof do
        QGastos.Next;
    finally
...
procedure TFMGastosServicioVenta.CancelarAltas;
begin
  QGastos.Cancel;
  cambiarEstado(teLocalizar);
end;

procedure TFMGastosServicioVenta.AceptarModificar;
begin
  if ValidarDatos then
  begin
    if producto_gsv.Text = '' then
...
procedure TFMGastosServicioVenta.CancelarModificar;
begin
  QGastos.Cancel;
  cambiarEstado(teLocalizar);
end;
...
procedure TFMGastosServicioVenta.ABorrarExecute(Sender: TObject);
begin
  QGastos.Delete;
  cambiarEstado(teConjuntoResultado);
end;

procedure TFMGastosServicioVenta.cambiarEstado(estad: TEstado);
var
  vis: Boolean;
begin
  estado := estad;
  ...
  tipo_gsv.Visible := vis;
  BGBTipoGastos.Visible := vis;
  STTipoGastos.Visible := vis;
  LImporte.Visible := vis;
  importe_gsv.Visible := vis;
  ref_fac_gsv.Visible := vis;
  nota_gsv.Visible := vis;
  LApunte.Visible := vis;
  LProducto.Visible := vis;
  Producto_gsv.Visible := vis;
...
procedure TFMGastosServicioVenta.Botones;
begin
  case estado of
    teAlta:
      begin
...
procedure TFMGastosServicioVenta.AAltaExecute(Sender: TObject);
begin
  cambiarEstado(teAlta);
  QGastos.Insert;
  QGastos.FieldByname('empresa_gsv').AsString := sEmpresa;
  QGastos.FieldByname('servicio_gsv').AsInteger := iServicio;
  QGastos.FieldByname('pagado_gsv').AsInteger := 0;
  producto_gsv.SetFocus;
  producto_gsv.Change;
  tipo_gsv.Change;
end;

procedure TFMGastosServicioVenta.AModificarExecute(Sender: TObject);
begin
...
procedure TFMGastosServicioVenta.CalculoGastos;
begin
  with DMAuxDB.QAux do
  begin
    SQL.Clear;
    SQL.Add(' select sum(importe_gsv) importe ');
    SQL.Add(' from frf_gastos_servicios_venta ');
    SQL.Add(' where empresa_gsv = :empresa ');
    SQL.Add('   and servicio_gsv = :servicio ');
...
procedure TFMGastosServicioVenta.tipo_gsvRequiredTime(Sender: TObject;
  var isTime: Boolean);
begin
  isTime := false;
...
procedure TFMGastosServicioVenta.APrimeroExecute(Sender: TObject);
begin
  rejilla.DataSource.DataSet.First;
  botoneraDespl;
end;

procedure TFMGastosServicioVenta.AAnteriorExecute(Sender: TObject);
begin
  rejilla.DataSource.DataSet.Prior;
  botoneraDespl;
end;

procedure TFMGastosServicioVenta.ASiguienteExecute(Sender: TObject);
begin
  rejilla.DataSource.DataSet.Next;
  botoneraDespl;
end;

procedure TFMGastosServicioVenta.AUltimoExecute(Sender: TObject);
begin
  rejilla.DataSource.DataSet.Last;
  botoneraDespl;
end;

procedure TFMGastosServicioVenta.botoneraDespl;
begin
...
procedure TFMGastosServicioVenta.EuroConversorExecute( const AFecha: TDateTime );
var
  aux: string;
begin
...
procedure TFMGastosServicioVenta.rejillaDblClick(Sender: TObject);
begin
  SBModificar.Click;
end;

procedure TFMGastosServicioVenta.tipo_gsvChange(Sender: TObject);
begin
  STTipoGastos.Caption:= desTipoGastos(tipo_gsv.Text );
end;

procedure TFMGastosServicioVenta.producto_gsvChange(Sender: TObject);
begin
  stProducto.Caption:= desProducto(sEmpresa, producto_gsv.Text );
  if stProducto.Caption = '' then
    stProducto.Caption:= 'Vacio = Todos los productos';
end;

procedure TFMGastosServicioVenta.ARejillaFlotanteExecute(Sender: TObject);
begin
  if producto_gsv.Focused then
    BGBProducto.Click
  else
  if Tipo_gsv.Focused then
    BGBTipoGastos.Click
  else
  if Fecha_fac_gsv.Focused then
    btnFecha_fac_gsv.Click;
end;

procedure TFMGastosServicioVenta.BGBProductoClick(Sender: TObject);
begin
  if DMBaseDatos.QDespegables.Active then
    DMBaseDatos.QDespegables.Close;
  DMBaseDatos.QDespegables.SQL.Clear;

  RejillaFlotante.BControl := producto_gsv;
...
procedure TFMGastosServicioVenta.BGBTipoGastosClick(Sender: TObject);
begin
  if DMBaseDatos.QDespegables.Active then
    DMBaseDatos.QDespegables.Close;
  DMBaseDatos.QDespegables.SQL.Clear;

  RejillaFlotante.BControl := tipo_gsv;
  ...
  DMBaseDatos.QDespegables.SQL.Add('   from frf_gastos_servicios_venta ');
  DMBaseDatos.QDespegables.SQL.Add('   where empresa_gsv = :empresa ');
  DMBaseDatos.QDespegables.SQL.Add('     and servicio_gsv = :servicio ');
  DMBaseDatos.QDespegables.SQL.Add('   and tipo_gsv = tipo_tg ');
  if producto_gsv.Text <> '' then
  begin
    DMBaseDatos.QDespegables.SQL.Add('and producto_gsv = :producto ');
  end;
  DMBaseDatos.QDespegables.SQL.Add(' ) ');
  ...
  if producto_gsv.Text <> '' then
  begin
    DMBaseDatos.QDespegables.ParamByName('producto').AsString:= producto_gsv.Text;
  end;
  DMBaseDatos.QDespegables.ParamByName('empresa').AsString:= sEmpresa;
...
procedure TFMGastosServicioVenta.rejillaDrawColumnCell(Sender: TObject;
...
procedure TFMGastosServicioVenta.btnFecha_fac_gsvClick(Sender: TObject);
var
  dFecha: TDate;
begin
  dFecha:= StrToDateDef( fecha_fac_gsv.Text, Date );
  Calendario.BControl:= fecha_fac_gsv;
  Calendario.Date:= dFecha;
  btnFecha_fac_gsv.CalendarShow;
end;

procedure TFMGastosServicioVenta.QGastosAfterPost(DataSet: TDataSet);
begin
...
procedure TFMGastosServicioVenta.QGastosAfterDelete(DataSet: TDataSet);
begin
  bGastoModificado:= True;
end;

function TFMGastosServicioVenta.GastosModificados: boolean;
begin
