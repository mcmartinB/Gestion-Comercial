object FDFacturaProforma: TFDFacturaProforma
  Left = 362
  Top = 278
  BorderIcons = []
  Caption = '   FACTURA PROFORMA'
  ClientHeight = 430
  ClientWidth = 755
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 148
    Width = 755
    Height = 282
    Align = alBottom
    TabOrder = 1
    object bvlPrecioPlasticos: TBevel
      Left = 536
      Top = 171
      Width = 108
      Height = 87
    end
    object Label1: TLabel
      Left = 388
      Top = 14
      Width = 23
      Height = 13
      Caption = 'Neto'
    end
    object labelIva: TLabel
      Left = 502
      Top = 14
      Width = 17
      Height = 13
      Caption = 'IVA'
    end
    object Label3: TLabel
      Left = 616
      Top = 14
      Width = 24
      Height = 13
      Caption = 'Total'
    end
    object LMoneda: TLabel
      Left = 731
      Top = 31
      Width = 39
      Height = 13
      Caption = 'Moneda'
    end
    object Label4: TLabel
      Left = 731
      Top = 55
      Width = 23
      Height = 13
      Caption = 'EUR'
    end
    object Label5: TLabel
      Left = 27
      Top = 14
      Width = 36
      Height = 13
      Caption = 'Factura'
    end
    object Label6: TLabel
      Left = 141
      Top = 14
      Width = 30
      Height = 13
      Caption = 'Fecha'
    end
    object Label7: TLabel
      Left = 255
      Top = 14
      Width = 32
      Height = 13
      Caption = 'Cliente'
    end
    object Label8: TLabel
      Left = 388
      Top = 88
      Width = 48
      Height = 13
      Caption = 'Suministro'
    end
    object Label9: TLabel
      Left = 502
      Top = 88
      Width = 33
      Height = 13
      Caption = 'Pedido'
    end
    object Label10: TLabel
      Left = 616
      Top = 88
      Width = 43
      Height = 13
      Caption = 'Veh'#237'culo'
    end
    object Label2: TLabel
      Left = 27
      Top = 153
      Width = 110
      Height = 13
      Caption = 'Observaciones Factura'
    end
    object Label11: TLabel
      Left = 654
      Top = 175
      Width = 46
      Height = 13
      Caption = 'N. Copias'
    end
    object lblIcoterm: TLabel
      Left = 388
      Top = 130
      Width = 41
      Height = 13
      Caption = 'Incoterm'
    end
    object lblPrecioCajaPlastica: TLabel
      Left = 544
      Top = 176
      Width = 94
      Height = 13
      Caption = 'Precio Caja Pl'#225'stica'
    end
    object lblPrecioPaletPlastico: TLabel
      Left = 541
      Top = 215
      Width = 97
      Height = 13
      Caption = 'Precio Palet Pl'#225'stico'
    end
    object memoDireccionEnv: TMemo
      Left = 27
      Top = 51
      Width = 341
      Height = 97
      TabStop = False
      Color = 14737632
      TabOrder = 20
    end
    object MemoObservaciones: TMemo
      Left = 27
      Top = 171
      Width = 506
      Height = 87
      Color = 14737632
      Lines.Strings = (
        'MemoObservaciones')
      ReadOnly = True
      TabOrder = 13
    end
    object memoDireccion: TMemo
      Left = 27
      Top = 51
      Width = 341
      Height = 97
      TabStop = False
      Color = 14737632
      TabOrder = 6
    end
    object BitBtn1: TBitBtn
      Left = 654
      Top = 201
      Width = 75
      Height = 25
      ModalResult = 1
      TabOrder = 17
      TabStop = False
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object BitBtn2: TBitBtn
      Left = 654
      Top = 233
      Width = 75
      Height = 25
      TabOrder = 19
      TabStop = False
      OnClick = BitBtn2Click
      Kind = bkCancel
    end
    object exNeto: TEdit
      Left = 388
      Top = 29
      Width = 113
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 3
    end
    object exIva: TEdit
      Left = 502
      Top = 29
      Width = 113
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 4
    end
    object exTotal: TEdit
      Left = 616
      Top = 29
      Width = 113
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 5
    end
    object exTotalEUR: TEdit
      Left = 616
      Top = 51
      Width = 113
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 7
    end
    object exFactura: TEdit
      Left = 27
      Top = 29
      Width = 113
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 0
      Text = 'PROFORMA'
    end
    object exFecha: TEdit
      Left = 141
      Top = 29
      Width = 113
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 1
    end
    object exCliente: TEdit
      Left = 255
      Top = 29
      Width = 113
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 2
    end
    object exSuministro: TEdit
      Left = 388
      Top = 103
      Width = 113
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 9
    end
    object exPedido: TEdit
      Left = 502
      Top = 103
      Width = 113
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 10
    end
    object exVehiculo: TEdit
      Left = 616
      Top = 103
      Width = 113
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 11
    end
    object nCopias: TnbDBNumeric
      Left = 712
      Top = 171
      Width = 17
      Height = 21
      About = 'NB 0.1/20020725'
      CharCase = ecUpperCase
      Text = '3'
      TabOrder = 14
      NumIntegers = 1
    end
    object UpDown1: TUpDown
      Left = 729
      Top = 171
      Width = 16
      Height = 21
      Associate = nCopias
      Max = 9
      Position = 3
      TabOrder = 15
    end
    object chkCuatroDecimales: TCheckBox
      Left = 388
      Top = 53
      Width = 221
      Height = 17
      TabStop = False
      Caption = 'Precio Cuatro e Importe Tres Decimales'
      TabOrder = 8
      OnClick = chkCuatroDecimalesClick
    end
    object edtIncoterm: TEdit
      Left = 440
      Top = 126
      Width = 288
      Height = 21
      TabStop = False
      Color = 14737632
      ReadOnly = True
      TabOrder = 12
    end
    object edtPrecioCaja: TBEdit
      Left = 568
      Top = 193
      Width = 70
      Height = 21
      InputType = itReal
      MaxDecimals = 3
      TabOrder = 16
    end
    object edtPrecioPalet: TBEdit
      Left = 568
      Top = 230
      Width = 70
      Height = 21
      InputType = itReal
      MaxDecimals = 3
      TabOrder = 18
    end
  end
  object StringGrid1: TStringGrid
    Left = 0
    Top = 0
    Width = 755
    Height = 148
    Align = alClient
    Color = 14737632
    ColCount = 13
    DefaultColWidth = 10
    DefaultRowHeight = 18
    FixedColor = clScrollBar
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goEditing]
    TabOrder = 0
    OnDrawCell = StringGrid1DrawCell
    OnSelectCell = StringGrid1SelectCell
    OnSetEditText = StringGrid1SetEditText
    ColWidths = (
      10
      165
      125
      185
      64
      37
      63
      65
      10
      10
      10
      10
      10)
  end
end
