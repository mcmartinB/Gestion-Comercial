...
begin
  ...
  try
    ...
    QRLExpedicionesResumen.LabelCentro := 'Centro expedición: ' + Trim(STCentro.Caption);
    QRLExpedicionesResumen.LabelSemana := 'Semana nº. ' +
      funcTiempo(1, MEDesde.Text) + '   Del ' +
      funcTiempo(2, MEDesde.Text) + ' al ' +
      funcTiempo(2, MEHasta.Text) + ' de ' +
      funcTiempo(3, MEDesde.Text);
    QRLExpedicionesResumen.LabelComunicado := 'Número de orden de comunicación: ' +
      funcTiempo(1, MEDesde.Text) + '/' +
      Copy(funcTiempo(3, MEDesde.Text), 3, 2);
...
procedure TFLExpediciones.eAgrupacionChange(Sender: TObject);
begin
...
begin
{
  ...
  auxComunitario := ' and ( 1=1 ';
  if ckComunitario.Checked then
    auxComunitario := auxComunitario + ' or ((es_comunitario_c = ' + QuotedStr('S') + ' OR es_comunitario_c IS NULL) AND    pais_c <> ' + QuotedStr('ES') + ') ';
  if ckExtra.Checked then
    auxComunitario := auxComunitario + ' or (es_comunitario_c = ' + QuotedStr('N') + ') ';
  if ckNacional.Checked then
    auxComunitario := auxComunitario + ' or (pais_c = ' + QuotedStr('ES') + ') ';
  auxComunitario := auxComunitario + ')';
  ...
  begin
    ...
    SQL.Add(' SELECT empresa_sl, centro_salida_sl, n_albaran_sl, fecha_sl, ' +
      ...
      if EAgrupacion.Text <> '' then
        SQL.Add('and   producto_sl in (select producto_a from frf_agrupacion where codigo_a = '+ QuotedStr(EAgrupacion.Text) + ') ' );
...
begin
{
  ...
  begin
    if auxOr then
      ...
      auxComunitario := auxComunitario + ' ((es_comunitario_c = ' + QuotedStr('S') + ' OR es_comunitario_c IS NULL) AND    pais_c <> ' + QuotedStr('ES') + ') ';
    auxOr := true;
  end;
  if ckExtra.Checked then
  begin
    if auxOr then
      ...
      auxComunitario := auxComunitario + ' (es_comunitario_c = ' + QuotedStr('N') + ') ';
    auxOr := true;
  end;
  if ckNacional.Checked then
  begin
    if auxOr then
      ...
      auxComunitario := auxComunitario + ' (pais_c = ' + QuotedStr('ES') + ') ';
    auxOr := true;
  end;
  auxComunitario := auxComunitario + ')';
...
//    if RGEuropa.ItemIndex = 0 then
    ...
    begin
      ...
      //Agrupacion producto
      if EAgrupacion.Text <> '' then
        SQL.Add(' and   producto_tl in (select producto_a from frf_agrupacion where codigo_a = '+ QuotedStr(EAgrupacion.Text) + ') ');
