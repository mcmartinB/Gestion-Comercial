...
type
  TFMSalidasServiciosVenta = class(TForm)
    rejilla: TDBGrid;
...
var
  FMSalidasServiciosVenta: TFMSalidasServiciosVenta;
...
procedure TFMSalidasServiciosVenta.CargaParametros( const AEmpresa: string; const AServicio: integer;
                                              const AFecha: TDateTime; const AMatricula: string );
begin
...
procedure TFMSalidasServiciosVenta.ReOpen;
begin
...
procedure TFMSalidasServiciosVenta.FormCreate(Sender: TObject);
begin
  with QSalidasPosibles do
  begin
    SQL.Clear;
    ...
    SQL.Add('        transporte_sc, ( select descripcion_t from frf_transportistas where transporte_t = transporte_sc ) transportista, ');
  ...
  begin
    ...
    SQL.Add(' select centro_salida_sc centro, n_albaran_sc albaran, fecha_sc fecha, ');
    SQL.Add('        cliente_sal_sc cliente, dir_sum_sc suministro, vehiculo_sc matricula, ');
    SQL.Add('        transporte_sc transporte, ( select descripcion_t from frf_transportistas where transporte_t = transporte_sc ) transportista, ');
    SQL.Add('        sum(n_palets_sl) palets ');
...
procedure TFMSalidasServiciosVenta.btnCerrarClick(Sender: TObject);
begin
  Close;
end;


procedure TFMSalidasServiciosVenta.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  with QSalidasPosibles do
  begin
    Close;
...
procedure TFMSalidasServiciosVenta.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
function TFMSalidasServiciosVenta.GastosModificados: boolean;
begin
  result := bGastoModificado;
end;

procedure TFMSalidasServiciosVenta.Borrar;
begin
...
procedure TFMSalidasServiciosVenta.Anyadir;
begin
...
procedure TFMSalidasServiciosVenta.btnBorrarClick(Sender: TObject);
begin
  Borrar;
end;

procedure TFMSalidasServiciosVenta.btnAnyadirClick(Sender: TObject);
begin
  Anyadir;
end;

procedure TFMSalidasServiciosVenta.DBGrid1DblClick(Sender: TObject);
begin
  Borrar;
end;

procedure TFMSalidasServiciosVenta.rejillaDblClick(Sender: TObject);
begin
  Anyadir;
end;

procedure TFMSalidasServiciosVenta.btnFiltarClick(Sender: TObject);
begin
  with QSalidasPosibles do
  begin
    Close;
