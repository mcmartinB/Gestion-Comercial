...
uses
  LiqProdVendidoDM, CGlobal, UDMAuxDB, bMath;

procedure TDMLiqProdVendidoTransitos.DataModuleDestroy(Sender: TObject);
begin
  //
end;
...
procedure TDMLiqProdVendidoTransitos.TransitosSemana(const AKey, AEmpresa, ACentro, AProducto, ASemana: string;
                             const ADesde, AHasta: TDateTime; var VKilosIn, VKilosOut: Real);
begin
  sEmpresa:= AEmpresa;
  sCentro:= ACentro;
...
function TDMLiqProdVendidoTransitos.GetCosteEnvasado: real;
begin
...
procedure TDMLiqProdVendidoTransitos.PutKilosTransitosSal;
var
 raux: real;
begin
  rKilos1:= 0;
...
procedure TDMLiqProdVendidoTransitos.AltaTransitosSal;
begin
...
procedure TDMLiqProdVendidoTransitos.PutKilosTransitosEnt;
begin
  rKilos1:= 0;
  rKilos2:= 0;
  rKilos3:= 0;
...
procedure TDMLiqProdVendidoTransitos.AltaTransitosEnt;
begin
  with DMLiqProdVendido do
  begin
    (*
...
procedure TDMLiqProdVendidoTransitos.ValorarTransitos;
begin
  (*
  DMLiqProdVendido.kmtCentros.First;
  while not DMLiqProdVendido.kmtCentros.Eof do
  begin
    if DMLiqProdVendido.kmtSemana.Locate('keysem', DMLiqProdVendido.kmtCentros.FieldByName('keysem').AsString, [] ) then
    begin
      ValorarTransitosSalida;
      ValorarTransitosEntrada;
    end;
    DMLiqProdVendido.kmtCentros.Next;
  end;
  *)
end;

procedure TDMLiqProdVendidoTransitos.ValorarTransitosSalida;
var
  sAux: string;
  rPrecioGastos:  Real;
begin
...
procedure TDMLiqProdVendidoTransitos.ValorarTransitosEntrada;
var
  sAux: string;
begin
  (*
  ...
  begin
    ...
    sAux:= DMLiqProdVendido.kmtCentros.FieldByName('keysem').AsString;
    kmtTransitosIn.Edit;
    if kmtAux.Locate('keysem', sAux, [] ) then
    begin
...
function TDMLiqProdVendidoTransitos.GetGastoTransito: real;
var
