...
type
  ...
  private
    ...
    function EsOrdenTransito (const AOrden: string): boolean;
...
function TFDRecepcionTransitos.EsOrdenTransito(const AOrden: string): boolean;
begin
  with DMRecepcionForm.QAux do
  begin
    if Active then Close;

    SQL.Clear;
    SQL.Add(' select cliente_sal_occ from frf_orden_carga_c ');
    SQL.Add('  where orden_occ = :orden ');

    ParamByName('orden').AsString:= AOrden;
    Open;

    if FieldByName('cliente_sal_occ').AsString = ''  then
      result := true
    else
      result := false;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if QPaletPbLocal.Active then QPaletPbLocal.Close;
        QPaletPbLocal.ParamByName('sscc').AsString:=  QPaletPbRemota.FieldByName('sscc').AsString;
        QPaletPbLocal.Open;
        if (QPaletPbLocal.IsEmpty) or
           (QPaletPbLocal.FieldByName( 'Status' ).AsString = 'S') or
           (QPaletPbLocal.FieldByName( 'Status' ).AsString = 'T') or
           ((QPaletPbLocal.FieldByName( 'Status' ).AsString = 'C') and (EsOrdenTransito( QPaletPbLocal.FieldByName( 'orden_carga' ).AsString) ) ) then
        begin
          if (QPaletPbLocal.FieldByName( 'Status' ).AsString = 'S') or
             (QPaletPbLocal.FieldByName( 'Status' ).AsString = 'T') or
             ((QPaletPbLocal.FieldByName( 'Status' ).AsString = 'C') and (EsOrdenTransito( QPaletPbLocal.FieldByName( 'orden_carga' ).AsString ) ) ) then
          begin
              //Si existen en Local y estan en Stock o en Transito o cargado en un Transito, lo borramos.
              if QBorrarPaletPbLocal.Active then QBorrarPaletPbLocal.Close;
              QBorrarPaletPbLocal.ParamByName('sscc').AsString:=  QPaletPbRemota.FieldByName('sscc').AsString;
              QBorrarPaletPbLocal.ExecSQL;
          end;
        ...
        begin
          lblMessage.Caption := 'Palet de proveedor "' + QPaletPbRemota.FieldByName('sscc').AsString + '" ya cargado o volcado.';
        end;
        QPaletPbLocal.Close;
        QPaletPbRemota.Next;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if QBuscarPaletCabLocal.Active then QBuscarPaletCabLocal.Close;
        QBuscarPaletCabLocal.ParamByName('ean128_cab').AsString:=  QPaletCabRemota.FieldByName('ean128_cab').AsString;
        QBuscarPaletCabLocal.Open;
        if (QBuscarPaletCabLocal.IsEmpty) or
           (QBuscarPaletCabLocal.FieldByName( 'status_cab' ).AsString = 'S') or
           (QBuscarPaletCabLocal.FieldByName( 'status_cab' ).AsString = 'T') or
           ((QBuscarPaletCabLocal.FieldByName( 'status_cab' ).AsString = 'C') and (EsOrdenTransito( QBuscarPaletCabLocal.FieldByName( 'orden_carga_cab' ).AsString) ) ) then
        begin
          inc( iPalet );
  ...
  begin
    ...
    begin
      ...
      begin
        if QBuscarPaletCabLocal.Active then QBuscarPaletCabLocal.Close;
        QBuscarPaletCabLocal.ParamByName('ean128_det').AsString:=  QPaletDetRemota.FieldByName('ean128_det').AsString;
        QBuscarPaletCabLocal.Open;
...
begin
  ...
  begin
    ...
    begin
      ...
      while not QPaletCabRemota.Eof do
      begin
        if qryPaletCabSGPLocal.Active then qryPaletCabSGPLocal.Close;
        qryPaletCabSGPLocal.ParamByName('ean128_cab').AsString:=  QPaletCabRemota.FieldByName('ean128_cab').AsString;
        qryPaletCabSGPLocal.Open;
        if (qryPaletCabSGPLocal.IsEmpty) or
           (qryPaletCabSGPLocal.FieldByName( 'status_cab' ).AsString = 'S') or
           (qryPaletCabSGPLocal.FieldByName( 'status_cab' ).AsString = 'T') or
           ((qryPaletCabSGPLocal.FieldByName( 'status_cab' ).AsString = 'C') and (EsOrdenTransito( qryPaletCabSGPLocal.FieldByName( 'orden_carga_cab' ).AsString) ) ) then
        begin
          inc( iPalet );
  ...
  begin
    ...
    begin
      ...
      while not QPaletDetRemota.Eof do
      begin
        if qryPaletDetSGPLocal.Active then qryPaletDetSGPLocal.Close;
        qryPaletDetSGPLocal.ParamByName('ean128_det').AsString:=  QPaletDetRemota.FieldByName('ean128_det').AsString;
        qryPaletDetSGPLocal.Open;

        if (qryPaletDetSGPLocal.IsEmpty) or
           (qryPaletDetSGPLocal.FieldByName( 'status_cab' ).AsString = 'S') or
           (qryPaletDetSGPLocal.FieldByName( 'status_cab' ).AsString = 'T') or
           ((qryPaletDetSGPLocal.FieldByName( 'status_cab' ).AsString = 'C') and (EsOrdenTransito( qryPaletDetSGPLocal.FieldByName( 'orden_carga_cab' ).AsString) ) )then
        begin
          inc( iPalet );
