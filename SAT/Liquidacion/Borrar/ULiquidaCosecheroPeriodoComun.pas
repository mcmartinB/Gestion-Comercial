(*
  2005.04.19 PACO dijo que los abonos en su totalidad se restarian a la primera
             en la liquidacion
*)
unit ULiquidaCosecheroPeriodoComun;
...
uses
  UDMAuxDB, UDMBaseDatos, bSQLUtils, CGestionPrincipal, RLiquidaCosecheroPeriodo,
...
begin
  ...
  begin
    ...
    begin
      DecodeDate(ini, anyo, mes, dia);
      CosteEnvasadoSemanal_(AUsuario, AEmpresa, ACentro, AProducto, IntToStr(anyo), IntToStr(mes), ANyoSemana(ini));
    end
    else
    begin  //Hay datos de por lo menos dos semanas
      ...
      DecodeDate(fin, anyo, mes, dia);
      CosteEnvasadoSemanal_(AUsuario, AEmpresa, ACentro, AProducto, IntToStr(anyo), IntToStr(mes), ANyoSemana(fin));
...
begin
  ...
  QRLiquidaCosecheroPeriodo := TQRLiquidaCosecheroPeriodo.Create(Application);

  PonLogoGrupoBonnysa(QRLiquidaCosecheroPeriodo, AEmpresa);
  with QRLiquidaCosecheroPeriodo.QCosecheros do
  begin
    Cancel;
    Close;
    SQL.Clear;
  ...
  with QRLiquidaCosecheroPeriodo.QKGSCosechados do
  begin
    Cancel;
    Close;
    SQL.Clear;
  ...
  with QRLiquidaCosecheroPeriodo.QFOB do
  begin
    Cancel;
    Close;
    SQL.Clear;
  ...
  with QRLiquidaCosecheroPeriodo do
  begin
    empresa := AEmpresa;
    //Centro
