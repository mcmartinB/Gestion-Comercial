...
unit LiquidarEntradasSalidasPeriodoFD;
...
type
  TFDLiquidarEntradasSalidasPeriodo = class(TForm)
...
uses CVariables, CAuxiliarDB, CGestionPrincipal, bSQLUtils,
  UDMAuxDB, bMath,  bDialogs, UDMBaseDatos,
  LiquidarEntradasSalidasPeriodoDM;


{$R *.DFM}

procedure TFDLiquidarEntradasSalidasPeriodo.BBCancelarClick(Sender: TObject);
begin
...
function TFDLiquidarEntradasSalidasPeriodo.ValidarParametros: Boolean;
var
  aux: string;
...
procedure TFDLiquidarEntradasSalidasPeriodo.BBAceptarClick(Sender: TObject);
begin
  ...
  begin
    if not LiquidarEntradasSalidasPeriodoDM.ValorarPendiente(sEmpresa, sCentro, sProducto, dFechaIni, dFechaFin, sCosechero,
                                                             cbbTipo.ItemIndex, chkDefinitiva.Checked  ) then
    begin
      case cbbTipo.ItemIndex + 1 of
          0: //pendientes
             begin
               ShowMessage('Sin entradas o todas marcadas como defintivas.');
             end;
          else ShowMessage('Sin entradas para los parametros introducidos.');
      end;
    end;
  end;
...
procedure TFDLiquidarEntradasSalidasPeriodo.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  BEMensajes('');
  gRF := nil;
  gCF := nil;
...
procedure TFDLiquidarEntradasSalidasPeriodo.FormCreate(Sender: TObject);
begin
  FormType := tfOther;
  BHFormulario;
...
procedure TFDLiquidarEntradasSalidasPeriodo.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFDLiquidarEntradasSalidasPeriodo.ADesplegarRejillaExecute(Sender: TObject);
begin
...
procedure TFDLiquidarEntradasSalidasPeriodo.PonNombre(Sender: TObject);
begin
    if (gRF <> nil) then
...
procedure TFDLiquidarEntradasSalidasPeriodo.EProductoChange(Sender: TObject);
begin
  PonNombre(Sender);
end;
